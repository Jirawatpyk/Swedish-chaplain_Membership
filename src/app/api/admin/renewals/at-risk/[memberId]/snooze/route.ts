 ...
 * carries `F8.AT_RISK_SNOOZE` with some suffix — so an alert keyed on `F8.AT_RISK_SNOOZE.*` matches those.
 *
 ...
 */
const ERROR_ID = 'F8.AT_RISK_SNOOZE';
...
export async function POST(
  ...
  try {
    ...
    if (!result.ok) {
      ...
      return assertNever(
        result.error,
        `snooze-at-risk-member: unhandled error kind '${(result.error as { readonly kind: string }).kind}'`,
      );
    }

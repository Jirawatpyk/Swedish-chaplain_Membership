...
/**
 * Route-level error boundary for /portal/renewal/[memberId]. Recoverable,
 * page-scoped error (Retry + `error.digest`) in the detail container so a throw
 * in the renewal lookup doesn't bubble to the root portal boundary.
 */
...
export default function PortalRenewalError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[portal renewal error boundary]', error);
  }, [error]);

...
/**
 * Route-level error boundary for /portal/invoices/[invoiceId]. Recoverable,
 * page-scoped error (Retry + `error.digest`) in the detail container so a throw
 * in the invoice lookup / PDF link doesn't bubble to the root portal boundary.
 */
...
export default function PortalInvoiceDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[portal invoice detail error boundary]', error);
  }, [error]);

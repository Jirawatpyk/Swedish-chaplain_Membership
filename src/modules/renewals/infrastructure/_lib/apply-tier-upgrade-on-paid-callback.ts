 ...
 *     audit — all in F4's tx. F4 commit fails ⇒ the whole apply rolls
 *     back.
 *
 * The F2 `scheduled_plan_changes` row's pending → applied finalisation is
 ...
 * the member-row lock in F4's tx, so finalising here — on a SEPARATE
 * `runInTenant` connection — self-deadlocks (F4's tx is parked in a JS
 ...
 * (the F5 webhook `confirmPayment`, the admin F4 manual-pay route, and — via
 * its own inline post-commit call — `mark-paid-offline.ts`). See
 * `finaliseF2PlanChangeForPaidInvoiceOnline`.
 *
 * Pure Infrastructure — `@/lib/logger`, `@/lib/metrics`, dynamic imports for

...
export async function invitePortal(
  ...
  if (!linked.ok) {
    // Orphan created. We return success to the admin (the invitation
    // email is already in flight) but flag a server_error so the log
    // channel carries the breadcrumb. Choice: treat as success because
    // the invitation email is the user-visible action.
    return ok({

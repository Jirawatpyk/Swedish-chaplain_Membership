...
export async function getMemberBroadcast(
  ...
  if (found.broadcast === null) {
    // Exhaustiveness over `probeKind` is enforced at the type level by
    // the discriminated union returned from `findOwnedByMember`: the
    // `broadcast === null` narrowing here forces `probeKind` to be
    // `'not_found' | 'cross_member'` only (the `'owned'` arm carries a
    // non-null broadcast and is handled below). Adding a new
    // null-broadcast variant to the port type would surface as a TS
    // narrowing error on the if/else-if branches.

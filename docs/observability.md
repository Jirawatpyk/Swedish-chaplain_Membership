...
| `broadcasts.recipient_count.duration_ms` | histogram | `tenant` | **108 PR-C T090** — compose-time recipient count (`GET /api/broadcasts/recipient-count` + the admin proxy variant), observed on every outcome incl. 503 `count_unavailable`. SLO-F7-013 p95 < 3 s @ 20k (FR-043); warn above. |
| `broadcasts.audience_resolved.total` | counter | `tenant`, `segment` (all_members|tier), `mode` (primary_only|all_contacts) | **108 PR-C T090** — one per successful member-based resolve, any phase. The cutover flag flip is visible as the `mode` label changing; after T099 only `all_contacts` remains. |
| `broadcasts.audience_pages.total` | counter | `tenant` | **108 PR-C T090** — F3 keyset pages (1,000 rows) walked per completed 1:N resolve; never incremented on a failed page. Pages per resolve rising toward the per-tick budget is the early warning for a large tenant. |
...
| SLO-F7-013 recipient-count endpoint p95 | < 3 s @ 20,000 contacts (FR-043) | `broadcasts.recipient_count.duration_ms`. **108 PR-C T090** — warn when p95 > 3 s over 15 min; the client shows "count unavailable" on 503, never a stale number (FR-040b). |

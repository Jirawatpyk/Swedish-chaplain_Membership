...
| `broadcasts.dispatch_resolve_failed.total` | counter | `tenant` | **108 PR-C review (errors HIGH-4)** — a `dispatch-scheduled` tick that could not BUILD the audience: `resolve.server_error` → `dispatch.server_error`. The row stays `approved` and is retried next tick. *(Round 2 R2-34: this named `split-large-broadcasts` and `dispatch-batches`, both deleted by `ca51f59a1`; `dispatch-scheduled` is the only claimant. R2-17/round-3 3-7: "NO wall-clock budget" is also no longer true — the import leg now enforces the same 1-hour FR-021 budget as the legacy one, so a persistently retryable failure becomes terminal with `dispatchBudgetExhausted` instead of slipping forever.)* Runbook `broadcast-audience-build.md` § C. |

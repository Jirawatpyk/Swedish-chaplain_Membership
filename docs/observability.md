...
| `broadcasts.recipient_count_ms` | histogram | `tenant` | **108 PR-C T090** — compose-time recipient count (`GET /api/broadcasts/recipient-count` + the admin proxy variant), observed on every outcome incl. 503 `count_unavailable`. SLO-F7-013 p95 < 3 s @ 20k (FR-043); warn above. |
| `broadcasts.audience_resolved.total` | counter | `tenant`, `segment` (all_members|tier), `mode` (primary_only|all_contacts) | **108 PR-C T090** — one per member-based resolve that REACHED the source read (attempts, not sends: a resolve later refused as empty or too large is counted) — was documented as one per successful member-based resolve, any phase. The cutover flag flip is visible as the `mode` label changing; after T099 only `all_contacts` remains. |
...
| `broadcasts.approved_overdue_count` ≥ 1 sustained 30 min | **alarm** | a schedule is slipping tick after tick | `docs/runbooks/broadcast-audience-build.md` § C |

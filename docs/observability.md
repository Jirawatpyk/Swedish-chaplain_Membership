...
| `broadcasts.dispatch_resolve_failed.total` | counter | `tenant` | **108 PR-C review (errors HIGH-4)** — a dispatch tick (dispatch-scheduled, split-large-broadcasts, dispatch-batches) that could not BUILD the audience: `resolve.server_error` → `dispatch.server_error`. The row stays `approved` and is retried next tick with NO wall-clock budget (that budget is FR-021's, Resend branch only), so until this counter existed a broadcast could slip its schedule forever log-only. Runbook `broadcast-audience-build.md` § C. |
...
| `broadcasts.batch_no_progress_count` | gauge | `tenant` | **108 Phase 9b (T144, FR-044 f)** — `sending` broadcasts holding a `pending` batch manifest that have moved NO manifest to a terminal status in the last 30 min, sampled by the `broadcasts-gauges` cron. `stuck_sending_count` fires at 24 h, which was an adequate proxy while a batched broadcast was a rarity; once the split threshold became the per-tick batch size a 100-batch send legitimately spends hours in `sending`, so duration stopped being evidence. Progress is: a healthy multi-tick broadcast retires ≥ 1 manifest per 5-minute tick, so 30 min with none is six missed ticks. Zero-filled per tenant (the same latch class as `approved_overdue_count`). |
...
| `broadcasts.batch_no_progress_count` ≥ 1 sustained 30 min | **alarm** | a batched broadcast has stopped advancing — most likely the T143 drift halt (its audience changed between ticks, so every remaining manifest would slice a shifted list). NOT a page: nothing is lost and no retry fixes it; the recovery is a staff cancel + re-submit. Auto-resolves when the gauge returns to 0. | `docs/runbooks/broadcast-audience-build.md` § C |

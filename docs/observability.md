...
| `broadcasts.audience_import_stuck_count` ≥ 1 sustained 30 min | **alarm** | a Contacts-Import has not completed in half an hour. Nothing is lost — the broadcast is terminal or about to be — so this is not a page; what it tells you is whether Resend's import pipeline is answering at all. Check several tenants / several broadcasts before treating it as one broadcast's problem. | `docs/runbooks/broadcast-audience-build.md` § C |

...
| `broadcasts.audience_pages.total` | counter | `tenant` | **108 PR-C T090** — F3 keyset pages (5,000 rows) walked per completed 1:N resolve; never incremented on a failed page. Pages per resolve rising toward the per-tick budget is the early warning for a large tenant. |

...
| `broadcasts.failed_to_dispatch.count` | counter | `tenant`, `failure_reason` (resend_5xx\|resend_429\|resend_403\|app_error\|timeout\|member_ineligible) | Dispatch-failure forensics. `member_ineligible` (F119 PR-A, § 29.8) is a send-time standing refusal, not a fault |
...
`readMemberSendStanding`) before any Resend call. **Unflagged**: it applies to every F7 send. It
adds no metric; it adds one label and one phase to two existing series, and no log field (the
refusal logs the closed `dispatchFailureKind` token on the import leg, never an address or a
member name).

| Series | Change | Meaning |
|---|---|---|
| `broadcasts_failed_to_dispatch_count` | `failure_reason` += **`member_ineligible`** | A standing refusal: the member was halted, suspended or terminated when the send came due, and the row went `failed_to_dispatch` (`failure_reason` column `member_halted` / `member_not_in_good_standing`). A decision about the member, **not** a fault — it never counts as `app_error`. The computed `broadcasts.dispatch_failure_rate` (> 10 % alert) sums every label, and a refused row never reaches `send_started`, so a burst of refusals can raise it: group by `failure_reason` first — `member_ineligible` is members losing standing, not infrastructure. |
| `broadcasts_dispatch_resolve_failed_total` | `phase` += **`standing`** (both legs) | The standing read itself failed (the F3 halt list threw, or the F8 access lookup answered its error arm). Nothing was sent and the row stays `approved`; the next tick asks again. The § 22 alarm (> 0 sustained 15 min) covers it; `standing` → Neon / the F3 and F8 reads, not the audience resolver. The route's `cron.broadcasts.dispatch.server_error` warn carries the `errClass`. |
...
broadcast. A refusal also writes the refusal audit row under submit's own event types with
`surface: 'dispatch'` (actor `system:cron`, `actor_role: null`) in the terminal transaction.

...
| `broadcasts.dispatch_budget_exhausted.count` | counter | `tenant`, `sub_kind` (network\|timeout\|server_5xx\|api) | **Phase 8 / FR-021 / AS2 (E2 verify-fix 2026-05-02)** — incremented when the 1-hour retry budget elapses with Resend still failing → row transitioned to `failed_to_dispatch`. **Budget anchor (F119 PR-E, mig `0311`):** the hour counts from the FIRST retryable failure of the current dispatch attempt (`broadcasts.dispatch_first_failed_at`, stamped by `markDispatchRetryStarted`, which RETURNS the effective stamp the budget uses; reset on any status change or re-time, on every held tick and right after a successful `sendBroadcast`), not from `scheduled_for` — before `0311` a row resumed more than an hour late (a suspended-member hold, a READ_ONLY_MODE freeze) paged here on its first blip. Residual: a READ_ONLY_MODE freeze does not reset a clock already running (runbook `cron-jobs.md` § Read-only mode — re-time after a long freeze). A retryable throw of the legacy leg's inherited-id probe is never budgeted (`broadcasts.dispatch.inherited_probe_retryable`, warn). A failed stamp write logs `broadcasts.dispatch.retry_epoch_stamp_failed` / `broadcasts.audience_import.retry_epoch_stamp_failed` (warn, `err` = class only), increments the counter below, and the tick stays retryable; a failed reset logs `broadcasts.{dispatch,audience_import}.retry_clock_clear_failed` (warn) and changes no outcome. **Steady state = 0**; any non-zero count in a 15-minute window pages on-call (a member's scheduled E-Blast did not go out). Backed by audit event `broadcast_failed_to_dispatch` + Slice E member transactional notification email. |
...
- **HELD** — the membership is `suspended` (F8: `awaiting_payment`, `pending_admin_reactivation`,
  or an unpaid period that has ended). Nothing is sent or written, the row stays `approved`, and

...
| `broadcasts_retention_provider_copy_kept_total` | counter | `tenant`, `reason` (`transient` \| `refused`) | Expired E-Blasts KEPT because their Resend copy could not be deleted before the row. `transient` (5xx / 429 / network) retries on the next run; `refused` (a 4xx) repeats daily until an operator acts. Resend documents that a SENT broadcast cannot be deleted (unmeasured here), so from ~2031 a steady `refused` rate is the expected signature of that — `docs/runbooks/cron-jobs.md` § F7 retention-sweep "Resend copies". Log: `broadcasts.retention_sweep.provider_copy_kept` (warn; `tenantId`, `outcome`, `errorKind`, `subKind` / `code` when present — no ids, never the provider's text). |
...
`completed`, `provider_copy_kept_transient`, `provider_copy_kept_refused`,

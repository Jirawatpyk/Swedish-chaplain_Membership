...
Extends § 22.1 with **4 new metrics** for pagination (US1) + image embedding (US2). Module path: `src/lib/metrics/broadcasts-f71a.ts`. Cardinality discipline carries from § 22.1; the explicit `broadcast_id` label on `broadcasts.manual_retry_count` is intentional for ad-hoc forensics and is documented in the module header.

...
| `broadcasts.dispatch_resolve_failed.total` | counter | `tenant` | **108 PR-C review (errors HIGH-4)** — a `dispatch-scheduled` tick that could not BUILD the audience: `resolve.server_error` → `dispatch.server_error`. The row stays `approved` and is retried next tick. *(Round 2 R2-34: this named `split-large-broadcasts` and `dispatch-batches`, both deleted by `ca51f59a1`; `dispatch-scheduled` is the only claimant. Round 4 D6 — an earlier version of this cell said "NO wall-clock budget" was "no longer true, the import leg now enforces the same 1-hour FR-021 budget". **That sentence was written between two changes and is false again.** Round 3 (3-7) did add a resolve-side budget on the import leg; round 4 (F3) deleted it, because it measured from `scheduledFor ?? approvedAt ?? createdAt` rather than from first failure — so a row already an hour past its epoch died on its FIRST transient DB error — and because it made the same fault terminal on one leg and retryable on the other, decided by a flag. **Both legs now budget GATEWAY failures only.** A resolve failure leaves the row `approved` and this counter is the alarm, on both. That is the whole reason the counter exists.)* Runbook `broadcast-audience-build.md` § C. |

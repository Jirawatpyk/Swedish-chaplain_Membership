...
| `outbox_permanent_failures_total` | counter | `{notification_type, reason}` where `reason ∈ {max_retries, invalid_recipient, no_template_handler, attachment_sha_mismatch, request_gone, recipient_gone, request_not_decided, read_failed}` (`request_gone` / `recipient_gone` / `request_not_decided`: deterministic misses, first tick; `read_failed`: an F119 `eblast_*` row whose dependency read failed on every attempt — #400 item 4) | `permanently_failed` flips after 5 retries or unrenderable payload |

...
- [ ] **Before MERGING the Phase-9 branch**: run `scripts/inventory-broadcast-outbox.ts` against prod. **CORRECTED 2026-09-08** — this line used to say the merge lands a ceiling *clamp*, and that any `approved` row above 500 would become a terminal `failed_to_dispatch` with the FR-021 email. That clamp never merged. Phase 9b replaced it with batching, so the merge leaves the accepted ceiling **UNCHANGED** (5,000) and instead lowers the SPLIT THRESHOLD 10,000 → 500. A row above 500 is **split and delivered across ticks, not refused** — do not cancel broadcasts the system will handle by itself. Run the inventory anyway: it is the safety net for the batching-OFF state, and a quiet outbox is worth confirming before any behaviour change. Empty at 13:30 on 2026-09-08 — re-run rather than cite, the snapshot ages the moment anyone approves a broadcast.

...
- [x] **T095 — MEASURED 2026-09-08 12:41.** `ratelimit-policy: 10;w=1` read from the API (account limit **10 req/s**, so the "~2 req/s" in four source comments was wrong), but the push is a serial `await` loop, so `throughput = min(account_limit, 1 / RTT)` and the warm RTT is ~0.29 s ⇒ **~3.45 req/s — latency binds, not the plan**. `per_tick_max ≈ 827`; the enforced ceiling is now 800. Full record + caveats in `research.md` § R9 (T095 block) and `reviews/cutover.md` § 5a; capture command kept there for re-measuring from `sin1` on the first real send.
- [ ] **Before MERGING the Phase-9 branch** (the ceiling clamp lands on merge, not on the flip): run `scripts/inventory-broadcast-outbox.ts` against prod. Any `approved` row above 800 becomes a terminal `failed_to_dispatch` on its next tick, audited, with the FR-021 email to the member. Empty at 13:30 on 2026-09-08 — re-run rather than cite, the snapshot ages the moment anyone approves a broadcast.

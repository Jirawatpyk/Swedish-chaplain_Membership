...
### Task 8: `issueCreditNote` (event) + `getInvoiceForPayment` null-member (B5, NF-B, B6)

**Files:** Modify `issue-credit-note.ts`, `get-invoice-for-payment.ts:61-67`, `src/modules/payments/.../initiate-payment.ts` (F5 ownership check). Use the `F4NonTimelineEventType` alias added in Task 7. (`event_buyer_pii_redacted` audit type moved to Task 15 where it is emitted — adding it here would dangle an unemitted type past the audit-coverage test.)

> **BUG to fix (found in review):** `issue-credit-note.ts` ~L222-223 does `const memberId = loaded.memberId; if (memberId === null) return err({ code: 'no_snapshot_on_invoice' });` — this WRONGLY blocks crediting a non-member event invoice (which HAS a pinned buyer snapshot, just no memberId). The real snapshot guard is already at ~L206-211 (`!loaded.memberIdentitySnapshot` etc.). Remove the `memberId === null` early-return; keep `memberId` as a nullable local used only to branch the audit.
> **VAT-inclusive credit notes are already correct:** `calculateCreditNoteVat` uses the STORED `loaded.subtotal`/`loaded.vat` (the Task-7 split), NOT a recompute from lines — so event credit notes reconcile exactly. Verify, don't rebuild.

- [ ] **Step 1:** failing tests — (a) credit-note a full + partial amount on an issued NON-member event invoice → succeeds, `credit_note_issued` via the NON-timeline branch (no `member_id`, has `event_registration_id`), VAT reconciles to the stored split, no `no_snapshot_on_invoice` crash; (b) `getInvoiceForPayment` maps a non-member event invoice with `memberId: null` (not a crash/forbidden); (c) a matched-member event credit note still emits via the timeline branch.
- [ ] **Step 2:** Run — FAIL.
- [ ] **Step 3:** implement: (i) remove the `memberId === null → no_snapshot_on_invoice` guard; (ii) credit-note audit emit — branch `memberId === null` → `credit_note_issued as unknown as F4NonTimelineEventType` non-timeline payload (no `member_id`, add `event_registration_id`); else timeline as today; (iii) the credit-note email — guard `if (recipientEmail) enqueue` (non-member snapshot email may be empty); (iv) widen `InvoiceForPayment.memberId: string | null` (update the DTO doc comment — payment binds to tenant+invoice; member binding optional); (v) in `initiate-payment` (F5), skip the F3 member-ownership check when the invoice `memberId` is null (non-member event invoices are not member-self-payable; paid by admin-record / link) — confirm no other F5 path assumes non-null memberId.
- [ ] **Step 4:** Run — PASS (+ existing membership credit-note + payment tests green).
- [ ] **Step 5:** typecheck + lint + `pnpm check:audit-events`/`check:audit-counts` + commit.

---

## Phase 6 — PDF (event_fee line)

### Task 9: `invoice-template.tsx` event_fee line + **§86/4 doc-type gate (SHIP-BLOCKER)**

**Files:** Modify `issue-invoice.ts` (the `kind` selection — Application-layer, see Step 0), `invoice-template.tsx`; Test: `tests/integration/invoicing/event-invoice-pdf-golden.test.ts` + extend `issue-event-invoice.test.ts`.

- [ ] **Step 0 (SHIP-BLOCKER — thai-tax §86/4 ruling):** the PDF `kind` is currently hard-coded `'invoice'` in `issue-invoice.ts` → every issued invoice renders the full "ใบกำกับภาษี / Tax Invoice" title. Per Thai Revenue Code §86/4, a buyer WITHOUT a 13-digit TIN (non-member individual, `memberSnap.tax_id` null) must NOT receive a full tax invoice with a blank TIN — they must get a **receipt (ใบเสร็จรับเงิน)** or **abbreviated tax invoice (ใบกำกับภาษีอย่างย่อ §86/6)**. Implement an **Application-layer** doc-type gate before the PDF render: `const pdfKind = (memberSnap.tax_id ?? '').trim() !== '' ? 'invoice' : <the receipt/abbreviated kind the template registry supports — check PdfDocKind>;` and pass it instead of the hard-coded `'invoice'`. Confirm the `PdfDocKind` union + template title map support a TIN-less variant (receipt vs abbreviated tax invoice — pick per `template-registry.ts`; receipt is the safe default). **Membership-consistency check:** this gate also fixes the pre-existing membership-individual-without-TIN case (they currently get a full tax invoice too). Run the FULL membership issue + PDF test suite — if any test asserted the old "ใบกำกับภาษี for a TIN-less member" behaviour, that surfaces the pre-existing gap; update those expectations to the §86/4-correct doc-type and REPORT the membership impact (the team may want to confirm). Add integration assertions: non-member WITH tin → `pdfRender.render` called with `kind:'invoice'`; non-member WITHOUT tin → the receipt/abbreviated kind.
- [ ] **Step 1: Failing golden test** — render an event invoice (matched-member buyer, AS-VAT-01 amounts) → assert the line description = event name + CE date (YYYY-MM-DD), the `event_fee` line amount = `1,070.00` (the inclusive Model-B line), the summary subtotal 1,000.00 / VAT 70.00 / total 1,070.00, a "VAT included / ราคารวมภาษีมูลค่าเพิ่มแล้ว" annotation present for `vat_inclusive` invoices, and the correct title per Step 0 (TIN → ใบกำกับภาษี; no TIN → receipt/ย่อ).
- [ ] **Step 2:** Run — FAIL.
...
- [ ] **Add the `event_buyer_pii_redacted` audit type** (moved from Task 8 — add it HERE where it is emitted, so the audit-coverage test doesn't see a dangling unemitted type): F4 audit union (`F4NonTimelineEventType` — it has no member_id) + `F4_AUDIT_RETENTION_YEARS` = **10** (Thai RD §87/3 tax-document retention) + the Drizzle `audit_event_type` pgEnum (own non-tx `ALTER TYPE … ADD VALUE` migration) + the F4 anti-drift tests (`audit-event-type-parity.test.ts` + `audit-coverage.test.ts` — NOT the F1 auth `audit-event.test.ts`/`completeness.test.ts`). Run `pnpm check:audit-events`/`check:audit-counts`.
- Create `POST /api/cron/invoicing/redact-expired-event-buyers` (Bearer `CRON_SECRET`, retry-OFF, `gateCronBearerOrRespond`). Predicate: `invoice_subject='event' AND member_id IS NULL AND status<>'draft' AND issue_date < now() − interval '10 years'`. Action: tombstone `member_identity_snapshot` PII (`[REDACTED]`/`''`) preserving `*_satang`/`document_number`/dates; emit `event_buyer_pii_redacted` per row (non-timeline, no member_id). Runbook entry in `docs/runbooks/cron-jobs.md`. Integration test: a >10y row gets tombstoned, a <10y row untouched, financial fields preserved. NOTE the `invoices_enforce_immutability` trigger locks `member_identity_snapshot` once `status != 'draft'` — the redaction UPDATE must run with a trigger-bypass path (e.g. a `SECURITY DEFINER` function or a documented trigger exception for the retention sweeper) OR the trigger must be amended to allow the tombstone transition; resolve this explicitly (it's the same class as F6's pseudonymisation sweeper — mirror that).

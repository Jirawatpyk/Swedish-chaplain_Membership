...
- [ ] **Step 1: Write failing unit tests** covering: matched member → buyer auto from F3 (uses `memberIdentityAdapter` stub); non-member → manual buyer; `ticket_price_thb` 0/null → `no_fee_free_event`; pseudonymised → `attendee_erased`; `amountOverride` 0/neg/`MAX+1` → `invalid_amount`; non-member bad `tax_id` (`'12'`) → `invalid_tax_id_format`; non-member empty address → `invalid_buyer_snapshot`; company member with null tax_id → `tax_id_required`; happy path → one `event_fee` line, `vat_inclusive=true`, `vat_rate_snapshot=tenant rate`, subtotal/vat via `splitVatInclusive`.
- [ ] **Step 2:** Run — FAIL.
- [ ] **Step 3: Implement** the use-case:
  - zod input: `eventRegistrationId: uuid`, `amountOverride: z.number().int().min(1).max(MAX_EVENT_INVOICE_SATANG).optional()`, optional `buyer` (non-member: `legal_name z.string().min(1).max(500)`, `address .min(1).max(1000)`, `tax_id z.string().regex(/^\d{13}$/).nullable()`, contact name/email).
  - `runInTenant` → `eventRegistrationLookup.findById(tx, …)` → guards → buyer snapshot (matched: `memberIdentityAdapter.getForIssue` + company-no-tin → `tax_id_required`; non-member: `makeMemberIdentitySnapshot(buyer)` → `invalid_buyer_snapshot` on throw) → `total = amountOverride ?? ticketPriceThb*100` (satang) → `splitVatInclusive(total, rateBps)` → one `event_fee` line (`unitPrice/total = subtotal`, `proRateFactor = null`) → `insertDraft({ invoiceSubject:'event', vatInclusive:true, eventId, eventRegistrationId, memberId: matched?member:null, planId:null, planYear:null, subtotal, vat, vatRate, … })`. Dedup: `SELECT … FOR UPDATE WHERE event_registration_id=? AND invoice_subject='event' AND status<>'voided'`; if found → `duplicate`; catch 23505 → `duplicate`.
- [ ] **Step 4:** Run unit + integration (matched + non-member, live Neon) — PASS.
- [ ] **Step 5:** typecheck + lint + commit.
...
**Files:** Modify `issue-invoice.ts` (~L202-209 member-lock, ~L222-228 tax-id gate, ~L233 line guard); Test: extend `tests/integration/invoicing/audit-coverage.test.ts` or a new `issue-event-invoice.test.ts`.

- [ ] **Step 1: Write failing integration test** — issue an `event`/non-member draft (member_id null) → succeeds, allocates the next INV §87 number, pins the buyer snapshot, emits `invoice_issued` via the **non-timeline** audit branch (member_id null), does NOT crash on the member-lock.
- [ ] **Step 2:** Run — FAIL (current `enforceOneMembershipLine` + unconditional member-lock + tax-id gate).
- [ ] **Step 3: Implement:** (a) replace the `enforceOneMembershipLine` call with `enforceOneSubjectLine(draft.invoiceSubject, draft.lines)`; (b) `if (draft.memberId !== null) { …member FOR UPDATE lock + archive guard… }` else validate the pinned snapshot; (c) remove the company-tier tax-id gate here (now enforced at draft, Task 6); (d) audit emit: branch — `member_id === null` → emit `invoice_issued` through the `Exclude<F4AuditEventType, F4MemberTimelineAuditEventType>` branch (payload `Record<string,unknown>` with `event_registration_id` + `contact_email_sha256` omit-when-empty); else timeline branch as today.
...
- [ ] **Step 1: Failing golden test** — render an event invoice (matched-member buyer, AS-VAT-01 amounts) → assert the line description = event name + CE date (YYYY-MM-DD), subtotal 1,000.00 / VAT 70.00 / total 1,070.00, doc-type label per tax-id.
- [ ] **Step 2:** Run — FAIL.
- [ ] **Step 3:** in the line-rendering, when `kind==='event_fee'` show `descriptionTh/En` (event name + CE date — the use-case already stored CE ISO). Membership-only fields guarded by the line kind / subject. (Buyer/seller/§86/§87/doc-type already generic + shipped.)
- [ ] **Step 4:** Run — PASS.
- [ ] **Step 5:** commit.

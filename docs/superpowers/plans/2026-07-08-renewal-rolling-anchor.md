...
- Create: `scripts/backfill-cycle-anchors.ts` — STUB IS FORBIDDEN; write the full script now, run later: reads a CSV (`member_number,payment_date`), for each row `runInTenant` → find member by number → `findOpenCycleForMemberInTx` → `reanchorPeriodInTx` with `anchorInvoiceId: null`, `anchoredAt = now`, period from payment_date; `--dry-run` flag prints the plan without writing; refuses when `DATABASE_URL` host is in `TEST_DB_HOST_BLOCKLIST` conventions do NOT apply (this one targets prod deliberately — instead require explicit `--confirm-prod` flag). Unit-test the CSV parsing + dry-run plan builder (pure part).

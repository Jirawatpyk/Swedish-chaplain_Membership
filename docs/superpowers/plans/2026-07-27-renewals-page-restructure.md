...
## Decisions (agent-finalized 2026-07-27 — build to these)

These were flagged during drafting and have been DECIDED so the plan is
execution-ready. Each is an engineering/UX-sound default; #1 and #3 are
money/tax-facing, so surface the chosen value to the tenant accountant during
Wave 2/3 review and revisit only if their house convention differs. Appendix B
records the original framing; these values govern.

1. **Collection-rate definition (Wave 2 Task 6):** `rate = collected / (collected + overdue)` — **money-weighted, not-yet-due excluded** (a billed-in-advance, not-yet-due invoice is not a collection failure). Divide-by-zero (no dues in window) → render "—". NOT count-based, NOT `collected / billed`. The tile ships **fully** (with its one-click filter shortcut into the overdue bucket), not display-only.
2. **"Collected this period" window (Wave 2 Task 6):** **current Asia/Bangkok calendar month** (`date_trunc('month', now AT TIME ZONE 'Asia/Bangkok')`) — matches monthly reconciliation + bank-statement cadence. A period toggle (month/quarter/year) is a later enhancement, not this plan.
3. **Bulk Mark-paid scope (Wave 3, Principle IV):** **restrict bulk Mark-paid to rows that already carry a live linked invoice** (`linkedInvoiceId != null` / status `awaiting_payment`), so every THB total shown is a real issued-invoice amount — no "—", no guessed number, no mixed dialog. Minting a §86/4 (the `upcoming`/no-bill case) is a per-member tax-document act and stays on the **single-row** guarded flow; such rows render "not bulk-payable" with a note. This is the safest first cut on a live-money screen.
4. **Bulk shared payment reference (Wave 3):** one operator-entered reference applied to all N cycles (models a single batch bank transfer), stored verbatim per-cycle in `audit_log`. Acceptable for SweCham reconciliation; per-row references would make bulk impractical.
5. **Bulk partial-failure UX (Wave 3):** continue-on-error; the run mints each row sequentially and reports an aggregate `ok / skipped / failed` toast (mirrors the shipped members-directory bulk pattern). Per-row orphan-invoice deep-links are NOT surfaced in the aggregate — if a row fails with `f4_orphan_invoice`, the admin resolves it from that cycle's detail page (a known, acceptable limitation for v1).
...
7. **Wave 3 PR split:** the Wave 3 PR ships **Tasks 9–12 only** (settlement-preview + row-selection + bulk-action-bar + mobile card-stack). **Task 13 (Today worklist)** and **Task 14 (saved segments)** split to their own follow-up branches (`nnn-renewals-today-worklist`, `nnn-renewals-saved-views`) to keep the diff reviewable and because both are deferrable stretch items.
8. **THB minor-unit (Wave 3, impl note not a decision):** amounts are satang (`amountThbMinor`, /100 for display); before implementing the Drizzle join, verify the invoices grand-total column's exact unit/name and match what `mark-paid-offline` mints (apply migration + integration test before commit).

...
## Decisions to confirm with the PO / accountant BEFORE building the flagged tasks

1. **Collection-rate definition (Wave 2 Task 6):** the band computes `rate = collected / (collected + overdue)`. Confirm vs `collected / billed-this-period` or `paid-count / issued-count`. Load-bearing for the KPI's meaning — until confirmed the tile ships display-only (no filter shortcut).
2. **"Collected this period" window (Wave 2 Task 6):** implemented as the current Asia/Bangkok calendar month. Confirm vs a rolling 30-day / fiscal-period window.
3. **Bulk Mark-paid THB total truthfulness (Wave 3 Task 3, Principle IV):** `mark-paid-offline` MINTS the §86/4 for `upcoming` cycles with no invoice yet, so no real per-invoice total exists pre-confirm for those rows. The settlement-preview returns `previewable:false` + null amount for them; the confirm dialog shows "—" + a note rather than a guessed number. **Decision needed:** accept the mixed dialog (real totals for `awaiting_payment`/linked-invoice rows, "computed on confirm" for `upcoming` rows), OR restrict bulk Mark-paid to rows with an existing live bill.

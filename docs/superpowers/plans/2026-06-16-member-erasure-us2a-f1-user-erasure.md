...
- **Auth audit registration (4 places)** for `user_erased`:
  ...
  4. `tests/unit/auth/domain/audit-event.test.ts:33` — `expect(AUDIT_EVENT_TYPES).toHaveLength(32)` → bump to 33. (The `all-audit-event-types.test.ts` `>100`/sorted/deduped check auto-passes; the live `assert-enum-parity` integration test passes once the migration is applied.)

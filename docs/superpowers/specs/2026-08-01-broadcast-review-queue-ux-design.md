...
- **Remove dead i18n keys (SHOULD).** `filters.apply`, `pagination.*`, `filters.statusAll` are orphaned (filters are URL-driven, no Apply button, no pagination UI). Delete from all three locales.
...
- **Bulk-approve confirm dialog (MUST).** Currently `queue-table-client.tsx:305` posts a hardcoded `{ decision: 'send_now' }` for every selected row with no confirm. Add a confirm dialog that:
  - summarises **N broadcasts → ~M total recipients** (M = sum of the selected rows' `recipientCount`, already on `EnrichedQueueRow` — no extra query);
  ...
  - on confirm, the fan-out posts the **chosen** decision (`{decision:'send_now'}` or `{decision:'schedule', scheduledAt}`) per row — the approve endpoint already accepts both (single uses schedule).
...
- **Single-approve shows recipient count (SHOULD).** Thread `estimatedRecipientCount` (already on `QueueRow`, `queue-table.tsx:32`) → `ReviewActions` → `ApproveDialog` and display it in the dialog, so a single approve also shows who it reaches.
...
EN/TH/SV parity (`check:i18n`); TH no `italic`; SV bulk strings need `whitespace-nowrap` + wrap (PR2). `text-muted-foreground` stays the empty-`—` sentinel.

## Test impact

- **PR1** — unit: overdue-count derivation, ICU plurals, segment-label mapping; component: bulk-count `aria-live`, `indeterminate` header checkbox, breach banner gating, token classes; e2e: `@a11y` axe (banner + bulk bar), `@i18n` EN/TH/SV (no raw-enum leak in Audience).
...
- **PR3** — component: recipient-total sum, send-now vs schedule branch, confirm-required-before-fan-out; contract/integration: the bulk-schedule fan-out posts `{decision:'schedule', scheduledAt}` correctly (the endpoint's schedule path is already covered for single — add the bulk case); e2e: bulk-approve confirm flow + schedule flow.
...
| **PR3 (WS-F)** | **⚠️ Behaviour change — irreversible SEND path** | **≥ 2 reviewers + tests; approve/reject endpoint semantics UNCHANGED, RBAC UNCHANGED (manager still sees no buttons) — client confirm + decision passthrough only** |
...
- **Pagination** (DEFER) — `page.tsx:144-152` pages at 50 with no Next/Prev UI (`nextCursor` unused). At SweCham scale (~131 members) a pending queue > 50 is near-impossible. **If deferred, PR1 should still add a "showing 50 of {totalPending}" note when `totalPending > 50`** (piggybacks on WS-A's count) so rows never vanish silently.
- **Bulk `BULK_CAP`** — adopt the shared members/renewals cap (100) with an over-cap `role="alert"` when the bulk bar is extracted (PR2), replacing the current cap-less `BULK_CHUNK=5` throttle (throttle stays for concurrency).

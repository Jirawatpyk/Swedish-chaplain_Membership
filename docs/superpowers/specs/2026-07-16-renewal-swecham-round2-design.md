...
> **Wording note (T5-review M2):** `deferred_no_prior_warning` covers two sub-cases — (a) no qualifying warning exists at all (truly unwarnable), and (b) a qualifying warning was sent but **< 14 days ago** (the min-notice window; self-resolving without admin action). The escalation task summary + label are therefore **neutral** — *"no statutory warning ≥ 14 days old"* — true for both, rather than the false *"never warned"*. Both cases are fail-safe; case (b) re-creates the task for ≤ 14 days after an admin closes it — accepted as bounded, non-misleading noise (splitting the outcome to skip (b) would need an extra reminder read in the cron route — not worth it at ~110 members).

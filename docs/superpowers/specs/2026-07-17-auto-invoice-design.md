...
- **Issue + Send** → mints the bill (via `issueAutoDraftedRenewal`), patching `autoEmailOnIssue=true` for the issue so exactly one locale-correct outbox row is enqueued (`resolveRecipientLocale` → primary contact; empty-recipient skipped-with-warning). **Mechanism (review M3):** the override is threaded into the issue path / the draft row is patched before issue — `issue-invoice.ts:858` resolves `draft.autoEmailOnIssue ?? settings.autoEmailEnabled`, so the override must be explicit.
...
- **Cron-side zero tax blast radius** (drafts only) — true. **Issue-side is different (review I4):** `issueAutoDraftedRenewal` inherits the create→issue→link **orphan window** — the SC number mints (issue tx commits) then the link tx fails → an **issued orphan bill with a burned number** that the prune sweep (which targets drafts) cannot catch. **Recovery:** an idempotent link retry (`WHERE linked_invoice_id IS NULL OR = <invoiceId>`) on the next queue action / a reconcile pass, plus admin-void as the manual backstop; covered by an integration test (issue-success, link-fail → retry links, no duplicate).
...
**Correction (review I3/I6):** the ladder does **not** "skip only on unreconciled-paid" — it already stands its email steps down for cycles in **`dueTrackCycleIds`** (`dispatch-one-cycle.ts:618-624`, from `listDueTrackCandidates`). So the handoff is **an extension of that existing mechanism, not a new parallel gate**: (a) `issueAutoDraftedRenewal` transitions the cycle `upcoming|reminded → awaiting_payment` on issue; (b) widen `listDueTrackCandidates` (or its window) so a freshly-issued bill enters `dueTrackCycleIds` immediately — closing the narrow window between issue and the candidate query first picking the cycle up. Net: **one** dunning stream — ladder before a bill exists → due-track after. This also cleans up the pre-existing 066 double-track globally.
...
**Mutual-void interplay:** #1's per-member serialization lock + this content guard together ensure a second concurrent same-member issue **sees the first and refuses**, rather than the two voiding each other. The paid-race latch test (live Neon) is a **failing-first ship blocker** before `FEATURE_AUTO_INVOICE` is flipped.
...
9. **Issue-success / link-fail orphan recovery** — number minted, link tx fails → the idempotent link retry links it, no duplicate.

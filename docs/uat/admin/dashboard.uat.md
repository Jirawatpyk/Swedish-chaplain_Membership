...
| # | ขั้นตอน | ผลที่คาดหวัง |
|---|---|---|
| 1 | ในชุดผลที่มี > 50 รายการ กด **Load older events** | โหลดหน้าถัดไปต่อท้าย (keyset) ไม่ค้าง |
| 2 | แก้ URL ใส่ `from`/`to` รูปแบบผิด (เช่น `from=99-99-99`) | ขึ้นสถานะ **"Invalid filter"** (ไม่ throw / ไม่ขาว) |

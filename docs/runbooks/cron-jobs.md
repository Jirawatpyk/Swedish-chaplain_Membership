...
| **renewals reconcile-coverage-ends** (0305) | **`POST /api/cron/renewals/reconcile-coverage-ends`** | **`37 * * * *`** (hourly) | **`Authorization: Bearer ${CRON_SECRET}`** | Ends a refunded member's coverage once their refund settles `succeeded` (keeps it if the refund failed), retries a failed inline end, and recovers "End membership" decisions whose route call was lost. See `docs/observability.md` § 28 |
...
| `/api/cron/renewals/reconcile-coverage-ends` | `37 * * * *` | hourly at :37 (0305 — its own route so the daily reactivation pass can never skip it) | GET+POST |

...
**Updated**: 2026-07-12 — added `refund.updated` webhook handling (Stripe deprecated
`charge.refund.updated`; charge-less async PromptPay refunds settle via `refund.updated`). OP-2
now requires subscribing to BOTH events; KL-7 redundancy note extended. See
`docs/runbooks/out-of-band-refund.md` § 1.6.
...
| OP-2 | **Subscribe the Stripe endpoint to BOTH `charge.refund.updated` (legacy) AND `refund.updated` (forward path)** — **after** the `processRefundUpdated` handler is deployed | 🔴 | `charge.refund.updated` is **DEPRECATED** ("only sent for refunds with a corresponding charge; listen to `refund.updated` for updates on all refunds instead") but STILL fires for card refunds + backs the OOB forensic redundancy — keep it. **`refund.updated` is the settlement signal for charge-less async refunds (PromptPay/GrabPay/bank transfer)** — WITHOUT it those refunds hang `pending` until the sweep. Both route to the same handler (idempotent). `refund.failed` NOT needed (`refund.updated` carries `status→failed`). Ordering-sensitive: enabling too early swallows events on the `acknowledged_only` 200-ack branch; never enabling hangs every async refund forever. Watch `payments_refund_pending_awaiting_processor_total` (sustained > 0 = subscription broken). See `docs/runbooks/out-of-band-refund.md` § 1.6. |
...
| KL-7 | OOB forensic redundancy assumes the webhook keeps `charge.refunded`, `charge.refund.updated`, and `refund.updated` — runbook §1.4 / §1.6 | Config/ops invariant, not a code gap: dropping one while keeping the others is a Stripe endpoint misconfiguration. On-call dedupes the multi-source audit by `processor_refund_id`; the paging metric is single-owner (`charge.refunded`). `refund.updated` is REQUIRED for charge-less async (PromptPay) settlement, not just redundancy — see OP-2. |

...
- **Do NOT issue a credit note** (§ 2.3 Option A does not apply here). The auto-refund fired because the invoice was already `paid` by another method (cause `invoice_already_paid` — the online payment is a duplicate), or already `void` / `credited`. Either way the online payment is not a sale: a credit note would reduce real output VAT on the ภ.พ.30, turn a correctly paid invoice `credited`, and still return nothing — F4's manual credit-note flow does not call Stripe.

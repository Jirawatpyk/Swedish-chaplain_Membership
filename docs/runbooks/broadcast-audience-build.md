...
6. **Empty check, then the ceiling**: **the ENFORCED ceiling is 800** (`DELIVERABLE_RECIPIENTS_PER_TICK`, 2026-09-08) — `min(configured, 800)`, where `configured` is 5,000 unless BOTH the F7.1a batching flag AND the 1:N flag are ON (then 50,000). **The clamp applies in every flag state and the 800 is what every count, submit and dispatch compares against**; it exists because the serial Resend push delivers ~3.45 req/s and cannot finish more than ~827 inside `maxDuration = 300`. `specs/108-contact-recipient-rules/research.md` § R9 (T095) has the measurement. Over the ceiling → `broadcast_audience_too_large { count, cap }` with the TRUE count — never a cut list.

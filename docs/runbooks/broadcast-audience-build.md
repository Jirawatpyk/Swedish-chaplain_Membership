...
6. **Empty check, then the ceiling**: **the ENFORCED ceiling is 500** (`DELIVERABLE_RECIPIENTS_PER_TICK`, 2026-09-08) — `min(configured, 500)`, where `configured` is 5,000 unless BOTH the F7.1a batching flag AND the 1:N flag are ON (then 50,000). **The clamp applies in every flag state and the 500 is what every count, submit and dispatch compares against**; it exists because the serial Resend push delivers ~2.08 req/s and cannot finish more than ~623 inside `maxDuration = 300`. `specs/108-contact-recipient-rules/research.md` § R9 (T095) has the measurement. Over the ceiling → `broadcast_audience_too_large { count, cap }` with the TRUE count — never a cut list.
...
2. ~~Turn on F7.1a batching to raise the ceiling~~ — **this is a NO-OP since 2026-09-08.** The enforced ceiling is `min(configured, 500)`, so raising the configured ceiling to 50,000 changes nothing an audience can reach, and the split path it opens is unreachable. Narrowing the segment (option 1) or splitting the send by hand are the real remedies until the push stops being serial. Original text: turn on F7.1a batching (`FEATURE_F71A_US1_PAGINATION` in `src/lib/env.ts` — deployment-wide, not per tenant); with the 1:N flag also ON it raises the ceiling to 50,000 and routes the broadcast through `split-large-broadcasts` above `SPLIT_THRESHOLD_RECIPIENTS` = 10,000. With the 1:N flag OFF the ceiling stays 5,000 regardless (review H-2: the wide ceiling belongs to the wide audience).

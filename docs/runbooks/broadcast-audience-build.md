...
6. **Empty check, then the ceiling**: the accepted ceiling is `configured` — 5,000, or 50,000 when BOTH the F7.1a batching flag AND the 1:N flag are ON. **CORRECTED 2026-09-08 (the text here said the enforced ceiling was `min(configured, 500)` in every flag state, contradicting § D two screens down).** That clamp was interim and Phase 9b removed it: `DELIVERABLE_RECIPIENTS_PER_TICK = 500` is now the BATCH SIZE and the split threshold, so an audience above 500 is **split and delivered across ticks, not refused**. The clamp survives only with batching OFF, where there is no split path and the serial push (~2.08 req/s, ~623 per `maxDuration = 300`) is still the real bound. Prod has batching ON. So a submit at 800 recipients is ACCEPTED and split — if you are triaging one that was refused, the cause is the configured ceiling or the resolver, not the per-tick bound.
...
2. **Turn on F7.1a batching to raise the ceiling** — **this IS a real remedy again (un-struck 2026-09-08).** It was marked a NO-OP for part of one day, while an interim clamp held the enforced ceiling at `min(configured, 500)` and made the split path unreachable. Phase 9b removed that clamp and made 500 the batch size instead, so batching now both raises the configured ceiling to 50,000 (with the 1:N flag) and opens the split path that actually delivers it, one wave per tick. Before turning it on, read § C item 4's concurrency-cap note — keep `dispatch_concurrency_cap` ≤ 4 on any plan, and 2 on Resend Free, where the 3-audience cap makes `POST /audiences` fail outright for a third in-flight broadcast (measured 2026-09-08). Narrowing the segment (option 1) remains the faster answer for a one-off.
...
4. **`broadcasts.batch_no_progress_count ≥ 1` for 30 min — a BATCHED broadcast has stopped advancing** (Phase 9b, T143/T144). Alarm, not page: nothing is lost and no retry fixes it.

   Almost always the **drift halt**. `dispatch-batches` re-resolves the audience every tick, but each manifest slices a list frozen at split time (`allRecipients.slice(rangeStart, rangeEnd + 1)`). If the audience changed size since the split — one opt-out is enough — every later slice points at different people, so recipients would be skipped or sent to twice (FR-044 a/d). The route therefore dispatches NOTHING for that broadcast and logs `cron.broadcasts.dispatch_batches.audience_drift_halted` with `expected` (split-time total) and `observed` (now). The response body's `driftHalted` counts them.

   **Recovery: cancel it and ask the member to re-submit.** `cancelBroadcast` as an admin actor releases the quota; the re-submit re-resolves and re-splits against the current audience. There is deliberately **no "accept the new count"** path, and do not add one: the batches already sent used the OLD slices, so accepting would double-send or skip exactly the recipients the drift moved. The manifests stay `pending` until then — no batch of that broadcast goes out, so a halted broadcast is safe to leave over a weekend.

   If the log shows no drift halt, the broadcast is simply large: `dispatch-batches` sends at most `dispatch_concurrency_cap` batches per invocation and defers the rest (`batchesDeferred` in the response). Check that the deferred count is falling tick over tick. **Operator note on `dispatch_concurrency_cap`** (`tenant_broadcast_settings`, default 4, range 1–8): keep it **≤ 4** on any plan — 4 workers × 2.08 req/s ≈ 8.3 sits under Resend's 10 req/s policy, while `MAX_CONCURRENCY_CAP = 8` is still accepted by the setting and would be ~16.6, turning the batch path 429-bound. On Resend **Free**, set it to **2**: the plan allows 3 audiences and `General` holds one, so more than two ephemeral audiences at a time is a permanent 4xx. Free's real bound is ~1,000 contacts per reap window regardless of the cap (`cleanup-audiences` grace is 1 h) — a cap of 2 does not make a 1,200-recipient broadcast deliverable on Free; the plan does. A batch that hits the contact cap fails LOUDLY with an audit row and, since Phase 9b, is **not** auto-retried — that is the intended signal to upgrade, not a bug.

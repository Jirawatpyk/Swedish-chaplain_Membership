...
- **F7.1a — `sending`+batches mid-dispatch halt UI → BUILT (admin-only).** The admin broadcast-detail page now surfaces a **"Halt sending"** action when `status === 'sending'` AND the already-loaded `batches` has ≥1 `pending` (not-yet-dispatched) row (derived with no extra query; fail-safe to hidden on a batch-load failure). It reuses the shared `CancelBroadcastDialog` via `CancelBroadcastAction variant="halt"` → the **same** `/api/admin/broadcasts/{id}/cancel` endpoint (the use-case detects the `sending`+batches state and stops only the pending batches; already-dispatched batches still deliver). New i18n `admin.broadcasts.haltDialog.*` + `toast.halted`; admin `toast.cancelTooLate` reworded to neutral, all-cases-accurate copy ("past the point where it can be stopped") at both the client i18n and the server envelope (`broadcasts-route-helpers.ts`) — avoids the false "started/finished sending" claim that contradicts a Halt control on an in-flight send. **Member path intentionally NOT built** — mid-dispatch halt is an admin-ops action and the member page has no batch data. **Dormant** for SweCham (<10k recipients never split → no pending batches → button never shows), but the surface is otherwise complete + inert for the current tenant. The show/hide gate is a unit-tested pure helper (`can-halt-sending.ts`); the page-RSC integration + the >10k split path stay e2e-only (untested until a >10k tenant onboards).

  **Known limitation (dormant >10k debt) — halt reuses the cancel notification.** A mid-send halt reuses `cancelBroadcast` unchanged, so the member receives the `broadcast_cancelled` email ("Your E-Blast was cancelled" + "quota slot released") and the audit logs `broadcast_cancelled`, even though on a real mid-send halt some batches already delivered. This is **pre-existing F7.1a use-case behaviour** (the API halt path predates this UI; the button only surfaces it) and never fires for SweCham. A halt-specific member notification + audit event + quota-footer reconciliation is future F7.1a work, to be done in the >10k batch-path QA before a >10k tenant goes live (alongside the existing US1 dormant items: H-1 trigger / no-roll-up / double-count).
- **Dialog-family dedup → analyzed → LEAVE-AS-IS (deliberate).** `approve-dialog` (radio + datetime schedule pickers, no reason field), `retry-confirmation-dialog` (budget/warning paragraphs, no reason field, non-destructive), and `accept-partial-dialog` (closest, but adds a summary line + an interpolated counter + lacks an `errors.reasonTooLong` key + uses a 2-link `?? null` finalFocus chain pinned by `dialog-final-focus.test.ts`) each carry load-bearing non-reason UI. Forcing them onto `ReasonConfirmationDialog` would require a `children`/slot escape hatch that reject+cancel don't need — a leaky abstraction (and the hook swap would break the 2-link focus contract test). The shared dialog is correctly scoped to its two genuine peers (reject + cancel); a half-fitting migration here is worse than none.

...
| `broadcast_image_removed` | info | Record of a reference removal or a byte deletion — carries `blob_deleted` and a `reason`: `draft_discarded`, `draft_pruned`, `member_erased` (the three stamping sites, each emitted in the same transaction as the state change) and `sweep` / `sweep_orphaned` / `sweep_referenced` (the three sweep outcomes, so "kept by reference" is audited as distinctly as "deleted"). PR-2 adds the withdrawal and rejection reasons | 5y |

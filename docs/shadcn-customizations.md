...
| `src/components/ui/select.tsx`             | SelectTrigger: `data-[size=default]:h-8` → `data-[size=default]:h-[var(--input-height)]`, `pl-2.5` → `pl-[var(--input-padding-x)]`. **Phase 6 review-round 2 (CR1)**: added `TranslatedSelectValue` component (i18n-safe placeholder rendering for `<Select>` triggers), changed `<SelectContent>` `alignItemWithTrigger` default to `false` (popup measured to content, not trigger), and `<SelectScrollUpButton>` / `<SelectScrollDownButton>` now use `hidden data-[visible]:flex` so chevrons render only when content actually overflows (Base UI emits `data-visible` on the scroll-button when scrolling is possible). | FR-019, US9; Phase 6 CR1 |

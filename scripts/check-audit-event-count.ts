...
const F5_MIGRATION = resolve(
  ROOT,
  'drizzle/migrations/0040_audit_log_f5_extension.sql',
);
...
async function computeCanonicalCount(): Promise<number> {
  const sql = await readFile(F5_MIGRATION, 'utf8');
  // F5 migration uses `ALTER TYPE … ADD VALUE '…'` inside DO blocks.
...
async function main(): Promise<void> {
  ...
  if (canonical === 0) {
    console.error(
      '[check:audit-events] FAIL — could not extract any ADD VALUE statements from migration 0040. ' +
        `File: ${relative(ROOT, F5_MIGRATION)}`,
    );
    process.exit(1);
  }

...
async function main(): Promise<void> {
  ...
  try {
    ...
    const canaries: ReadonlyArray<{ name: string; query: string }> = [
      ...
      {
        name: 'member_change_requests FK-column indexes (mig 0302 — PR-1 review, Mig M-2)',
        query: `SELECT 1 AS hit WHERE (SELECT count(*) FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'member_change_requests' AND indexname IN ('member_change_requests_tenant_decided_by_idx', 'member_change_requests_tenant_submitted_by_contact_idx', 'member_change_requests_tenant_replaced_by_idx')) = 3`,
      },
      {

...
export const REQUIRED_ENUM_VALUES: Readonly<Record<string, readonly string[]>> = {
  ...
  audit_event_type: [
    ...
    // F114 (0301) — `submitChangeRequest` (submitted + the replace path's
    // withdrawn) / `decideChangeRequest` (PR-1), the erasure scrub (T078)
    // and the US5 withdraw + US6 setting use cases (PR-2 / PR-3) INSERT
    // these in the same tx as the state change; a non-persisting ADD VALUE
    // would 500 every submit and decide in prod instead of failing the
    // deploy here.

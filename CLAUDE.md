...
- TypeScript 5.7+ strict (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`); Node 22 LTS + Next.js 16 App Router · React 19 · Drizzle ORM · next-intl · shadcn/ui `Switch`. **Zero new npm dependencies** (Constitution X). (108-contact-recipient-rules — **ALL FOUR PRs MERGED**: PR-A #340 + PR-B #342 (2026-09-05), PR-D #344 (2026-09-06), PR-C #346 `91505b8f2` (2026-09-07). PR-C ships the 1:N contact audience behind `FEATURE_CONTACT_MARKETING_RECIPIENTS`, default OFF — but **nine changes in it are UNFLAGGED and live on merge**, listed with their operator consequences in `specs/108-contact-recipient-rules/quickstart.md` § Rollback matrix row C. The flag flip is T094 and carries four preconditions, one of them a blocker — see below.)
- Neon Postgres `ap-southeast-1` (Drizzle, hand-written SQL). **PR-A shipped migration `0292`** (`audit_event_type += 'auto_email_skipped_no_recipient'`, enum-only, applied to dev) plus `scripts/check-money-email-recipient.ts` wired into pre-push + `quality-gates.yml`. **PR-B shipped migration `0293`** (`contacts_check_member_primary()` SECURITY DEFINER with `row_security = off` + two DEFERRABLE INITIALLY DEFERRED constraint triggers + a deploy-failing pre-check; predicate narrowed by spec AMENDMENT to members with ≥ 1 contact row — see `specs/108-contact-recipient-rules/reviews/pr-b.md`, four review rounds, 61 findings). **PR-D (#344) shipped migrations `0294`** (`contacts` +3 nullable marketing opt-out columns `marketing_opt_out_{at,source,by_user_id}` + correlated CHECK + partial index) **and `0295`** (`audit_event_type += contact_marketing_opted_out / contact_marketing_opted_in`), both applied to dev; permission key `contacts.marketing` (catalogue 42 keys; admin / super_admin / marketing — never manager); `MembersBridgePort.filterMarketingOptedOut` drops opted-out addresses from every segment at dispatch (fail-closed), metric `broadcasts_marketing_opt_out_filter_count`. **PR-C (#346) shipped `0296`** (`contacts_tenant_lower_email_all_idx` — NOT partial, so the carry-forward opt-out lookup can match REMOVED rows) **and `0297`** (`marketing_unsubscribes.contact_id`, written by every unsubscribe from merge with NO flag read — do not drop the column while this code is deployed). The `broadcast_audience_members` working table was NOT needed. Flag `FEATURE_CONTACT_MARKETING_RECIPIENTS` (default OFF) gates the audience LEG, the 50,000 ceiling (which also needs the batching flag) and one line of compose copy — nothing else. Money emails resolve the live primary contact via the widened `RecipientLocalePort`; F5 gets a `BillingRecipientPort`. (108-contact-recipient-rules)
...
Last updated: 2026-09-07 (**108 IS COMPLETE — PR-C MERGED, PR #346 `91505b8f2`.** The 1:N contact audience ships behind `FEATURE_CONTACT_MARKETING_RECIPIENTS` (default OFF), but **nine changes in the PR are UNFLAGGED and live on merge** — full list with operator consequences in `specs/108-contact-recipient-rules/quickstart.md` § Rollback matrix row C. The two most surprising: `marketing_unsubscribes.contact_id` (migration `0297`) is written by **every** unsubscribe with no flag read, so dropping the column while this code is deployed 42703s the GDPR Art. 21 path; and **the sender is no longer excluded from a custom list or the attendee segment** (pre-108 their primary address was filtered out of every segment kind). Migrations `0294`–`0297` applied to prod on the deploy. Rolling any of the nine back is a code revert, not a flag flip.

**T094 flag flip — every PRECONDITION is now cleared (2026-09-08, branch `108-phase9-cutover`); the flip itself has not happened.** T094 completes at first-send observation, not at flag-set, so it stays open. What changed:

- **The push-capacity blocker is CLOSED IN CODE**, and its old description here was wrong twice over. T095 measured the account limit at **10 req/s** (`ratelimit-policy: 10;w=1`, read from the API — the "~2 req/s" that lived in four source comments was wrong), but `addContactsToAudience` is a serial `await` loop, so throughput is `min(limit, 1/RTT)` and the warm round trip is ~0.481 s ⇒ **~2.08 req/s, latency-bound**. That puts the undeliverable band near **830** — *below* the 5,000 ceiling enforced with the flag OFF — so it was never "5,001–10,000 that the flip adds"; the exposure already existed and the fix was owed either way. First fix (T095): `DELIVERABLE_RECIPIENTS_PER_TICK = 500` in `src/modules/broadcasts/domain/audience-ceiling.ts` with `currentAudienceCeiling() = min(configuredAudienceCeiling(), 500)` — a CLAMP, i.e. refuse what one tick cannot push.

  **Phase 9b superseded that clamp.** The maintainer's objection to it was that a growing chamber should not hit an engineering wall needing a code change; headcount should be a billing decision. So `DELIVERABLE_RECIPIENTS_PER_TICK` became the BATCH SIZE: `SPLIT_THRESHOLD_RECIPIENTS` is now *derived* from it (was 10,000), `splitBroadcastIntoBatches` sizes batches at it (was `RESEND_PER_AUDIENCE_CAP` = 10,000), and `currentAudienceCeiling()` clamps only when batching is OFF.

  **⚠️ STATE THE DELTA AGAINST PROD, NOT AGAINST THIS BRANCH.** This paragraph said "the accepted ceiling goes 500 → 5,000" for several hours and that was **wrong** — caught by the whole-branch review, 2026-09-08. The 500 clamp only ever existed *on this branch*; `origin/main` has no `DELIVERABLE_RECIPIENTS_PER_TICK` at all and `currentAudienceCeiling()` there is a plain `audienceCeiling(flags)`. So the "500 → 5,000" was a branch-internal figure measured against an intermediate commit that never merged. Verified by reading `git show origin/main:…`, not inferred. **Against prod, on merge, all unflagged:**
  - **accepted ceiling: UNCHANGED** — 5,000, or 50,000 once `FEATURE_CONTACT_MARKETING_RECIPIENTS` flips (that flip therefore widens 5,000 → 50,000, which is the number the T094 operator needs);
  - **split threshold: 10,000 → 500** — the real change, and what first routes an ordinary broadcast down the batch path;
  - **batch size: 10,000 → 500**, plus the four behavioural changes below.

  Nothing accepted is undeliverable either way — with batching on the delivery simply spans ticks (100 batches × one wave per tick ≈ 2 h for 50,000 at `dispatch_concurrency_cap = 4`). The lesson worth keeping past this branch: when a constant changes meaning MID-branch, every "before → after" written afterwards has two possible baselines, and the useless one is the one that is easiest to reach for.

  Four consequences worth knowing before touching this path: (1) `batch-dispatcher` now dispatches **one wave per invocation** — it used to drain the queue, which was harmless at 10,000-contact batches because a second wave never happened, and at 500-contact batches (~240 s each) starts a wave that dies at `maxDuration` and re-burns the whole batch's contact quota on retry; (2) the two every-5-minutes crons now **partition** `approved` on `estimated_recipient_count`, and because that number is frozen at submit while delivery is bounded by the resolved count, each cron hands off rather than dropping a row — `dispatch-scheduled` writes the resolved count back and defers, `split-large-broadcasts` splits a shrunken audience instead of releasing a row it claimed; (3) a `permanent` gateway failure is **no longer auto-retried** (the classification now survives as a prefix on the existing `failure_reason` column — no migration; unclassified stays retryable); (4) the batch path now **halts a broadcast on audience drift** rather than sending stale index slices, and a new gauge `broadcasts_batch_no_progress_count` distinguishes a halted broadcast from a legitimately slow multi-tick one — `stuck_sending_count` at 24 h cannot, now that spending hours in `sending` is normal. `configuredAudienceCeiling()` stays separately exported and pinned so the H-2 flag-expression guard survives.
- **The Resend account is on the FREE plan** — 1,000 contacts (13 in use) and 3 segments (1 in use, `General`). So ~987 usable contacts is a second, independent bound agreeing with the 830 wall clock, and only **two** broadcasts can be in flight at once. Upgrading to Pro raises the contact cap to 5,000 but changes no latency: ~623/tick survives the upgrade.
- **T093 + the GDPR Art. 14 attestation are CLEARED as VACUOUS**, measured not assumed: prod holds 150 members / 150 primaries / **0 secondary contacts** / 0 `marketing_unsubscribes` / 0 invariant violations (2026-09-08 10:45). With no secondary contacts the 1:N audience is identical to primary-only. **Both sign-offs expire on SweCham's secondary import** — re-run `scripts/inventory-primary-contact-invariant.ts` then.
- Still genuinely open: **FR-043's 400 ms @ 5,000 band has no test at all** and the 3 s @ 20,000 budget is asserted only through `ciScaled(3_000)` = 18 s on CI — both must be measured from `sin1`, never from CI or a workstation. Plus the walk bound and a tenant-level limiter before any ceiling is raised again.
- **Setting the env var IS the flip on this repo**: `vercel.json` has no `ignoreCommand`, so every merge to `main` deploys production. The variable was set at 09:44 and deleted at 10:41 once that was noticed; it is absent now (`src/lib/env.ts:638` defaults it to `false`). Add it only when ready to redeploy immediately.

**Seven review rounds, and the last three each found what the round before had missed by one hop.** Worth keeping: (1) a **100 % coverage pin was right** — the only line it flagged in `resolve-segment-recipients.ts` was a `default: { return _exhaustive }` arm that returns the reason STRING, i.e. truthy, so an unknown orphan reason was audited as `broadcast_member_missing_primary_contact_email`; `rg "return _exhaustive"` still finds ~11 more in `src/`, including `payments/domain/tenant-payment-settings.ts:114`, whose comment claims it prevents the "always consistent" false positive its runtime arm produces. (2) `ContactRepo.findByEmail` compared `contacts.email` with a case-SENSITIVE `eq` while its own use-case docblock promised a case-insensitive match and all three email indexes are on `lower(email)` — a mixed-case row returned zero rows CLEANLY, writing `contact_id = NULL` and sending the invite duplicate-check into a 23505; prod measured 150 contacts / 0 mixed-case, so it was latent, and SweCham's pending secondary-contact import is the vector. (3) **CI caught a regression the author's own consumer-trace had cleared**: narrowing the `hasOptedOutContact` EXISTS to the orphan branch was safe for every READER in `src/` and broke three ASSERTIONS in a live-Neon test whose name is the contract — reverted, not amended. A contract lives in its tests as much as in its readers. Full 44-row ledger: `specs/108-contact-recipient-rules/reviews/pr-c.md`. Full history → docs/changelog.md)

...
- 014-email-broadcast-advance Phase 4 (US2 Image Embedding) **~95% IMPLEMENTED on branch `014-email-broadcast-advance` 2026-05-20** (T078 compose-Tiptap integration deferred to a same-branch follow-up commit before T145 US2 flag flip is operationally meaningful). 11 of 24 Phase-4 tasks shipped across 8 [Spec Kit] commits: Pre-flight P0 audit event 54→55 (`76a78241`); Wave A 7 RED tests (`6eebd226`); Wave B Domain VO (`ab6f1e17`); Wave C 3 use-cases + `ImageStoragePort` (`da34eac4`); Wave D Drizzle repo real impl + Tiptap config + Vercel Blob image storage + deps wiring (`156eef0b`); Wave E API routes + Wave I feature flag (`0a8960ff`); Wave F 3 React components (`9e3ce989`); Wave H i18n EN+TH+SV — also fixes namespace drift from `member.*` to `portal.broadcasts.compose.*` (`9d4ada6d`); Wave G admin settings page + loading skeleton (`ca676680`). Adds: `<img>` source allowlist Domain VO (`asHostname` brand + `validateHostname` + `extractImgSources` pure fns); 3 Application use-cases (`validateImageSourceAllowlist` + `uploadInlineImage` + `manageImageAllowlist`); `ImageStoragePort` Application port; completes Phase-2 skeleton `drizzle-image-allowlist-repo.ts` real impl; `tiptap-image-extension-config.ts` (inline=false, allowBase64=false); `vercel-blob-image-storage.ts` tenant-scoped path `broadcasts/images/{tenantId}/{contentHash}.{ext}`; POST `/api/admin/broadcasts/settings/allowlist`; POST `/api/broadcasts/inline-image-upload` (multipart, Node runtime, maxDuration=60, 5.5 MB content-length cap); admin allowlist editor + member compose inline-image-uploader + ClamAV unreachable banner (3 React components); `/admin/broadcasts/settings` server page + loading skeleton; `isF71aUs2Enabled()` + `f71aUs2DisabledReason()` 3-layer kill-switch (f7Broadcasts AND f71aBroadcastAdvanced AND f71aUs2Images). 1 new audit event type (`broadcast_image_unsafe`); F7 audit-event count 54 → 55. +45 i18n keys per locale (135 total) under `admin.broadcasts.settings.allowlist.*` + `portal.broadcasts.compose.imageUpload.*` + `.clamavBanner.*`. `pnpm check:i18n` GREEN at 3034 keys × EN+TH+SV (up from 2989). `pnpm check:layout` GREEN at 100 page/loading pairs consistent. Test suite state: F7.1a US2 contract+unit = 35/35 GREEN (6+7+5+17); RED tests T065 (cross-tenant probe) + T066 (ClamAV scan) + T068 (E2E) authored env-gated. Broader broadcasts suite 41 files / 573 tests green on the new Wave D adapter wiring. Ships dark behind `FEATURE_F71A_BROADCAST_ADVANCED=false` + `FEATURE_F71A_US2_IMAGES=false` (US1 unaffected). Constitution v1.4.0: I ✅ (RLS+FORCE via migration 0166 + T065 cross-tenant probe authored RED), II ✅ (TDD RED-first verified; 7 RED tests committed before any T069+ impl), III ✅ (Domain pure + Application owns ports; barrel-only cross-module imports), IV n/a (no payment surface), V ✅ (3034 keys × 3 locales), VI ✅ (WCAG 2.1 AA — semantic `<table>` + `<Label htmlFor>` + `<progress aria-label>` + `role="alert"` on inline errors + `aria-live="polite"` on table mutation), VII partial (scan latency SC-005 budget enforced in use-case + audit event payload carries `durationMs`; live ClamAV verification deferred to T139), VIII ✅ (pipeline-order invariant: bytes never reach storage before verdict=clean; fail-closed on scanner error+timeout), IX ✅ (solo-maintainer substitute via `[Spec Kit]` prefixed commits), X ✅ (zero new npm deps — `@tiptap/extension-image` already at `^3.22.5` from Phase-2 prep; reuses F4 `@vercel/blob` client + Phase-2 ClamAV adapter). **Same-branch follow-up to land before T145 US2 flag flip**: T078 compose Tiptap integration (touches `compose-form.tsx` + `tiptap-editor.tsx` + relaxes `sanitize-html.test.ts:142` `<img>`-strip invariant) — separately landable so the diff stays coherent and reviewable. Full provenance: `specs/014-email-broadcast-advance/tasks.md` Phase 4 (T062-T085 lines 208-253) + `docs/superpowers/plans/2026-05-19-f71a-phase4-image-embedding.md`.

...
- Concurrent-initiate guard (R2-E2): partial unique index `payments_processor_payment_intent_id_uniq` + tenant-filtered `SELECT … FOR UPDATE` on `payments(processor_payment_intent_id)` (CR-2 review 2026-04-27 explicit tenantId filter) + Stripe idempotency key `inv-{invoiceId}-attempt-{n}`. NOTE: F4 sequential-numbering uses Postgres advisory locks per `(tenant_id, document_type, fiscal_year)`; F5 does NOT — partial unique index + tenant-filtered FOR UPDATE replaces advisory lock for payment-row contention since each PaymentIntent maps to exactly one row by construction. (009-online-payment)
...
Last updated: 2026-04-27 (009-online-payment Phase 9 — review-ready)

...
- TypeScript 5.7+ strict (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`); Node 22 LTS + Next.js 16 App Router · React 19 · Drizzle ORM · next-intl · shadcn/ui `Switch`. **Zero new npm dependencies** (Constitution X). (108-contact-recipient-rules — PLANNED, no code yet)
- Neon Postgres `ap-southeast-1` (Drizzle, hand-written SQL). Planned DDL 0292–0297: `contacts` +3 nullable marketing opt-out columns + partial index · `marketing_unsubscribes.contact_id` · deferred constraint triggers for the exactly-one-primary invariant (with a data pre-check that fails the deploy) · 3 `audit_event_type` values · optional `broadcast_audience_members` working table. New permission key `contacts.marketing`; temporary flag `FEATURE_CONTACT_MARKETING_RECIPIENTS`. Money emails resolve the live primary contact via the widened `RecipientLocalePort`; F5 gets a `BillingRecipientPort`. (108-contact-recipient-rules)

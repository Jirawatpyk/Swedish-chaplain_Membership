...
- **The push-capacity blocker is CLOSED IN CODE**, and its old description here was wrong twice over. T095 measured the account limit at **10 req/s** (`ratelimit-policy: 10;w=1`, read from the API — the "~2 req/s" that lived in four source comments was wrong), but `addContactsToAudience` is a serial `await` loop, so throughput is `min(limit, 1/RTT)` and the warm round trip is ~0.29 s ⇒ **~3.4 req/s, latency-bound**. That puts the undeliverable band near **830** — *below* the 5,000 ceiling enforced with the flag OFF — so it was never "5,001–10,000 that the flip adds"; the exposure already existed and the fix was owed either way. Fix: `DELIVERABLE_RECIPIENTS_PER_TICK = 800` in `src/modules/broadcasts/domain/audience-ceiling.ts` with `currentAudienceCeiling() = min(configuredAudienceCeiling(), 800)`. **The effective audience ceiling is therefore 5,000 → 800, unflagged and live on merge.** `configuredAudienceCeiling()` stays separately exported and pinned so the H-2 flag-expression guard survives. Side effect: nothing can reach `SPLIT_THRESHOLD_RECIPIENTS`, so the split path is unreachable — no loss, `dispatch-batches` is the same serial push under the same 300 s.
- **The Resend account is on the FREE plan** — 1,000 contacts (13 in use) and 3 segments (1 in use, `General`). So ~987 usable contacts is a second, independent bound agreeing with the 830 wall clock, and only **two** broadcasts can be in flight at once. Upgrading to Pro raises the contact cap to 5,000 but changes no latency: ~830/tick survives the upgrade.

...
Pre-push (**~50 s**) runs ten static gates — `check:layout` + `check:template-seed` + `check:fixme` + `check:dates` + `check:env-example` + `check:env-boot` + the four RBAC/audit-truth gates `check:staff-page-guard` + `check:api-route-guard` + `check:authorization-role-reads` + `check:actor-role-truth` (14 s for the four, measured) — then `tests/unit/architecture/` (24 s — the settlement-tx money guard), plus a **conditional per-module integration gate** when `src/modules/<m>/**` is touched (emergency override: `SKIP_INTEGRATION_PREPUSH=1 git push`; if you use it, run the touched risk surface manually and disclose it in the PR). The 190-file `tests/contract/` run moved to CI; run it locally with `pnpm vitest run tests/contract/` (~4.3 min) before opening a PR. Additional `check:*` gates available (NOT in pre-push): `check:strict-aria`, `check:audit-events`, `check:audit-counts`, `check:bundle-budgets`, `check:multi-tenant`, `check:portal-guard`, `check:plan-divergence`, `check:f71a-schema`, `check:f9-schema`.
...
- TypeScript 5.7+ strict (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`); Node 22 LTS + Next.js 16 App Router · React 19 · Drizzle ORM · next-intl · shadcn/ui `Switch`. **Zero new npm dependencies** (Constitution X). (108-contact-recipient-rules — PR-A IMPLEMENTED on branch `108-contact-recipient-rules`, not yet merged; PR-B/D/C still planned)
- Neon Postgres `ap-southeast-1` (Drizzle, hand-written SQL). **PR-A shipped migration `0292`** (`audit_event_type += 'auto_email_skipped_no_recipient'`, enum-only, applied to dev) plus `scripts/check-money-email-recipient.ts` wired into pre-push + `quality-gates.yml`. Remaining planned DDL 0293–0297: `contacts` +3 nullable marketing opt-out columns + partial index · `marketing_unsubscribes.contact_id` · deferred constraint triggers for the exactly-one-primary invariant (with a data pre-check that fails the deploy) · 3 `audit_event_type` values · optional `broadcast_audience_members` working table. New permission key `contacts.marketing`; temporary flag `FEATURE_CONTACT_MARKETING_RECIPIENTS`. Money emails resolve the live primary contact via the widened `RecipientLocalePort`; F5 gets a `BillingRecipientPort`. (108-contact-recipient-rules)
...
Last updated: 2026-09-05 (108-contact-recipient-rules **PR-A IMPLEMENTED, review-complete, unmerged** on branch `108-contact-recipient-rules`: money emails + the F5 PromptPay billing address now resolve the recipient LIVE from the member's primary contact instead of the frozen `member_identity_snapshot`; migration `0292`, new `check:money-recipient` gate, three review rounds closed — ledger in `specs/108-contact-recipient-rules/reviews/pr-a.md`. PR-B/D/C not started; PR-B's migration `0293` needs a fresh V1 inventory run immediately before merge. See `docs/contacts-primary-secondary-gap-analysis.md` for the gap evidence. Previous: actor-role truth invariant CLOSED with no exceptions — #332–#335 merged, new `check:actor-role-truth` pre-push gate, migration 0290; the two June-dormant PRs #123/#124 landed as #336/#337 incl. migration 0291, so F6 attendance + F7 broadcasts finally bump `last_activity_at`. Zero open PRs, main green. Known doc drift to watch: the "Repository status" paragraph is a dated 2026-07-28 snapshot — only its migration number is kept live. Full history → docs/changelog.md)

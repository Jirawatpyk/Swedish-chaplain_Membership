...
- **The push-capacity blocker is CLOSED IN CODE**, and its old description here was wrong twice over. T095 measured the account limit at **10 req/s** (`ratelimit-policy: 10;w=1`, read from the API — the "~2 req/s" that lived in four source comments was wrong), but `addContactsToAudience` is a serial `await` loop, so throughput is `min(limit, 1/RTT)` and the warm round trip is ~0.481 s ⇒ **~2.08 req/s, latency-bound**. That puts the undeliverable band near **830** — *below* the 5,000 ceiling enforced with the flag OFF — so it was never "5,001–10,000 that the flip adds"; the exposure already existed and the fix was owed either way. First fix (T095): `DELIVERABLE_RECIPIENTS_PER_TICK = 500` in `src/modules/broadcasts/domain/audience-ceiling.ts` with `currentAudienceCeiling() = min(configuredAudienceCeiling(), 500)` — a CLAMP, i.e. refuse what one tick cannot push.
  ...
  **Phase 9b was BUILT AND THEN DELETED on this same branch (2026-09-08/09).** The maintainer's objection to the clamp was that a growing chamber should not hit an engineering wall needing a code change — headcount should be a billing decision — so `DELIVERABLE_RECIPIENTS_PER_TICK` briefly became a BATCH SIZE. Then the Resend **Contacts-Import API was shown to work**, which removes the constraint the whole batch model existed to work around: one multipart upload carries the entire audience in a single size-independent call (~412 ms for one address or fifty thousand), instead of a serial per-contact loop at a measured 2.08 req/s. `ca51f59a1` deleted the batch path outright — 42 files, −13,380 lines: both crons, the manifests repo, the retry/accept-partial routes and their UI.

  **⚠️ STATE THE DELTA AGAINST `origin/main`, NOT AGAINST THIS BRANCH.** This paragraph has now been wrong about this one constant three times, each time because the baseline moved underneath the sentence. `origin/main` has no `DELIVERABLE_RECIPIENTS_PER_TICK` at all and its `currentAudienceCeiling()` is a plain `audienceCeiling(flags)`. **Against prod, on merge, all unflagged:**
  - **accepted ceiling: 5,000 → 500** when `FEATURE_F7_IMPORT_AUDIENCE` is off, which is the default and therefore the state at merge. Deliberate, with the reasoning in `reviews/cutover.md` § 5: audiences of 501–5,000 are accepted today and *already fail silently*, because 300 s of the serial loop drains ~623. The refusal replaces a silent non-delivery with a legible error;
  - **split threshold / batch size: GONE** — there is no batch path to route to;
  - with the import flag ON the accepted ceiling is what the flags configure (5,000, or 50,000 once `FEATURE_CONTACT_MARKETING_RECIPIENTS` flips), because a single upload has no per-tick capacity to clamp against.

  `configuredAudienceCeiling()` stays separately exported and pinned even though nothing outside its own file reads it: its test asserts `configured` and `enforced` as SEPARATE columns, and rows where they differ are what keep an inverted flag expression failing (the H-2 guard).

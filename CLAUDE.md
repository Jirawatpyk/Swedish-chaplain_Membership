...
**The flag flip (T094) is NOT cleared** — four preconditions, one a blocker: the ceiling accepts 5,001–10,000 recipients, a band `split-large-broadcasts` skips (`> 10_000`) and `dispatch-scheduled`'s serial one-contact-per-request push cannot finish inside `maxDuration = 300`; `plan.md:268` states this itself and the US5 AMENDMENT had claimed the opposite in its worked example (corrected). Unreachable while the flag is off. The other three: the GDPR Art. 14 first-contact attestation gate in `docs/compliance/processing-records.md`, FR-043's 400 ms band measured from `sin1` (never from CI, which scales budgets ×6, nor from a workstation), and a walk bound + tenant-level limiter before the ceiling is raised.

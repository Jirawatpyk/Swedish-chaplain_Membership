...
- 012-eventcreate-integration: F6 EventCreate Integration **Phase 10 in progress** on branch `012-eventcreate-integration` (4 waves committed 2026-05-17). 7 user stories (US1 webhook ingest / US2 admin list+detail / US3 wizard / US4 quota / US5 CSV / US6 relink / US7 rotation grace) closed across Phases 3–9. **Phase 10 closure (per user directive "ทำให้จบที่ 10 — ไม่มี Phase 11")** delivers admin PII erasure surface (T110–T112 Wave 1 commits `ab8d49b5`+`3b7dee69`), F8 EventAttendees port adapter for at-risk-scorer (T120–T123 Wave 3 commit `fdb0f885` — silent-failure-critical per analyze finding U-1), observability gap-fill with 2 new metrics + 7 runbooks + 1 hourly recompute cron + cron-jobs.md updates (T124–T135 Wave 4), and 34 audit-event i18n keys × EN+TH+SV bringing total to **2888 keys × 3 locales** plus retrospective Phase 10 sections (T142–T143 + T148 Wave 5 partial). New F6 modules: erase-attendee-pii use-case + `findPriorErasureCompletion` audit port method + `hardDelete` Drizzle impl + ErasePiiDialog + admin route + server page; getEventAttendeesByMember Application wrapper + drizzleEventAttendeesAdapter (structural match to F8 EventAttendeesPort — no F6→F8 import); 7 f6-*.md runbooks (signature-burst, precondition-burst, match-rate-degradation, secret-rotation-procedure, idempotency-sweep, admin-event-detail-not-found, audit-fallback-double-failure). Integration test counts at Phase 10 close: **28 F6 integration GREEN on live Neon Singapore** (Wave 1 pii-erasure 6/6 + Wave 3 f8-port-wiring 7/7 + prior 15). Ships dark behind `FEATURE_F6_EVENTCREATE=false`. **Deferred to follow-up sessions on the SAME branch (NOT Phase 11)**: Wave 2 retention sweeps (T113–T119 pseudonymise + idempotency-ttl + 2 integration tests + Drizzle adapter impls), 4 perf benches (T136–T139), 2 RBAC tests (T140–T141), final E2E/integration/a11y/cross-tenant probe execution (T144–T147), F6.1 backlog 2 items (admin-remap E2E + 5/5 match-type byte-equivalence), T154b fast-check stress profile. Pre-flag-flip operator gates (T150–T154a, 6 items including F8 live-wired verification at flag-flip) consolidated in `specs/012-eventcreate-integration/retrospective.md` § "Pre-flag-flip operator checklist". Constitution v1.4.0: I ✅ (cross-tenant probes GREEN both PII erasure + F8 bridge Review-Gate blockers), II ✅ (TDD RED-first), III ✅ (structural typing on F8 bridge keeps F6 lower-level than F8), IV n/a, V ✅ (2888 keys × 3), VI ✅ (AlertDialog WCAG 2.1 AA pattern), VII ✅ (11 metrics + 6 alerts + 7 runbooks; 4 perf benches deferred to flag-flip operator gate), VIII ✅ (runInTenantWithRollbackOnErr on all admin writers), IX ✅ (solo-maintainer substitute + 4 [Spec Kit] commits in Phase 10), X ✅ (zero new npm deps).
...
Last updated: 2026-05-17 (F6 EventCreate Integration Phase 10 in progress on branch `012-eventcreate-integration`: 4 commit waves shipped — Wave 1 PII erasure (T110-T112) + Wave 3 F8 bridge (T120-T123) + Wave 4 observability gap-fill (T124-T135) + Wave 5 partial (i18n 2888 keys × 3 + retrospective + CLAUDE.md update). 28 F6 integration GREEN on live Neon. Wave 2 retention sweeps + 4 perf benches + 2 RBAC tests + final E2E sweep + F6.1 backlog + T154b stress profile deferred to follow-up sessions on the SAME branch. Ships dark behind FEATURE_F6_EVENTCREATE=false.)

...
- **UI**: **AURA** (`@jirawatpyk/aura-react` + `@jirawatpyk/aura-tokens`, 5.6.0 exact) replacing the legacy kit module by module — spec 122, guide `docs/aura-adoption.md`; the legacy kit in `src/components/ui/` is shadcn/ui on **Base UI** primitives (not Radix) and is deleted at spec 122 US13 · Tailwind CSS v4 · lucide-react · `next-themes` for light/dark · toasts only through `@/lib/toast` (AURA toast, top centre; `sonner` removed)
...
- **122 AURA migration** (`122-aura-design-system-migration`, one PR per phase): `@jirawatpyk/aura-react` + `@jirawatpyk/aura-tokens` 5.6.0 exact (+2 deps; net −4 at exit) · CSS layer order + token bridge + generated brand theme `src/styles/aura-theme.css` · `AuraBridge` provider · `@/lib/toast` facade · UI import ratchet `@typescript-eslint/no-restricted-imports` + `MIGRATED_PATHS` · dual-library window ≤ 10 weeks after US0 merges (Principle VI exception) · AURA handoff items 1–56 all shipped (52–56 in 5.6.0; no local workarounds left).

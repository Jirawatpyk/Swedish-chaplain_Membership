...
**Repository status (as of 2026-04-18)**: **F1 (Auth & RBAC) SHIPPED via PR #1.** **F2 (Membership Plans) REVIEW-READY on `002-membership-plans`.** **F3 (Members & Contacts) REVIEW-READY on `005-members-contacts`** — 155/160 tasks complete; 5 remaining are human-gated items (T155a manual SR pass, T156 maintainer co-sign, T158 staging traces, T151/T152 full CI) + T159/T160 documentation. **Branch `006-layout-container-tier2` REVIEW-READY on PR #9** — ad-hoc UI-infrastructure (not canonical F5 per `docs/phases-plan.md` — F5 = Online Payment/Stripe, not started). Replaces `ContentContainer` with three semantic primitives (`TableContainer` 96rem / `FormContainer` 42rem / `DetailContainer` 72rem) across 19 admin + portal routes; adds `pnpm check:layout` static scope-gate; 100% spec adherence per `specs/006-layout-container-tier2/retrospective.md`. Test suite state: **~1249 unit+contract green** + **317 integration green** (2 pre-existing order-dependent flakies in `email-change-revert-route.test.ts` unrelated to any F-feature) on live Neon Singapore + **723 i18n keys × 3 locales**. Security/UX/a11y checklists 100% complete. Constitution v1.4.0 Principle I tenant-isolation test (Review-Gate blocker) 14/14 green. SC-002 perf (p95=258ms < 500ms @ 5k rows). Source modules: `src/modules/auth/**` (F1) + `src/modules/tenants/**` + `src/modules/plans/**` (F2) + `src/modules/members/**` (F3) + presentation surfaces in `src/app/(staff)/admin/members/**` + `src/app/(member)/portal/**` + `src/components/layout/**` (006).
...
Last updated: 2026-04-18 (006-layout-container-tier2 review-ready)

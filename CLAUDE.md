...
- **The push-capacity blocker is CLOSED IN CODE**, and its old description here was wrong twice over. T095 measured the account limit at **10 req/s** (`ratelimit-policy: 10;w=1`, read from the API — the "~2 req/s" that lived in four source comments was wrong), but `addContactsToAudience` is a serial `await` loop, so throughput is `min(limit, 1/RTT)` and the warm round trip is ~0.481 s ⇒ **~2.08 req/s, latency-bound**. That puts the undeliverable band near **830** — *below* the 5,000 ceiling enforced with the flag OFF — so it was never "5,001–10,000 that the flip adds"; the exposure already existed and the fix was owed either way. First fix (T095): `DELIVERABLE_RECIPIENTS_PER_TICK = 500` in `src/modules/broadcasts/domain/audience-ceiling.ts` with `currentAudienceCeiling() = min(configuredAudienceCeiling(), 500)` — a CLAMP, i.e. refuse what one tick cannot push.
  ...
  **Phase 9b superseded that clamp, and the headline is now the OPPOSITE of what this paragraph said for a day.** The maintainer's objection to the clamp was that a growing chamber should not hit an engineering wall needing a code change; headcount should be a billing decision. So `DELIVERABLE_RECIPIENTS_PER_TICK` became the BATCH SIZE: `SPLIT_THRESHOLD_RECIPIENTS` is now *derived* from it (was 10,000), `splitBroadcastIntoBatches` sizes batches at it (was `RESEND_PER_AUDIENCE_CAP` = 10,000), and `currentAudienceCeiling()` clamps only when batching is OFF. **Prod has batching ON, so on merge, both unflagged: the accepted ceiling goes 500 → 5,000 (50,000 once the 108 flag flips) and the split threshold goes 10,000 → 500.** Lead any PR description with that, not with the Phase-9 docs. Nothing accepted is undeliverable either way — with batching on the delivery simply spans ticks (100 batches × one wave per tick ≈ 2 h for 50,000 at `dispatch_concurrency_cap = 4`).

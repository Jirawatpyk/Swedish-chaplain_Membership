...
- 010-email-broadcast (Phase 9 + Phase 10, 2026-05-02): F7 cross-cutting observability + polish + quality gates COMPLETE. Phase 9 (T172–T185) wired 18 OTel metrics + 5 root spans (`member_submit_broadcast`, `admin_approve_send_now`, `cron_dispatch_scheduled`, `webhook_receive_resend`, `public_unsubscribe`) + 11 alert rules + 5 runbooks; F7 pino redact paths added (RESEND_BROADCASTS_*, UNSUBSCRIBE_TOKEN_SECRET, Svix/Resend signatures, body_html, recipient_emails, unsubscribe_token); F3 archival/erasure cascade implemented end-to-end (`BroadcastsCascadePort` + `f7BroadcastsCascadeAdapter` + `cancelInFlightBroadcastsForMember` use-case + integration test 4/4 ✓); JCC-test fixture activated stub-Resend mode 3/3 ✓; multi-tenant readiness GitHub workflow active (nightly Mon-Fri); synthetic-load + bundle-budget scripts shipped (`pnpm check:bundle-budgets`); DPIA stub + processing-records § F7 documented. Phase 10 (T186–T218 active scope) closed: `pnpm check:i18n --orphans` flag implemented; static-key invariant ESLint warn-rule active (27 advisory warnings on existing legacy code); `pnpm audit --prod` HIGH closed via drizzle-orm 0.36 → 0.45.2 upgrade + next-intl 4.9 → 4.11 patch (2 transitive moderate accepted); 12 stale integration tests fixed for Drizzle 0.45 cause-chain wrapping (centralised `errorChainMessage` + `isUniqueViolation` helpers in `src/lib/db-errors.ts`); 1 stale contract test fixed for cron kill-switch 503→200 R6 contract; retrospective document captures solo-maintainer substitute evidence (`/speckit.review` ≥3 + `/speckit.staff-review` 2 rounds + DPIA + threat-model pre-flight + DB+App+Concurrency+Audit+Observability defence-in-depth). Final test counts: ~3070 unit+contract + ~770 integration green; 1662 i18n keys × 3 locales; typecheck + lint + check:i18n + check:layout green; 0 HIGH/CRITICAL prod vulns. 9 human-gated tasks deferred to ship-day with explicit re-evaluation cadences (T189 chamber liaison · T190–T197 e2e a11y/i18n · T191 manual SR · T198+T199 agent passes · T204–T209 perf 5k-fixture · T213 staging walkthrough · T215 7d prod RUM). Branch `010-email-broadcast` ready for `/speckit.review` final → `/speckit.ship`.
- 010-email-broadcast (Phase 8 US6 SHIPPED via PR #22, 2026-05-02): F7 final feature phase — scheduled cron dispatch + AS2 1h-retry-budget + dispatch-failure transactional email + daily draft-prune cron + `members.preferred_locale` (migrations 0080–0083). Squash-merged as `f0e624f`. 1312 unit+contract + 230 integration GREEN on live Neon. Post-merge prod smoke clean (0 budget_exhausted / failed_to_dispatch / uncaught_error). Phase 9+10 (cross-cutting + retrospective) remain before F7 epic close.
...
Last updated: 2026-05-02 (010-email-broadcast Phase 8 US6 SHIPPED via PR #22, squash-merged as `f0e624f`; F7 final feature phase live in prod with cron-driven scheduled send + AS2 dispatch-failure notifications + draft-prune; only Phase 9+10 remain before F7 epic close)

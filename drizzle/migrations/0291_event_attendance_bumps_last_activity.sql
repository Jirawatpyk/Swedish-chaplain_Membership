-- 0291 — F6 → F3/F8: deterministic event attendance bumps members.last_activity_at.
--
...
--
-- MEASURED, not assumed (2026-08-17, read-only prod query before landing this):
-- of the F6 audit rows in production — `attendee_non_member` (78),
-- `event_created` (4), `event_partner_benefit_toggled` (4),
-- `event_cultural_event_toggled` (3), `event_archived` (1) — ZERO carry a
-- `member_id` key of either casing, so the 0009 trigger has never fired for an
-- event. Scope note for whoever reads this next: the SAME query showed F7
-- broadcast rows (`broadcast_submitted`, `broadcast_sent`,
-- `broadcast_send_timeout_completed`) carry `memberId` in CAMEL case, which
-- `payload ? 'member_id'` also does not match — so F7 silently misses the 0009
-- trigger too. That is a separate gap (PR #124's territory), NOT fixed here.
--
...
--
-- DROP IF EXISTS first (repo convention, cf. 0055): `CREATE TRIGGER` has no
-- OR REPLACE, so without this the migration is not replayable — and it is not
-- hypothetical. This trigger was created on the SHARED dev Neon branch when
-- the original PR (#123, June) was authored there; the migration was never
-- recorded as applied, so every later `pnpm db:migrate` on dev died here on
-- "trigger already exists". Prod, a fresh CI Neon branch, and a replay after
-- a partial failure all take the same path now.
DROP TRIGGER IF EXISTS "event_registration_bump_member_last_activity" ON "event_registrations";--> statement-breakpoint

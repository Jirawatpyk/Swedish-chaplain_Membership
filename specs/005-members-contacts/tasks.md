...
- [X] T049 [US1/US3.b.1] Implement `src/modules/members/infrastructure/adapters/resend-email-port.ts` — outbox-backed dispatch for invitation. **Closed 2026-04-17 (full migration)**: 4-layer change shipped end-to-end. (1) Adapter `enqueue` + `enqueueInTx` (`resend-email-port.ts` 96 lines, writes `notifications_outbox` migration 0011). (2) Dispatcher template wiring (`outbox-dispatch/route.ts` `buildPayload` case `member_invitation` → `buildInvitationEmail({toEmail,token,role,locale})` with `isRole()` validation). (3) **F1 `create-user.ts` migrated from synchronous `email.send` to outbox `enqueueInvitation`** — `CreateUserDeps.email + buildInvitationEmail` removed, replaced by `EnqueueInvitationFn` port; concrete impl in `auth-deps.ts` inserts `notifications_outbox` row directly (`tenant_id=null` for cross-tenant F1 flow; outbox has no RLS per migration 0011 header). (4) Test updates: `tests/integration/auth/account-lifecycle.test.ts` swapped `stubEmailSender` → `stubEnqueueInvitation` (matching `EnqueueInvitationFn` shape). `tests/contract/invite.test.ts` (mocks `createUser` whole-fn — unaffected) + `tests/contract/members/invite-portal.test.ts` (wraps via `CreateUserPort` narrowed surface — unaffected) verified green. `pnpm typecheck` ✅ + `pnpm lint` ✅ + integration suite 304/308 (4 pre-existing FK failures in `clear-test-data.test.ts` per CLAUDE.md, unchanged from pre-migration baseline). **User-facing latency change**: invitation email arrives within ≤60s of admin clicking "Invite" instead of immediately — durability win (Resend outage no longer rolls back user creation) at the cost of dispatch lag.
...
- [X] T088 [US3/US3.b.1] Implement adapter extensions to `resend-email-port.ts` adding `email_verification` + `email_change_revert` notification types with 5-minute activation delay for verification. **Closed 2026-04-17**: both notification types land in the shared `notification_type` enum + `NotificationType` union; templates at `src/modules/members/infrastructure/email/email-verification-email.ts` + `email-change-revert-email.ts` (plain-HTML builder, EN/TH/SV, follows F1 reset-password-email pattern). **5-minute activation delay shipped** in `change-contact-email.ts:165–183` (sets `activatedAt = now + VERIFICATION_ACTIVATION_DELAY_MS` on token + outbox `contextData.activatedAt`); `verify-contact-email.ts` enforces `if (now < token.activatedAt) → 'not_yet_active'`. Test coverage: integration `tests/integration/members/verify-contact-email.test.ts:190` ("rejects not_yet_active when clock is before activatedAt") + contract `tests/contract/members/email-verification-route.test.ts:238` (5-min ahead activatedAt → not_yet_active branch). Constants in `src/modules/members/application/crypto-helpers.ts:8`.
- [X] T089 [US3/US3.b.1] Extend outbox dispatcher (F1) retry budget config for F3 notification types per spec § Security 4.2. **Correction 2026-04-15**: F1 did NOT have an outbox dispatcher — US3.b.1 creates it from scratch at `src/app/api/cron/outbox-dispatch/route.ts`. **Closed 2026-04-17** with: 5-attempt exponential backoff (60s / 5m / 30m / 3h / 12h per spec FR-012c — `RETRY_BACKOFF_SECONDS` constant `route.ts:75`), permanent-failure flip, CRON_SECRET auth, 50-row batch, `email_verification` + `email_change_revert` template wiring, **`FOR UPDATE SKIP LOCKED` concurrency guard** (`route.ts:170` — `.for('update', { skipLocked: true })`), and **`email_dispatch_failed` high-severity audit emission** with `system:cron` actor + tenant-scoped row (`route.ts:281–293`, FR-012c). **Deferred to US3.b.2** (non-blocking improvements): per-notification-type retry budgets, `member_invitation` template wiring once F1 migrates to the outbox.
...
- [X] T098 [US3/US3.b.3] Fill i18n `admin.members.edit.*`, `admin.members.bundleChangeWarning.*`, `auth.emailChangeRevert.*`, `auth.emailVerification.*`, `admin.members.emailChange.*` across EN/TH/SV. **Closed 2026-04-17**: all key groups present in `src/i18n/messages/{en,th,sv}.json` — `bundleChangeWarning.*` (8 keys: title, description, oldBundle, newBundle, affectedCount, confirm, cancel, loading), `admin.members.edit.*`, `auth.emailChangeRevert.*` (11 keys), `auth.emailVerification.*` (9 keys). Total 721 keys × 3 locales parity green via `pnpm check:i18n` (verified at T143 close-out).
...
- [X] T134 [P] [US7] Contract test `tests/contract/members/archive-undelete.test.ts` — **13/13 green** (archive: 200 happy, 200 no-body, 400 missing key, 400 invalid_body, 404 not_found, 409 state_error, 500 server_error; undelete: 200 happy, 400 missing key, 403 archive_window_expired, 404 not_found, 409 state_error, 500 server_error). 2026-04-17
- [X] T135 [P] [US7] Integration test `tests/integration/members/archive-cascade.test.ts` — **5/5 green on live Neon**: archive flips status + sets archived_at + member_archived audit; cascade revokes linked F1 user sessions + user_sessions_revoked audit; re-archive returns state_error; cascade soft-consumes pending unredeemed invitations + invitations_revoked_count on audit payload (verify-C1 fix); cross-tenant returns not_found (RLS). Requires migration 0016 (chamber_app SELECT + UPDATE(consumed_at) on invitations). 2026-04-17
- [X] T136 [P] [US7] Integration test `tests/integration/members/undelete-window.test.ts` — **4/4 green on live Neon**: within-window restore succeeds + audit; 91-day-old archive returns `state.undelete_window_expired`; undelete on non-archived returns `state.undelete_only_from_archived`; cross-tenant returns not_found. 2026-04-17
- [X] T137 [P] [US7] E2E `tests/e2e/members-archive-undelete.spec.ts @f3 @a11y @i18n` — authored: Archive CTA render, Archive dialog open/close + reason textarea, axe-core WCAG 2.1 AA scan (detail + open dialog), TH+SV i18n leak check. 2026-04-17
- [X] T138 [P] [US7] `application/use-cases/archive-member.ts` — zod input (optional `reason` ≤ 500), `runInTenant` atomic: `findByIdInTx` (SELECT ... FOR UPDATE) → domain `archive()` transition → `updateStatusInTx` → per-linked-user session revocation + `user_sessions_revoked` audit → soft-consume pending F1 invitations (`UPDATE invitations SET consumed_at = NOW() WHERE user_id IN (…) AND consumed_at IS NULL AND expires_at > NOW()`) → single `member_archived` audit with `cascaded_user_ids` + `sessions_revoked_total` + `invitations_revoked_count`. Typed errors: `not_found`, `state_error`, `server_error`. 2026-04-17
- [X] T139 [P] [US7] `application/use-cases/undelete-member.ts` — `runInTenant` atomic: `findByIdInTx` → domain `undelete()` (90-day window policy) → `updateStatusInTx` → `member_undeleted` audit. Maps `state.undelete_window_expired` with `daysSinceArchive` for the 403 response. 2026-04-17
- [X] T140 [US7] API routes `src/app/api/members/[memberId]/archive/route.ts` (POST) + `undelete/route.ts` (POST) — admin RBAC via requireAdminContext, Idempotency-Key parsing, classifyIdempotencyRequest + reserveIdempotencyRecord + rememberIdempotentResponse, 7-branch error mapping per route. 2026-04-17
- [X] T141 [US7] `src/components/members/archived-banner.tsx` — rendered on detail page when status='archived'; shows archivedAt + days remaining (within_window) OR daysSinceArchive (window_expired); Undelete CTA calls `POST /api/members/:id/undelete` with fresh UUID Idempotency-Key + sonner toast + router.refresh on success; disabled Undelete with tooltip when window expired. 2026-04-17
- [X] T142 [US7] Show-archived covered by existing `?status=archived` filter in `directory-filters.tsx` (uses status Select dropdown). Archive CTA lives on detail page via new `src/components/members/archive-member-button.tsx` (AlertDialog + optional reason textarea + destructive confirm). Disabled Undelete for > 90 days with tooltip wired via `ArchivedBanner`. 2026-04-17
- [X] T143 [US7] i18n `admin.members.archive.*` (19 keys — archiveCta, confirmTitle/Description, reasonLabel/Placeholder/Helper, cancel, confirmCta, archivingInProgress, archiveSuccess/Error, bannerTitle, withinWindow (plural), windowExpired, windowExpiredTooltip, undeleteCta, undeleteSuccess/Error, windowExpiredToast) + `admin.members.undelete.*` (5 keys — cta, confirmTitle/Description, success, error) across EN/TH/SV = 24 keys × 3 locales. `pnpm check:i18n` 721 keys parity green. 2026-04-17
...
- [X] T151 Run local full CI pipeline: `pnpm lint && pnpm typecheck && pnpm test:coverage && pnpm check:i18n && pnpm test:integration && pnpm test:e2e`
- [X] T152 Confirm F1 test suite (480 tests) + F2 test suite (495 + 163 integration) remain green — SC-010

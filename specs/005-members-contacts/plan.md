...
| `runInTenant` + `auditLog` direct schema import in Application-layer use cases (`change-contact-email.ts`, `verify-contact-email.ts`, `revert-contact-email.ts`) | These 3 use cases write audit rows inside the caller's tenant-scoped DB transaction for atomicity. Routing through `AuditPort` would open a separate connection and break the 6-step transactional guarantee (FR-012a/b). | Extract audit writes to a separate `AuditPort` — rejected because a non-transactional audit insert that fails after a successful email change leaves no audit trail, while a successful audit with a rolled-back email change creates a false record. Both violate Constitution Principle I worse than the import-path deviation. |
| `searchDirectory` + `DirectoryRow` export from Infrastructure used by `directory-search.ts` (Application layer) | The directory search query is a complex multi-table join with correlated subqueries, cursor pagination, and pg_trgm indexes. Abstracting through a port would require leaking SQL-level concepts (cursor encoding, index hints) into the port interface. | Define a `DirectorySearchPort` interface — rejected because every method on the port would mirror the Drizzle query builder's API surface, adding indirection without encapsulation. The query is read-only and tenant-scoped by RLS. |
| `auditLog` deep import from `@/modules/auth/infrastructure/db/schema` in `drizzle-member-repo.ts` + `drizzle-contact-repo.ts` | Repos insert audit rows inside the same DB transaction as the data mutation for atomicity. The auth module's public barrel intentionally excludes Drizzle schema types to prevent ORM leaks. | Export `auditLog` from the auth barrel — rejected because it would set a precedent for leaking Drizzle table objects through public module interfaces, undermining the barrel's purpose. The infra→infra cross-module import is less harmful than barrel→ORM leakage. |
| `auditLog` + `users` direct schema import from `@/modules/auth/infrastructure/db/schema` in `drizzle-timeline-repo.ts` (US6) | Timeline reads `audit_log` filtered by `payload->>'member_id'` + resolves `users.display_name` / `users.email` for actor display in the same query batch. Both tables live in the auth module; going through an application port would require a second round-trip per page (timeline + actor resolution) and lose the single-transaction consistency. | Route through an `AuthDirectoryPort` that exposes `resolveUserDisplayNames(userIds)` — rejected because it duplicates the cross-infra pattern already accepted for member/contact repos (above row) without added value. Export `users` from auth barrel — rejected for the same reason as `auditLog` (Drizzle leak). |
| `membershipPlans` import via `@/modules/plans` barrel in `drizzle-timeline-repo.ts` (US6 E2 enrichment) | The timeline enriches `member_plan_changed` / `plan_bundle_changed` audit payloads with resolved plan display names (spec AS1 requires "Plan changed from Regular Corporate → Premium Corporate"). Read-only JOIN inside the same runInTenant tx — tenant-scoped by RLS. | Emit plan_name in the audit payload at write time (change-plan.ts) — rejected because (a) it doesn't help existing historical rows and (b) denormalising the display string into the audit log couples audit shape to UI labels, making future plan renames confusing. The read-time lookup keeps audit canonical and UI i18n-friendly. Same barrel usage pattern as `drizzle-member-repo.ts`. |

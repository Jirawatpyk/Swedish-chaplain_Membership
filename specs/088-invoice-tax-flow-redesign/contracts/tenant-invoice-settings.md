...
1. **Withholding-tax / footer note** — `wht_note_th` + `wht_note_en` (both `text NULL`). Replaces
   ...
   tenant flag. TSCC seed text (editable, basis ม.65 ทวิ (13) + ท.ป.4/2528; see accountant doc
   แบบ B) is applied via this PATCH, not baked into code.
...
`receipt_number_prefix`, `receipt_numbering_mode` `'combined'|'separate'`,
...
- `tenant_receipt_prefix_changed` (10y) — when the `receipt` §87 prefix flips on an active
  tenant (the `combined→separate` + `RC` cutover triggers this — forensic §87 continuity trail).

...
**Next free Drizzle migration index: `0236`** (updated 2026-07-01 by US1 review-hardening). `0230`+`0231` are APPLIED (bill enum + bill-number/CHECKs/immutability). `0232`/`0233`/`0234` stay **RESERVED** for US3/US5/US8 (§ B.6, unbuilt). `0235_invoices_freeze_receipt_number.sql` was consumed OUT OF BAND by the US1 security-INFO fix (freeze `receipt_document_number_raw` in `invoices_enforce_immutability`) — it sits ABOVE the reserved block, so the reserved 0232–0234 remain a gap and, when US3/US5/US8 are built, they renumber to **0236/0237/0238** (next free = `0236`). The Drizzle migrator orders by the journal `when` timestamp, so the idx gap 0232–0234 is inert (they are never added to the journal).

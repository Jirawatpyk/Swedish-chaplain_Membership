...
> **⚠️ M1 §105/RC ordering — SHIP-GATE:** do **not** set `receipt_number_prefix='RC'` while **any** event-**without-TIN** §105 sale can occur unless **US7/T050 (the separate `receipt_105`/`RE` register) has shipped**. Until then the §105 event-no-TIN arm still allocates from the shared `receipt` stream, so a §105 receipt would be minted as `RC-…` onto the §86/4 RC register — **un-renumberable §87 pollution**. `scripts/verify-088-cutover.ts` enforces this (check 4b: it HARD-FAILS the RC prefix when US7 is unshipped and event-no-TIN §105 sales exist, and warns when zero exist). Order of operations: ship US7/T050 **before** any event-no-TIN sale, OR keep membership+event-with-TIN on RC while no event-no-TIN sales happen in the interim.

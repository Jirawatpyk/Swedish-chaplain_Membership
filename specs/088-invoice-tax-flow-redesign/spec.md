...
- **FR-016** (two-document disambiguation): once paid, the admin + member portal MUST clearly distinguish the two documents — the ใบกำกับภาษี/ใบเสร็จรับเงิน (RC) labelled as the **tax receipt** (the accounting document, visually primary) and the ใบแจ้งหนี้ (SC) marked "for reference / superseded once paid" — so a member knows which document is their tax receipt.
...
- **SC-005**: No user-facing surface (PDF, admin, portal, i18n) labels the pre-payment bill as ใบกำกับภาษี / Tax Invoice.
...
- ⚠️ **BUILD GATE (P1):** this build is gated on the 3 accountant fact-confirms (TSCC is VAT-registered; no fee tier is volume-based; the WHT note is membership-only). Front-load them into a pre-`/speckit.tasks` checkpoint. **If VAT-registration is unconfirmed by the end of `/speckit.tasks`, only the non-tax relabel + presentation polish (US4) proceed; the §86/4 tax-receipt surfaces (US1 / US2 / US6) pause until confirmed** (a chamber that is not a VAT registrant cannot issue §86/4 at all).

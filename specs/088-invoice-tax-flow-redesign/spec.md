...
- **FR-035** (palette + per-row): the command palette MUST gain admin actions **"Record payment for …"** (deep-link `?pay=1`) + **"Re-render tax receipt"**; issued bills MUST gain a **per-row "Record payment"** quick action (defaults today / bank-transfer). Undo / optimistic MUST **NEVER** apply on record-payment (§87-minting). An optional bulk record-payment MUST mint **one RC per invoice sequentially in-tx, gap-free, and is NOT undoable**. Palette keys live under `palette.actions.*` (the live command-palette action registry) + per-row keys under `admin.invoices.list.actions.*`, EN/TH/SV. **DESCOPED 2026-07-01 — the original "toast-with-undo (10s) after ISSUE → revert-to-draft" clause is REMOVED: the issue flow already carries a strong guard (FR-027's typed-phrase confirmation gate + explicit immutable-snapshot acknowledgement in the issue dialog), and an undo → revert-to-draft would contradict the "immutable, editable only by void" invariant that same dialog asserts. The correction path for a mis-issued bill is VOID, not undo.**

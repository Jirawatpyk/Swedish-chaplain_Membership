...
- **FR-024** (MFA-certificate capture, fail-closed): a `zero_rated_80_1_5` invoice MUST capture the **Ministry of Foreign Affairs (Protocol Dept) certificate** the embassy supplies — `zero_rate_cert_no` (the MFA note number, e.g. กต 0404/…), `zero_rate_cert_date`, and an optional attached scan `zero_rate_cert_blob_key` (Vercel Blob, reusing the F4 invoice-PDF blob adapter). A `zero_rated_80_1_5` invoice with a NULL `zero_rate_cert_no` MUST be **blocked** (fail-closed). Each purchase SHOULD be **≥ 5,000 baht** (**warn**, not hard-block).

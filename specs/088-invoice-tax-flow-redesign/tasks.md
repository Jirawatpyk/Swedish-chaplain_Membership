...
- [ ] T008 [P] Retire combined-numbering (data-model § F.5): tighten `receipt_numbering_mode` CHECK to `'separate'`-only, drop `'combined'` from the settings zod enum in `src/modules/invoicing/infrastructure/db/schema-tenant-invoice-settings.ts`, and delete the `combinedMode` number-reuse branch in `src/modules/invoicing/application/use-cases/record-payment.ts`. **[DEFERRED to US1 — BLOCKER: in this foundational slice `issue-invoice.ts` still allocates a §87 number at issue (that is T017/US1) and the pay path is NOT flag-gated yet (T022/US1). Deleting the `combinedMode` branch NOW makes every membership payment mint a SECOND §87 number on the unflagged default path — the double-§86/4 this feature removes. plan.md § Rollout line 114: "optionally tighten the CHECK … later" + "issue-side and pay-side numbering edits MUST ship together". Blast radius: record-payment.test.ts, record-payment-async-pdf.test.ts, render-receipt-pdf.ts. Land it in US1 with T017/T018/T022.]**

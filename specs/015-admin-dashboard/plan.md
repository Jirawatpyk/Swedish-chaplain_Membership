...
| **New dependency `fflate@0.8.3`** (US6 GDPR archive — deviates from the F6+ "zero new deps" goal, Principle X) | The GDPR archive bundles many files (README + 6 JSON categories + invoice PDFs + manifest) into one download — a ZIP container is unavoidable, and the repo had no zip writer. `fflate` is tiny (~8 KB), zero-transitive-dep, and deterministic (mtime-pinned) so the SC-008 per-file SHA-256 manifest stays reproducible. | A hand-rolled STORE-method zip writer was considered (no dep) but rejected: ~120 lines of bit-level ZIP-format code (local-header/central-directory offsets, CRC-32) to hand-maintain + test, versus an 8 KB audited library. The manifest checksums the *uncompressed* file contents, so archive integrity does not depend on the zip encoder. |

...
- [ ] T101a **Private Blob store provisioning (ship-day operator gate)** — the F9 export proxy stores E-Book/GDPR artefacts via `put({ access: 'private' })`. A Vercel Blob store is **public XOR private** (confirmed: dashboard offers one access mode at creation). The existing `BLOB_READ_WRITE_TOKEN` store is **public** (backs F4 invoice PDFs + F9 logos, all `access:'public'`), so private export puts on it are rejected. **CODE WIRING DONE (2026-05-29)**: `private-blob-adapter.ts` now reads `env.blob.privateReadWriteToken` = `BLOB_PRIVATE_READ_WRITE_TOKEN` ?? `BLOB_READ_WRITE_TOKEN` (fallback keeps dev/test/dark-launch booting; exports use an in-memory stub in tests + are flag-gated in prod). F4 PDFs + logos untouched (still on the public store). New `env-blob-private-token.test.ts` 3/3 GREEN. **OPERATOR step remaining**: Vercel dashboard → Storage → create a **private** Blob store → set `BLOB_PRIVATE_READ_WRITE_TOKEN` (Prod+Preview) + `EXPORT_DOWNLOAD_TOKEN_SECRET` (≥32 bytes) → smoke-test a real `processExportJob` → authenticated `/api/internal/exports/[jobId]/download` round-trip. (Discovered 2026-05-28 during US5; runbook §F9 T101a has the full procedure.)

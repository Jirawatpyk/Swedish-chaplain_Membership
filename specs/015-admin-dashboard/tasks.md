...
- [X] T038c [US1] Extend `DashboardSnapshot` VO (`revenueTrend[12]` + `memberGrowth[]`) + `computeDashboardSnapshot` (12 tenant-tz month keys → align series; cumulative = baseline + Σ byMonth) + `emptySnapshot` + drizzle-snapshot zod + `ProjectedDashboard` revenue-trend redacted for manager (FR-007) — done 2026-05-26. T022 integration asserts both arrays (12 buckets; revenue sum 150_000 in I-5, growth cumulative 8/1) GREEN on live Neon.

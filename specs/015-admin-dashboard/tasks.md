...
- [ ] T101a **Private Blob store provisioning (ship-day gate)** — the F9 export proxy stores E-Book/GDPR artefacts via `put({ access: 'private' })`; the linked dev Blob store is **public**, so private puts are rejected (`"Cannot use private access on a public store"`) until the Vercel Blob store is provisioned with **private access** (or a dedicated private store is added + `BLOB_READ_WRITE_TOKEN` repointed). Worker/proxy/adapter code is correct + tested with an in-memory blob stub (US5 export-worker integration) — **no code change**. Post-provision, verify with a real `processExportJob` → authenticated `/api/internal/exports/[jobId]/download` round-trip. (Discovered 2026-05-28 during US5 backend; R6 mandates private for both E-Book + GDPR, and US6 needs it regardless.)

...
- [ ] T038b [US1] Extend `InvoiceSource` (12-month monthly paid revenue, tenant-tz calendar months) + `MemberSource` (members by join month + current active/at-risk/overdue breakdown) aggregation in `src/modules/insights/infrastructure/sources/*`
- [ ] T038c [US1] Extend `DashboardSnapshot` VO + `computeDashboardSnapshot` to compute & store `revenueTrend[12]` + `memberGrowth[]` in the metrics JSONB (revenue trend redacted for manager per FR-007); extend T022 integration to assert the arrays
- [ ] T038d [P] [US1] Build `<RevenueTrendChart>` + `<MemberGrowthChart>` in `src/components/dashboard/` — **self-built SVG**, **stable display-ready prop contract** (no internal data fetch; SVG renderer is a swappable internal per research R8), visually-hidden `<table>` equivalent **inside** each component, non-colour encoding (WCAG 1.4.1), i18n axis/label keys EN/TH/SV
- [ ] T038e [US1] Wire both charts into `src/app/(staff)/admin/page.tsx` (data from snapshot, manager-redacted revenue); extend T023 e2e `@f9` + `@a11y` axe to assert charts render + table equivalents present + no console errors

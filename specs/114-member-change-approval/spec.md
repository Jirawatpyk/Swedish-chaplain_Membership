...
**Created**: 2026-09-11
**Status**: Draft — clarified 2026-09-11 · `spec-review-panel` 2026-09-11: GO WITH AMENDMENTS, amendments applied (`reviews/spec-review-panel-20260911.md`) · ready for `/speckit-plan`
...
3. **Given** a request that was rejected in full or in part, **When** the member follows the "edit and resubmit" link, **Then** the reason is shown and the form is prefilled with exactly the rejected values (approved fields show their new, live values), so they can correct and resubmit as a new request.
...
- **FR-009**: A member MUST be able to withdraw their pending request; a withdrawn request MUST remain in history and MUST NOT be decidable.
- **FR-010**: The member's portal MUST show the pending state with the proposed values next to the current values, and after a decision MUST show each field's outcome (applied value and time, or the reason and an "edit and resubmit" path prefilled with exactly the rejected values).
...
- **FR-015**: Confirming a decision MUST apply all approved fields to the member record in a single all-or-nothing step and MUST record the decision (reviewer, time, per-field outcome, reason, note) in that same step; if applying fails, nothing is applied, no decision is recorded, and the request stays pending.
...
- **FR-017**: A decision MUST be final: a decided or withdrawn request MUST refuse any further decision, and a repeated identical decision (retry, double-click) MUST be a harmless no-op that produces no second application, no second decision and no second queued notification.
...
- **FR-019**: The review page MUST show current and proposed values side by side per field, MUST flag any field whose current value differs from the value the member saw at submission (showing all three values), MUST present each address group as one row, and MUST flag as affecting tax documents the company name, the billing address, and — for a member with no billing address on record — the registered address (which is then the buyer address printed on tax documents).
- **FR-020**: Approval MUST be refused for an archived member and for a member being erased; the reviewer MUST be told why and what they can do instead.
...
- **FR-037**: The system MUST expose an operational count of pending requests and their oldest age so an alert can fire when requests go unreviewed; the threshold (set at the plan gate) MUST fire early enough that a request is decided well within the one-month clock that applies to data-subject requests (GDPR Art. 12(3) / PDPA § 30), so the operational alarm doubles as the statutory backstop.
...
- **Rollout**: the feature ships dark behind the platform's usual feature flag and is enabled per tenant by the setting in FR-031; SweCham's setting is turned on at launch. Turning the setting off is the rollback path — no code revert needed.

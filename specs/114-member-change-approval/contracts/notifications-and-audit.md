...
| `member_change_request_withdrawn` | member user (`member`, `replaced`), or the erasure's actor on `erasure` | `{ member_id \| related_member_id, request_id, contact_id, scope, withdrawn_reason: member\|replaced\|erasure, replaced_by_request_id?, actor_role }` — `withdrawn_reason`, never `reason` (the bare key is on the F9 redaction deny-list); the member key is a two-variant union in `ChangeRequestAuditPayload` (exactly one of `member_id` / `related_member_id`, compiler-checked). On `erasure` (PR-2 T078) `eraseMember` writes one row per CLOSED (formerly pending) request INSIDE the atomic scrub tx with `related_member_id` + `actor_role: 'system'` (the closure is the erasure's consequence, not a human decision — the #333 precedent; `'system'` is not a staff-role literal, so `check:actor-role-truth` admits it) and `actor_user_id` = the erasure's actor (the admin who clicked erase, or `system:cron` on a reconciler re-drive) | yes for `member` (member activity); `replaced` and `erasure` use `related_member_id` | member: yes; others: no |
| `member_change_request_rate_limited` | member user | `{ related_member_id, contact_id, window_count, retry_after_seconds, actor_role }` — `related_member_id`, never `member_id`: a REFUSED attempt is not member activity, so the 0009 `last_activity_at` trigger must not fire (review round 1, REL-3 / P-7; typed in `ChangeRequestAuditPayload`). `contact_id` is the SUBMITTER's (post-ship review #8 / T127): the row carries no `scope` (nothing was proposed), so it is the only key the FR-029 projection can admit it on | the submitter's OWN row only (`contact_id === mine`); a colleague never sees it | no |

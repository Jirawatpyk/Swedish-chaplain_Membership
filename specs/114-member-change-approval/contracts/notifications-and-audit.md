...
| `member_change_request_rate_limited` | member user | `{ related_member_id, window_count, retry_after_seconds, actor_role }` — `related_member_id`, never `member_id`: a REFUSED attempt is not member activity, so the 0009 `last_activity_at` trigger must not fire (review round 1, REL-3 / P-7; typed in `ChangeRequestAuditPayload`) | no (filtered like other refusals) | no |

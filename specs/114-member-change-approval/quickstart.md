...
### US4 — history
1. `/admin/members/[id]` → the "Change requests" section lists both requests, newest first, with per-field outcomes, the reviewer (a disabled staff account carries "(deactivated)") and the reason; "Open in queue" is `/admin/change-requests?memberId=…`. The queue's filters are a client bar on the shadcn `Select` (the URL is the source of truth — Apply patches the query, so every view is still a link): `/admin/change-requests?state=decided&outcome=partially_approved` finds the first; a pending row older than 3 days carries the OVERDUE badge; "Next page" carries the opaque keyset cursor. The queue is also in the ⌘K palette ("Change requests", flag-gated).

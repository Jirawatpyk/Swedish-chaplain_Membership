...
**Primary Dependencies**: next-intl · next-themes · Tailwind v4 · **+ `@jirawatpyk/aura-react@5.6.0`, `@jirawatpyk/aura-tokens@5.6.0`** (Constitution X justification in Complexity Tracking). US0 removes: `sonner`, `react-day-picker`. Later phases remove: `cmdk` (US1); `@base-ui/react`, `tw-animate-css`, `shadcn` (dev), and the `@tanstack/react-table` UI use (US13).  

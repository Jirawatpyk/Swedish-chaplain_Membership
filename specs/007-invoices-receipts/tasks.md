...
- [X] CP-8.4 First-time empty-state bootstrap flow E2E — un-fixme'd via T115t throwaway-tenant fixture (`invoice-settings.spec.ts` AS5). Gate: `E2E_X_TENANT_HEADER_ENABLED=1` in .env.local + re-run `pnpm test:e2e --workers=1 --grep "@us4"`.
...
- [X] T106 Dispatcher landed at `src/app/api/cron/outbox-dispatch/route.ts` (consolidated with F1+F3 path; canonical location — the `/api/cron/auto-email-dispatch` name was a spec stub). Drains up to BATCH_SIZE=50 `pending` rows per tick under FOR UPDATE SKIP LOCKED, Resend send with optional PDF attachment (FR-036 VOID-stamped), exponential backoff per FR-012c, marks sent/retried/permanently_failed. Dual-emits `auto_email_delivery_failed` (F4) alongside `email_dispatch_failed` (F1) on perm-fail for `invoice_auto_email` rows — both `no_template_handler` and Resend-send-failure branches. `vercel.json` shipped with 3 cron entries (`outbox-dispatch */1m`, `lockout-cleanup */15m`, `outbox-purge` daily 20:15 UTC). Integration test `auto-email-outbox.test.ts > (T106 dual-emit)` verifies the F4 audit row lands with correct `outbox_row_id` + `reason` payload (4/4 green on live Neon).
...
- [X] T108 React Email templates shipped. `src/modules/invoicing/infrastructure/email/templates/`: `copy.ts` (7 event × 3 locale matrix + ATTACHMENT_CLAUSE + REASON_PREFIX + `resolveCopy()`), `base-layout.tsx` (@react-email/components `<Html>`/`<Head>`/`<Body>`/`<Container>`/`<Button>` chrome, 560px, system fonts), and 4 template components (`invoice-issued`, `invoice-paid`, `invoice-voided`, `credit-note`) — each mapping to 1 or 2 event types. `invoice-auto-email.tsx` is now async (`render()` from `@react-email/render` is Promise-returning), dispatcher call site adds 1 `await`, 14/14 unit tests updated + green, T105 integration 4/4 still green. Return shape `{ subject, html, text }` preserved for zero downstream change.
...
- [X] T110 [P] **PDF render benchmark shipped**. `tests/integration/invoicing/pdf-render-benchmark.test.ts` gated by `RUN_PERF=1`. Measured on live Neon: **p50=72ms · p95=88ms · p99=132ms** over 100 renders of a realistic 5-line invoice — well under the 800ms budget (post-critique E6). Smoke assertion always runs as a compile-guard. File is at `tests/integration/**` (not `tests/perf/**`) to match the vitest runner include glob.
- [X] T110a [P] **Invoice-list query perf shipped**. `tests/integration/invoicing/invoice-list-perf.test.ts` seeds 5,000 invoices × 2 tenants (10,000 total rows) and measures first-page 50-row `listPaged` alternating between tenants across 100 samples. Measured: **p50=317ms · p95=324ms · p99=344ms** — under the SC-005 500ms budget. Dual-tenant seed proves the tenant-scoped index still bounds cost at raw table size 10k. `RUN_PERF=1` gated; smoke assertion always runs.
...
- [X] T115t **Throwaway-tenant E2E infra shipped**. (a) `src/lib/env.ts` adds `E2E_X_TENANT_HEADER_ENABLED` flag REFUSED in production at boot. (b) `src/lib/tenant-context.ts` `resolveTenantFromRequest` honours an `X-Tenant` header when the flag is on — triple-gated (build-time env check + runtime flag + header validator). (c) `tests/e2e/helpers/throwaway-tenant.ts` — `createThrowawayTenant({seedSettings, seedPlan, seedMember})` provisions a uniquely-slugged tenant + cleanup. (d) `invoice-settings.spec.ts` **AS1** (VAT 7→10), **AS2** (valid PNG upload + persist), **AS4×3** (SVG / oversized / undersized rejections), **AS5** (bootstrap empty-state) all un-fixme'd behind `E2E_X_TENANT_HEADER_ENABLED=1` gate. Logo-upload tests use inline sharp-generated PNG buffers + `page.setInputFiles({buffer})` — no checked-in fixture files. Orphan blob bytes under the throwaway slug's prefix get swept by future tenant-purge jobs; no cross-test contamination because each test uses a unique slug.
- [X] T115b [P] `CLAUDE.md § Repository status` paragraph updated to reflect F4 review-ready state — Phase 10 summary, test counts (~1300 unit+contract + ~340 integration on live Neon), perf-headroom numbers (PDF render p95=88ms, invoice-list p95=324ms @ 5k×2, 50-writer seq ~10s), audit-coverage 17/18, human-gated residuals (T114, T117, T118, T124, T115t).
- [X] T115c [P] `docs/phases-plan.md § Phase 1` table updated: F4 row now `007-invoices-receipts` (not stale `004-mb-invoicing`), status column added with all 4 Phase-1 features marked ✅ Shipped / ✅ Review-ready.
...
- [X] T119 `specs/007-invoices-receipts/retrospective.md` Appendix B written — Phase 10 polish + carry-forward arc (commit range `0a1df68..7c3a7ba`, 15 commits, 24 canonical tasks closed). Covers: shipped table by sub-phase (10a/10b/10c/10d/10g/10f), what went well (perf headroom, idempotent-emit pattern composing, T113a caught 2 emit gaps, renderAndUploadPdf helper, T108 zero-behaviour-change), what was harder (drizzle-kit `when` ordering hazard, T125 STD-resolver blocker for throwaway-tenant, T108 async API churn), metrics, gotchas (7 items), open items (6 deferred with rationale), reviewer hand-off (4 steps).
...
- [X] T120 [P] **S1 — Host-header MTA dual-bind shipped**. Migration 0031 adds `tenant_invoice_settings_cross_tenant_probe` to `audit_event_type`. PATCH /api/tenant-invoice-settings asserts `tenantCtx.slug === env.tenant.slug` and on mismatch emits the probe + returns 403. Dormant in STD (resolver hard-codes to env.tenant.slug) but activates automatically when F10 MTA introduces per-request tenant resolution. 1/1 integration test verifies round-trip to audit_log. Barrel export `makeF4AuditPort()` added for route handlers needing standalone emit.
- [X] T121 [P] **S2 — CR/LF strip + helper extraction**. Shipped `src/lib/content-disposition.ts` with `buildAttachmentContentDisposition(raw)` + explicit CR/LF strip (header-injection defense-in-depth). Replaced 3 inline `asciiSafe` copies across admin invoice, admin CN, and portal invoice PDF routes. 7/7 unit tests green.
- [X] T122 [P] **S3 — Behavioral audit coverage complete**. `invoice_draft_updated` + `invoice_draft_deleted` shipped in T113a; this round adds `pdf_render_failed` via `issueInvoice` with a failing-render stub (emits AFTER tx rollback so the audit survives). 7/7 tests green on live Neon. T113a inventory updated — all 3 types now covered behaviorally.
- [X] T123 [P] **S4 — VAT source chain pin shipped**. `tests/integration/invoicing/vat-source-chain.test.ts` seeds `tenant_invoice_settings.vat_rate = '0.0850'`, issues an invoice via the real `issueInvoice` use-case with `drizzleTenantSettingsRepo` (no mocks on the VAT path), and asserts `invoices.vat_rate_snapshot = '0.0850'` + subtotal 1_000_000 satang → vat 85_000 satang. Locks in the post-R9 "tenant_invoice_settings is the authoritative VAT source" contract. 1/1 green on live Neon.
...
- [X] T125 [P] **Mutating CN E2E happy-path shipped** — pragmatic un-fixme without the throwaway-tenant infra (which `T115t` explicitly defers as ≈1-2 days separate work). Wired against the existing idempotent `seed-f4-e2e-admin-fixtures.ts` "E2E Mutation Co" 990001-series paid fixture: each run picks the current unmutated paid 99xxxx row → POSTs the credit note → asserts badge flip on the invoice-detail page. AS1 (full → Credited) in `credit-note-full.spec.ts`, AS2 (60% → Partially credited) in `credit-note-partial.spec.ts`. Gated on `E2E_HAS_ADMIN_FIXTURES=1` so default CI ticks stay deterministic; humans re-run the seeder between sessions. Integration layer's `credit-note-partial-accumulation.test.ts` already covers DB-state correctness — this closes the UI-glue regression net.
- [X] T126 [P] **S7 — `renderAndUploadPdf` helper shipped**. New `src/modules/invoicing/application/lib/render-and-upload.ts` wraps the render→upload pair with a generic `wrap: (kind, reason) => Error` callback so each use-case keeps its typed `*InternalError` throwable. 4 call sites refactored: `issueInvoice` (H+I), `recordPayment` (H+I), `issueCreditNote` (G+H), `issueCreditNote` J2 re-annotation (with `allowOverwrite + reasonPrefix='annotation'`). 23/23 integration tests green (seq-number-atomicity, audit-coverage, credit-note-partial-accumulation) verify behavior unchanged.
- [X] T127 [P] **CN-PDF synthetic-line golden shipped**. `tests/integration/invoicing/credit-note-pdf-golden.test.ts` captures the `pdfRender.render` call args for a 50%-partial CN and asserts: (1) exactly 1 synthetic line, (2) `unitPrice.satang === total.satang === creditAmount` (50_000), (3) bilingual description contains the original document number, (4) CN-level subtotal/vat/total match (50_000/3_500/53_500), (5) `creditNote` context carries original doc ref + reason. Protects C-1 fix from future `loaded.lines`-on-CN-path regression. 1/1 green on live Neon.

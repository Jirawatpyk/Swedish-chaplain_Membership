...
## Audit Event Coverage (15 F5 + 17 F4)

- [x] CHK020 Are all 15 F5-introduced audit event types enumerated with complete payload schemas (required keys + optional keys + severity)? [Completeness, Spec §FR-020 + data-model.md § 7]
...
- [x] CHK022 Is the actor-correlation requirement (every audit entry includes acting user + tenant + correlation_id + timestamp UTC) specified consistently across all 15 event types? [Consistency, Spec §FR-020 + data-model.md § 7]

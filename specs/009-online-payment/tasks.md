...
- [ ] T097 [US3] Create `src/app/(staff)/admin/invoices/[invoiceId]/_components/payment-timeline.tsx` — server component fetching audit-log rows for the invoice's payments + refunds; renders timeline with shadcn `<Card>` + icon per event type + actor + ISO timestamp (display in Thai BE for `th` locale per i18n convention).

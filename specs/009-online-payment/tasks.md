...
5. **Audit event count** — canonical = **16 values** (verified against spec.md FR-020 + data-model.md § 7; the "15 F5 audit events" prose in this tasks.md T035 + checklist CHK020 is stale and should be re-phrased "16" during doc-sync). 16 are: `payment_initiated, payment_succeeded, payment_failed, payment_canceled, payment_auto_refunded_stale_invoice, payment_auto_refunded_concurrent_manual_mark, payment_environment_mismatch, payment_cross_tenant_probe, refund_initiated, refund_succeeded, refund_failed, out_of_band_refund_detected, webhook_signature_rejected, webhook_api_version_mismatch, tenant_payment_settings_updated, online_payment_toggled`.
...
- [X] T013 ⚠️ **STUB only** (full impl deferred to sub-batch B post-migration 0037) — `src/modules/invoicing/application/use-cases/issue-credit-note-from-refund.ts` exports stable signature `{tenantId, invoiceId, refundId, amountSatang: bigint, reason, actorUserId, requestId?}` → `Result<IssueCreditNoteFromRefundOutput, {code:'not_implemented'}>`. Body returns err with explicit reason + attempted refund id. JSDoc documents the 5-step rewire plan.
...
- [ ] T015 ⏸️ **Deferred to post-sub-batch B** — integration test for all 3 wrappers cannot assert full end-to-end until T013 rewire lands. Will cover: markPaidFromProcessor → invoice `paid` + audit + outbox; issueCreditNoteFromRefund → CN with populated `source_refund_id`; getInvoiceForPayment → DTO projection correctness.

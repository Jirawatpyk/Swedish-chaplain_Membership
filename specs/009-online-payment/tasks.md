...
- [X] T121 [P] [US5] E2E test `tests/e2e/payment-stale-invoice-auto-refund.spec.ts` — mid-flight payment; admin voids invoice via F4 path; settle webhook arrives; assert auto-refund + audit `payment_auto_refunded_stale_invoice`. *(TDD shell with `test.fixme` — requires admin-void surface + Stripe webhook driver + member refund-notification UI; equivalent coverage at unit + T122 integration layers.)*
...
- [ ] T166 [P] (post-R7-round-2 — webhook PDF off hot path, MEDIUM priority — unlocks optimistic-UI deletion): move F4 receipt-PDF generation off the `payment_intent.succeeded` webhook hot path. **Current state** (observed 2026-04-26 in dev): webhook latency p95 ~5-15s because `markPaidFromProcessor` synchronously renders the PDF + uploads to Vercel Blob + revalidates the path. **Plan.md target** (§ Performance Goals) is p95 < 500ms. **Reality** doesn't hit that target until PDF gen is async. **Implementation options**: (a) Vercel Queues (preferred — at-least-once delivery, durable, GA per knowledge update; matches the new platform-native pattern); (b) extend the existing `notifications_outbox` cron pattern that F4 already uses for invoice email dispatch — add a `receipt_pdf_render` notification type and let the dispatcher render+upload+enqueue email in one worker pass. **Acceptance**: webhook p95 drops to 1-3s; payment confirmation no longer blocks on PDF gen; PDF eventually appears in member's email + portal download within ~10s of payment success; failure isolation — PDF gen errors don't fail the webhook (durable retry instead). **Unlock**: after p95 < 1s, the optimistic-UI overlay (sessionStorage + BroadcastChannel + useSyncExternalStore in `optimistic-paid.ts`) becomes redundant — server data catches up before user sees the page → delete the entire layer (~250 LOC: `_components/optimistic-paid.ts` + `_components/optimistic-paid-overlay.tsx` + `tests/unit/components/payments/optimistic-paid.test.ts` + `dispatchInvoicePaid` call sites in PaySheet + `useOptimisticPaid` call site in `<PayNowButton>` + `<OptimisticPaidOverlay>` wrapper in invoice detail page). Net codebase delta: +1 worker − 250 LOC of overlay. Phase 9 polish — schedule alongside T159 retrospective.

...
- [ ] T001 **F4 merge gate** — verify F4 PR #12 (`007-invoices-receipts`) is merged to `main`. If not merged: STOP. F5 implementation cannot start until F4's `markPaid` + `issueCreditNote` use-cases are stable on `main`.
- [ ] T002 Rebase `009-online-payment` branch on latest `main` (post-F4-merge); resolve any `src/modules/invoicing/index.ts` conflicts; verify F4 barrel exports unchanged via `pnpm typecheck`.
- [ ] T003 [P] Verify Stripe deps already installed (`stripe@^22`, `@stripe/stripe-js@^9`, `@stripe/react-stripe-js@^6`) per commit 9555f43 by running `pnpm install --frozen-lockfile`.
- [ ] T004 [P] Verify `.claude/settings.json` has `stripe` + `neon` plugin entries enabled (per commit 9555f43).
- [ ] T005 Update `src/lib/env.ts` zod schema to add F5 env vars: `STRIPE_SECRET_KEY` (`.describe('SECRET — do not log')`), `STRIPE_PUBLISHABLE_KEY`, `STRIPE_WEBHOOK_SECRET` (`.describe('SECRET — do not log')`), `STRIPE_API_VERSION` (e.g. `2025-09-30.basil`), `STRIPE_ACCOUNT_ID_SWECHAM`, `STRIPE_LIVE_MODE` (boolean), `FEATURE_F5_ONLINE_PAYMENT` (boolean, default `false` for safety).
- [ ] T006 Update `vercel.json` with: (a) `crons` entry `{path: '/api/internal/metrics/stale-pending-count', schedule: '*/5 * * * *'}` (R2-E3); (b) verify webhook route `/api/webhooks/stripe` does NOT have edge runtime override (default Node.js runtime).
- [ ] T007 [P] Pull latest env vars from Vercel: `vercel env pull .env.local`; verify all 7 STRIPE_* vars present + correctly scoped (test for dev/staging, live for prod).
- [ ] T008 [P] Document Stripe CLI dev workflow in onboarding by linking `quickstart.md` from team's CONTRIBUTING.md or equivalent README section (one-line link is sufficient).
- [ ] T009 [P] Verify `BLOB_READ_WRITE_TOKEN` (F4-inherited) + `CRON_SECRET` (F4-inherited) env vars present in `.env.local` post-pull; F5 uses both — Vercel Blob for refund-confirmation credit-note PDFs delegated to F4 outbox; CRON_SECRET signs the stale-pending-count cron handler (T138).
...
- [ ] T010 Open small isolated PR adding F4 barrel exports needed by F5: edit `src/modules/invoicing/index.ts` to re-export `markPaidFromProcessor`, `issueCreditNoteFromRefund`, `getInvoiceForPayment`, and `Money` VO (as `AmountSatang` alias). NO F5-specific code in this PR.
- [ ] T011 [P] Add F4 unit tests for the new barrel exports in `tests/unit/invoicing/barrel-exports.test.ts` — assert public surface includes the 4 new exports + existing F4 exports unchanged.
- [ ] T012 Implement `markPaidFromProcessor(tenantCtx, invoiceId, {method, paymentIntentId, chargeId, settlementDate})` in `src/modules/invoicing/application/mark-paid-from-processor.ts` as a thin wrapper around the existing `markPaid` use-case (mapping processor metadata → F4 fields).
- [ ] T013 Implement `issueCreditNoteFromRefund(tenantCtx, invoiceId, {refundId, amountSatang, reason})` in `src/modules/invoicing/application/issue-credit-note-from-refund.ts` — populates `credit_notes.source_refund_id` and creates a single-line CN with the supplied amount.
- [ ] T014 Implement `getInvoiceForPayment(tenantCtx, invoiceId)` in `src/modules/invoicing/application/get-invoice-for-payment.ts` returning a read-only DTO (id, status, total_satang, member_id, tenant_id) for F5 to make payability + ownership decisions.
- [ ] T015 Add F4-side tests in `tests/integration/invoicing/processor-bridge.test.ts` — verify the 3 new wrapper use-cases work end-to-end against live Neon.

### F4 email template extension (post-critique FR-027)

- [ ] T016 Edit `src/modules/invoicing/infrastructure/email/templates/invoice-issued.tsx` to add bilingual "Pay online" CTA button linking to `/portal/invoices/[id]?pay=1` with `utm_source=invoice_email&utm_medium=email&utm_campaign=f5_pay_online` query params; conditional render on tenant `online_payment_enabled = true`. Use shadcn-button-equivalent inline-style button.
- [ ] T017 Regenerate F4 email-template Vitest snapshots: `pnpm test --filter=invoicing -u`; verify regenerated snapshots include the new CTA in EN+TH+SV variants.
- [ ] T018 [P] Add e2e test `tests/e2e/payment-email-deep-link.spec.ts` — issue F4 invoice → capture auto-email → assert CTA href = `/portal/invoices/[id]?pay=1&utm_*` → simulate click (signed-out path: sign-in → returnUrl preservation; signed-in path: direct land).

### Migrations + Drizzle schema

- [ ] T019 [P] Create `drizzle/migrations/0032_create_payments.sql` per `data-model.md` § 2 (15 columns + 5 indexes + 8 CHECK + RLS+FORCE + tenant policy).
- [ ] T020 [P] Create `drizzle/migrations/0033_create_refunds.sql` per `data-model.md` § 3 (13 columns + 4 indexes + 4 CHECK + RLS+FORCE + tenant policy).
- [ ] T021 [P] Create `drizzle/migrations/0034_create_tenant_payment_settings.sql` per `data-model.md` § 4 (11 columns + 2 indexes + 4 CHECK + RLS+FORCE + tenant policy).
- [ ] T022 [P] Create `drizzle/migrations/0035_create_processor_events.sql` per `data-model.md` § 5 (10 columns + 4 indexes + 2 CHECK + RLS+FORCE + special policies for INSERT-allow-NULL-tenant + UPDATE post-resolution + DELETE-forbidden).
- [ ] T023 Create `drizzle/migrations/0036_seed_swecham_payment_settings.sql` per `data-model.md` § 4.5 — INSERT one row for SweCham tenant with `enabled_methods=['card','promptpay']`, `online_payment_enabled=true`, processor=`stripe`, environment from `app.env` setting.
- [ ] T024 [P] Create `drizzle/migrations/0037_credit_notes_add_source_refund_id.sql` per `data-model.md` § 6 — ADD COLUMN `source_refund_id TEXT NULL REFERENCES refunds(id)` + partial INDEX on non-NULL.
- [ ] T025 **R2-E4 Review-Gate blocker**: Create `drizzle/migrations/0038_audit_log_add_retention_years.sql` per `data-model.md` § 7.2 — atomic transaction containing (a) `ALTER TABLE audit_log ADD COLUMN retention_years SMALLINT NOT NULL DEFAULT 5`, (b) CHECK constraint `retention_years IN (5, 10)`, (c) backfill `UPDATE audit_log SET retention_years = 10 WHERE event_type IN ('invoice_issued', 'invoice_paid', 'invoice_voided', 'credit_note_issued', 'invoice_pdf_resent', 'invoice_pdf_regenerated')`. Verify the F4 event-type list against `specs/007-invoices-receipts/data-model.md` § Audit events before merging.
- [ ] T026 Apply migrations to dev DB: `pnpm drizzle-kit migrate`; verify all 7 migrations succeed; verify `audit_log.retention_years` exists with correct backfilled values via `psql` query.
- [ ] T027 Create Drizzle schema in `src/modules/payments/infrastructure/schema.ts` — 4 `pgTable` definitions matching migrations 0032–0035 + relations(`payments↔refunds↔credit_notes`) + inferred types `NewPayment`, `SelectPayment`, etc. (Infrastructure-only; do NOT export from module barrel).
- [ ] T028 Add cross-cutting RLS-coverage test extension: extend `tests/integration/rls-coverage.test.ts` to include the 4 F5 tables — any tenant-scoped F5 table missing RLS+FORCE+policy = automatic red CI.

### Module skeleton (Clean Architecture per Constitution Principle III)

- [ ] T029 [P] Create module barrel `src/modules/payments/index.ts` (initially empty exports; populated as use-cases land).
- [ ] T030 [P] Add ESLint `no-restricted-imports` rule (`eslint.config.mjs` or equivalent) forbidding deep imports into `src/modules/payments/{domain,application,infrastructure}/**` from outside the module.
- [ ] T031 [P] Create empty layer scaffolds: `src/modules/payments/{domain,application,infrastructure}/.gitkeep` + add module to TypeScript `tsconfig.json` paths if needed.

### Cross-cutting infra

- [ ] T032 [P] Extend `src/lib/logger.ts` redact list per plan.md § Constraints — add `card_number`, `card_cvc`, `card[*]`, `stripe_secret_key`, `stripe_webhook_secret`, `Stripe-Signature`, full webhook body keys (whitelist `event_id+event_type+api_version+livemode` only), PAN regex `^(3[47]|4|5[1-5]|6(?:011|5))\\d+$`. Add unit test in `tests/unit/lib/logger-redact.test.ts` asserting all 7 forbidden field names + PAN-shaped strings are redacted to `[REDACTED]`.
- [ ] T033 [P] Extend `src/app/middleware.ts` CSP builder to add Stripe iframe allowlist conditional on route — `script-src 'self' https://js.stripe.com`, `frame-src https://js.stripe.com https://hooks.stripe.com`, `connect-src 'self' https://api.stripe.com` ONLY on `/portal/invoices/*` + `/admin/invoices/*` routes; webhook route does NOT need these.
- [ ] T034 [P] Extend RBAC matrix in `src/modules/auth/infrastructure/rbac-guard.ts` (or equivalent F1 location) to add `payments:*`, `refunds:*`, `payment-settings:*` resource families per security.md § 4 matrix.
- [ ] T035 [P] Extend `src/modules/auth/domain/audit-event-types.ts` (or equivalent F1+F4 location) to add the 15 F5 audit event types (`payment_initiated`, `payment_succeeded`, `payment_failed`, `payment_canceled`, `payment_auto_refunded_stale_invoice`, `payment_auto_refunded_concurrent_manual_mark`, `payment_environment_mismatch`, `payment_cross_tenant_probe`, `refund_initiated`, `refund_succeeded`, `refund_failed`, `out_of_band_refund_detected`, `webhook_signature_rejected`, `webhook_api_version_mismatch`, `tenant_payment_settings_updated`, `online_payment_toggled`).
...
- [ ] T042 [P] [US1] Contract test `tests/contract/payments/post-webhooks-stripe-events.contract.test.ts` — `payment_intent.succeeded` happy path: assert 200 + `payments.status='succeeded'` + F4 `markPaid` called once + audit chain `payment_initiated → payment_succeeded → invoice_paid`.
- [ ] T043 [P] [US1] Integration test `tests/integration/payments/tenant-isolation.test.ts` (**Constitution v1.4.0 Principle I clause 3 — Review-Gate blocker**) — create 2 tenants; create payments + refunds + tenant_payment_settings + processor_events for each; assert zero cross-tenant visibility on SELECT/INSERT/UPDATE/DELETE; assert `payment_cross_tenant_probe` audit emission.
- [ ] T044 [P] [US1] Integration test `tests/integration/payments/webhook-signature.test.ts` — 4 scenarios: valid → 200; missing `Stripe-Signature` → 401 + `webhook_signature_rejected{reason='missing_header'}` audit; malformed → 401; tampered body → 401. Verification MUST occur before body parse.
- [ ] T045 [P] [US1] Integration test `tests/integration/payments/webhook-idempotency.test.ts` (**SC-005 / FR-008**) — deliver same `payment_intent.succeeded` event twice; assert 2nd delivery returns 200 with no side effects (one Payment row, one F4 markPaid invocation, one outbox row, one `processor_events` row).
- [ ] T046 [P] [US1] E2E test `tests/e2e/payment-card-happy-path.spec.ts` — `--workers=1`; full sign-in → Pay-now → Stripe Elements iframe (assert iframe origin = `js.stripe.com`) → submit `4242…` → confirmation panel + receipt download CTA + audit chain verified.
...
- [ ] T067 [P] [US1] Implement `src/modules/payments/infrastructure/audit/payments-audit.ts` — emits the 15 audit event types with correct `retention_years` per data-model.md § 7.1 mapping (5 or 10).
...
- [ ] T138 Implement `src/app/api/internal/metrics/stale-pending-count/route.ts` — Vercel Cron handler signed with `CRON_SECRET`; runs Drizzle query per plan.md § VII.Metrics; emits one OTel gauge per tenant.

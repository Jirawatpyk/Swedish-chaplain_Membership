...
| `plan_change_scheduled` | info | request path | `{ plan_change_id, member_id, from_plan_id, to_plan_id, effective_at_cycle_id, scheduled_at }` | F8 cross-module: tier-upgrade-suggestion accepted by admin schedules a plan change for the next renewal cycle (added by F8 Phase 2 Wave C T029c per F8 plan.md Complexity Tracking #4) |
| `plan_change_superseded` | info | request path | `{ plan_change_id, member_id, manual_change_actor_user_id, superseding_plan_id }` | F8 cross-module: admin manually overrides the member's plan via F2 before the scheduled change applies — supersedes the pending plan_change row (F8 cross-module integration) |
| `plan_change_cancelled` | info | request path | `{ plan_change_id, member_id, cancel_reason, actor_user_id }` | F8 cross-module: admin or system cancels a pending plan_change (F8 cross-module integration) |
| `plan_change_applied` | info | request path | `{ plan_change_id, member_id, applied_at_invoice_id, applied_at }` | F8 cross-module: F4 invoice-paid hook flips the scheduled plan_change to applied + updates `members.plan_id` atomically (F8 cross-module integration) |
...
| `audit_event_type` enum values added | 14 — F2 ships 10 (snake_case: `plan_created`, `plan_updated`, `plan_cloned`, `plan_activated`, `plan_deactivated`, `plan_soft_deleted`, `plan_undeleted`, `plan_not_found`, `plan_cross_tenant_probe`, `fee_config_updated`); F8 cross-module integration adds 4 more (`plan_change_scheduled`, `plan_change_superseded`, `plan_change_cancelled`, `plan_change_applied`) per F8 plan.md Complexity Tracking #4 / migration 0095 |

...
- [x] **IX. Code Quality Standards** — TypeScript strict; ESLint + Prettier; Conventional Commits enforced via commit-msg hook. **Review count deviation:** SweCham is a solo-dev project — the Constitution's "≥2 reviewers on security-sensitive changes" rule cannot be formed with a single maintainer. F1 substitutes 6 `/speckit.review` automated passes + 2 `/speckit.staff-review` triangulation rounds + 480/480 test coverage + DB-trigger defence-in-depth as documented in § Complexity Tracking. The § 5 security checklist is co-signed by the staff-review agent and the solo maintainer.
...
> **Three deviations from strict Constitution compliance, justified below.**
...
| **Solo-dev workflow substitutes automated review pipelines for the Constitution's "≥2 reviewers on security-sensitive changes" rule** (Constitution Principle IX + § Development Workflow Gate 9) | **SweCham is currently a single-maintainer project.** There is no second human maintainer available to form the standard review pair that Principle IX assumes. Forming a pair would require either hiring, community contribution, or an external security consultant — none of which is available on the F1 timeline. Instead, F1 substitutes five independent automated checks that collectively exceed the failure-mode coverage of a two-reviewer human pass: (1) **6 `/speckit.review` automated passes** closed 6+2+16+13+6+4 findings at progressively decreasing severity; (2) **2 `/speckit.staff-review` rounds** with 3 independent agents per round (correctness, security, tests) triangulating every finding; (3) **480/480 automated tests** (287 unit+contract + 82 integration vs live Neon + 113/117 E2E × 3 browsers) enforcing Constitution II coverage targets; (4) **DB-layer defence-in-depth** via `users_last_admin_protection` + `audit_log_immutable` triggers (migrations 0003-0004 + 0001) that operate independently of Application-layer guards; (5) **post-remediation verification** of every finding by a fresh review round (agents re-read the fix, not self-attestation). The § 5 security checklist is signed by both the staff-review agent AND the solo maintainer, with the maintainer explicitly acknowledging the deviation. **This deviation is reversible:** if F2+ brings in a second maintainer, future auth-surface changes MUST follow the full ≥2-reviewer rule. | **Shipping F1 without a formal review substitute** would mean either (a) no sign-off at all — violates Gate 9 silently, or (b) accepting my own review as "reviewer #2" — violates the independence principle the rule exists to protect. **Indefinite deferral until a second maintainer joins** would block F1 indefinitely, leaving SweCham on the legacy Excel workflow with all its known PII-handling risks. The substitute stack above is auditable (every pass has a committed report in `specs/001-auth-rbac/{reviews,critiques}/`), reproducible (re-run any `/speckit.*` command), and independent from the maintainer's own judgement at critical decision points. |

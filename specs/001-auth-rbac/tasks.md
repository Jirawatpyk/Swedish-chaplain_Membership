...
- [X] T042 [P] Write contract test `tests/contract/csrf.test.ts` that sends requests with missing / wrong / correct Origin header and asserts 403 vs pass (security.md T-07) — **closed 2026-04-10 (verify gate)**: 20 cases exercising `checkCsrf()` directly (the pure function `src/proxy.ts` delegates to) across method-safe / unprotected-path / exempt-path / missing-origin / origin-not-allowed / origin-allowed / dev-loopback matrices. Contract-level 403-vs-pass behaviour is ALSO exercised end-to-end by `tests/integration/middleware/readonly-mode.test.ts` which asserts every response carries the security headers + request-id from `proxy.ts`.
...
- [X] T056 [P] [US1] Integration test `tests/integration/auth/sign-in.test.ts` against real Postgres — successful sign-in creates session row, updates `last_sign_in_at`, emits audit event `sign_in_success` — **landed 2026-04-10** (Phase 3 deferred list cleared): green in `pnpm test:integration` against live Neon `ap-southeast-1`.
- [X] T057 [P] [US1] Integration test `tests/integration/auth/enumeration-timing.test.ts` — 50 unknown-email + 50 known-email-wrong-password attempts, asserts p95 latency difference ≤ 5 ms (security.md T-03, spec SC-019) — **landed 2026-04-10**: green against live Neon.
- [X] T058 [P] [US1] Integration test `tests/integration/auth/enumeration-message.test.ts` — response bodies byte-identical across all 3 failure modes (spec FR-016, SC-019) — **landed 2026-04-10**: green against live Neon.
- [X] T059 [P] [US1] Integration test `tests/integration/auth/brute-force.test.ts` — 100 sign-in attempts per minute from one IP, asserts ≤ 10 reach argon2 path (security.md T-02, spec SC-010) — **landed 2026-04-10**: green against live Neon + Upstash.
- [X] T060 [P] [US1] Integration test `tests/integration/auth/lockout.test.ts` — 5 failures within 15 min triggers lockout, 6th attempt returns 403 account-locked with Retry-After header (spec FR-013, security.md T-01) — **landed 2026-04-10**: green against live Neon.
- [X] T061 [P] [US1] Integration test `tests/integration/auth/session-rotation.test.ts` — a cookie set before sign-in is replaced with a new 32-hex ID after sign-in (security.md T-06) — **landed 2026-04-10**: green against live Neon.
...
- [X] T063 [P] [US1] E2E test `tests/e2e/staff-sign-in.spec.ts` — happy path: open sign-in page, fill form, submit, land on `/admin`, verify user menu + role badge visible — **landed 2026-04-10**: 3/3 cases green against dedicated `e2e-admin@swecham.test` user (Phase 10 `beforeAll` rate-limit clear). See qa-20260410-1410.md TC-L-023.
- [X] T064 [P] [US1] E2E test `tests/e2e/staff-sign-in-a11y.spec.ts` using `@axe-core/playwright` — zero WCAG 2.1 AA violations on sign-in page (spec SC-005) — **landed 2026-04-10**: bundled into `staff-sign-in-a11y.spec.ts` + re-run as part of TC-L-023, zero serious/critical violations after the Phase 10 `--destructive` contrast fix.
...
- [X] T083 [P] [US2] E2E test `tests/e2e/manager-read-only.spec.ts` — sign in as manager, verify the `/admin/users` page loads with the table visible but zero Disable/Enable row actions and the Invite button in a disabled state, then POST to `/api/auth/invite` as the manager session and assert 403 — **landed 2026-04-10 (verify gate close-out)**: seeded `e2e-manager@swecham.test` via `scripts/seed-e2e-user.ts`. 2/2 cases green. Covers both the UI affordance (no destructive buttons rendered) AND the API path (403 `forbidden` + `manager_denied_write` audit event). Skips cleanly when `E2E_MANAGER_EMAIL`/`E2E_MANAGER_PASSWORD` env vars are absent.
...
- [X] T094 [P] [US3] Integration test `tests/integration/auth/reset-session-revocation.test.ts` — reset completion deletes all sessions for the user AND emits `concurrent_sessions_revoked` audit event — **closed 2026-04-10 (verify gate)**: merged into `tests/integration/auth/password-reset.test.ts` happy-path case (lines 163-185 assert both `password_reset_completed` and `concurrent_sessions_revoked` audit rows + zero remaining session rows). No separate file exists or is needed.

...
- [ ] T001 Create Next.js 16 project at repository root via `pnpm create next-app@latest .` with TypeScript, App Router, Tailwind, ESLint enabled, no `src/` dir flag (we'll manually create the layered structure)
- [ ] T002 Create the directory layout from plan.md § Project Structure (`src/app/(staff)`, `src/app/(member)`, `src/app/(auth-public)`, `src/modules/auth/{domain,application,infrastructure}`, `src/components/{ui,auth,shell,layout}`, `src/i18n/messages`, `src/lib`, `tests/{unit,contract,integration,e2e}`, `drizzle/migrations`, `scripts`)
- [ ] T003 Install runtime dependencies in `package.json`: `next@^16`, `react@^19`, `drizzle-orm`, `drizzle-kit`, `postgres`, `@node-rs/argon2`, `next-intl`, `zod`, `react-hook-form`, `@hookform/resolvers`, `@upstash/ratelimit`, `@upstash/redis`, `resend`, `@react-email/components`, `next-themes`, `sonner`, `lucide-react`, `@vercel/otel`, `@opentelemetry/api`, `pino`
- [ ] T004 Install dev dependencies: `vitest`, `@vitest/coverage-v8`, `@testing-library/react`, `@testing-library/user-event`, `playwright`, `@axe-core/playwright`, `msw`, `tsx`, `@types/node`, `@types/react`, `@types/react-dom`
- [ ] T005 [P] Configure TypeScript strict mode in `tsconfig.json` with `strict: true`, `noUncheckedIndexedAccess: true`, `noImplicitOverride: true`, `exactOptionalPropertyTypes: true`, path aliases `@/*` → `src/*`
- [ ] T006 [P] Configure ESLint in `.eslintrc.cjs` with Next.js preset + `no-restricted-imports` boundary rules for `src/modules/auth/domain/**` (forbid drizzle/next/react/resend/upstash) and `src/modules/auth/application/**` (forbid drizzle/next/react)
- [ ] T007 [P] Configure Prettier in `.prettierrc.json` and `.prettierignore` (2-space indent, single quotes, trailing comma)
- [ ] T008 [P] Install and initialise shadcn/ui via `pnpm dlx shadcn@latest init` then `pnpm dlx shadcn@latest add alert alert-dialog avatar badge button card dialog dropdown-menu form input label select separator skeleton sonner tabs toast tooltip`
- [ ] T009 [P] Configure Tailwind CSS v4 in `tailwind.config.ts` including the custom `shimmer` keyframes and `animate-shimmer` utility per `docs/ux-standards.md` § 2.1
- [ ] T010 [P] Configure Vitest in `vitest.config.ts` with coverage thresholds: Domain 100% lines, Application 80% lines+branches, security-critical files (sign-in, sign-out, change-password, reset-password, policies) 100% branches
- [ ] T011 [P] Configure Playwright in `playwright.config.ts` with projects for Chromium desktop + Mobile Safari + Chrome Android, `baseURL: http://localhost:3000`, `@axe-core/playwright` preset
- [ ] T012 [P] Configure next-intl in `src/i18n/config.ts` and `src/i18n/request.ts` with three locales (`en` default/fallback, `th`, `sv`) and create empty message catalogues `src/i18n/messages/{en,th,sv}.json`
- [ ] T013 Configure Drizzle in `drizzle.config.ts` pointing at `src/modules/auth/infrastructure/db/schema.ts` and `drizzle/migrations/`
- [ ] T014 Add scripts to `package.json`: `dev`, `build`, `start`, `test`, `test:watch`, `test:coverage`, `test:integration`, `test:e2e`, `lint`, `typecheck`, `check:i18n`, `db:generate`, `db:migrate`, `db:studio`
- [ ] T015 Add commit-msg hook at `.husky/commit-msg` enforcing Conventional Commits via `@commitlint/cli` + `@commitlint/config-conventional`
- [ ] T016 Create `.env.example` with placeholder values for `DATABASE_URL`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`, `RESEND_API_KEY`, `RESEND_WEBHOOK_SIGNING_SECRET`, `AUTH_COOKIE_SIGNING_SECRET`, `APP_BASE_URL`, `APP_ALLOWED_ORIGINS`, `READ_ONLY_MODE`

**Checkpoint**: `pnpm dev` starts the app · `pnpm test` runs Vitest · `pnpm test:e2e` runs Playwright · `pnpm lint` and `pnpm typecheck` pass on an empty project

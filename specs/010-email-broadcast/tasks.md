...
- [ ] T001 Add new dependencies to `package.json`: `@tiptap/react@^3` `@tiptap/starter-kit@^3` `isomorphic-dompurify@^2` `email-validator@^2` `@tanstack/react-virtual@^3` (CHK039 virtualization). Run `pnpm install` and commit `pnpm-lock.yaml`.
- [ ] T002 [P] Pin exact versions of `@tiptap/react`, `@tiptap/starter-kit`, `isomorphic-dompurify` in `package.json` (sanitiser is security-critical per Plan § Constitution OWASP A06; version bumps require extra reviewer scrutiny).
- [ ] T003 [P] Extend `src/lib/env.ts` zod schema: add `RESEND_BROADCASTS_API_KEY` (string min 1) · `RESEND_BROADCASTS_WEBHOOK_SECRET` (string min 1) · `UNSUBSCRIBE_TOKEN_SECRET` (string min 32 — distinct from `AUTH_COOKIE_SIGNING_SECRET` per research.md § 4) · `FEATURE_F7_BROADCASTS` (boolean default `false`). Update `src/lib/env.example.ts` mirror.
- [ ] T004 [P] Add `RESEND_BROADCASTS_API_KEY`, `RESEND_BROADCASTS_WEBHOOK_SECRET`, `UNSUBSCRIBE_TOKEN_SECRET`, `FEATURE_F7_BROADCASTS` to `vercel env add` for `development`, `preview`, `production` (run via Vercel CLI, NOT committed).
- [ ] T005 [P] Update `vercel.json` to add cron entry for `/api/cron/broadcasts/dispatch-scheduled` — but commented-out / disabled because Vercel Hobby plan limits 1×/day. Use cron-job.org HTTP trigger instead per F4/F5 pattern (CRON_SECRET reused). Document the cron-job.org configuration in `docs/runbooks/cron-jobs.md`.
- [ ] T006 [P] Pin Sarabun TTF fonts already inherited from F4 — NO new font assets for F7 (Tiptap inherits via Tailwind tokens).
- [ ] T007 [P] Update `eslint.config.mjs` `no-restricted-imports` rule: `src/modules/broadcasts/*/index.ts` is the only entry point; deep-imports forbidden from outside the module. Domain layer forbids `next`, `drizzle-orm`, `resend`, `@tiptap/*`, `isomorphic-dompurify`, `react`. Application layer forbids `drizzle-orm`, `next`, `react`, `@tiptap/*`, `isomorphic-dompurify`, `resend`.
- [ ] T008 [P] Add `next-bundle-analyzer` script to `package.json` per perf.md CHK038 (compose ≤180 KB gz / queue ≤120 KB gz / detail ≤100 KB gz / benefits ≤80 KB gz / unsubscribe ≤30 KB gz). Wire into `pnpm build:analyse`.
- [ ] T009 Create empty bounded-context skeleton at `src/modules/broadcasts/{domain,application,infrastructure}/.gitkeep` and `src/modules/broadcasts/index.ts` (empty barrel — populated by Foundational + per-story tasks).
- [ ] T010 Update `CLAUDE.md` § Active Technologies — add F7 row noting `@tiptap/react@^3` + `isomorphic-dompurify@^2` + `email-validator@^2` + `@tanstack/react-virtual@^3` + `RESEND_BROADCASTS_API_KEY` + `UNSUBSCRIBE_TOKEN_SECRET` + `FEATURE_F7_BROADCASTS`. Run `.specify/scripts/powershell/update-agent-context.ps1 -AgentType claude` to surface F7 in Recent Changes section.
...
- [ ] T011 Create migration `drizzle/migrations/0064_create_broadcasts.sql` — defines `broadcasts` table per data-model.md § 1.1 with: composite PK `(tenant_id, broadcast_id)`, all 8 status enum values, all check constraints (subject length / body size / custom cap / estimated cap / quota_year_only_on_sent / retention_years), 4 indexes (tenant_status_member · tenant_submitted_at partial · tenant_scheduled partial · resend_broadcast_id_uniq partial). RLS+FORCE policy `USING (tenant_id = current_setting('app.current_tenant', TRUE))`. Generate via `pnpm drizzle-kit generate` after creating `src/modules/broadcasts/infrastructure/schema.ts` (T020).
- [ ] T012 [P] Create migration `drizzle/migrations/0065_create_broadcast_deliveries.sql` — `broadcast_deliveries` table per data-model.md § 1.2 with: composite PK `(tenant_id, delivery_id)`, `(tenant_id, resend_event_id) UNIQUE` index (FR-025 idempotency primitive), `(tenant_id, broadcast_id, status)` index, `(tenant_id, recipient_email_lower)` index. RLS+FORCE policy.
- [ ] T013 [P] Create migration `drizzle/migrations/0066_create_marketing_unsubscribes.sql` — `marketing_unsubscribes` table per data-model.md § 1.3 with: PK `(tenant_id, email_lower)` (FR-018 tenant-scope invariant), reason enum, `member_id` nullable + `ON DELETE SET NULL`. RLS+FORCE policy. **Indefinite retention** — NO retention prune policy (GDPR Art. 21 requires forever).
- [ ] T014 [P] Create migration `drizzle/migrations/0067_create_broadcast_segment_definitions.sql` — `broadcast_segment_definitions` per data-model.md (per-tenant segment metadata). RLS+FORCE policy.
- [ ] T015 [P] Create migration `drizzle/migrations/0068_seed_default_segment_definitions.sql` — one-off seed inserting default segments (`all_members`, `tier:premium`, `tier:large`, `tier:regular`, `tier:diamond`, `tier:platinum`, `tier:gold`, `event_attendees_last_90d`, `custom`) for the SweCham tenant. Idempotent (`ON CONFLICT DO NOTHING`).
- [ ] T016 [P] Create migration `drizzle/migrations/0069_audit_log_extend_retention_default_trigger.sql` — extend the v1.4.0 trigger from F5's 0063 migration to map all 37 F7 audit event types to default `retention_years = 5` (NOT tax-document overlap).
- [ ] T017 [P] Create migration `drizzle/migrations/0070_alter_members_add_broadcasts_halted_until_admin_review.sql` — add `members.broadcasts_halted_until_admin_review boolean NOT NULL DEFAULT false` (Q14 per-broadcast complaint-rate auto-halt). Index `(tenant_id) WHERE broadcasts_halted_until_admin_review = true` for fast halt-list queries.
- [ ] T018 [P] Create migration `drizzle/migrations/0071_alter_members_add_broadcasts_acknowledged_at.sql` — add `members.broadcasts_acknowledged_at timestamp with time zone` (Q15 GDPR Art. 7 acknowledgement). Index `(tenant_id, member_id) WHERE broadcasts_acknowledged_at IS NULL` (banner-eligible members).
- [ ] T018a [P] Create migration `drizzle/migrations/0072_alter_broadcast_actor_role_enum_add_system.sql` (FR-005 — N1 remediation post-/speckit.analyze 2026-04-29). Adds `'system'` value to `broadcast_actor_role` enum: `ALTER TYPE broadcast_actor_role ADD VALUE IF NOT EXISTS 'system';`. Required for T178a auto-cancel cascade to emit audit events with `actor_role = 'system'` without enum violation. The `'system'` value is currently used only by the F3 archival/erasure cascade in T178a; future system-initiated state mutations (e.g., complaint-rate auto-halt, scheduled-send cron transitions) MAY also use it. NOTE: Postgres `ALTER TYPE ... ADD VALUE` is non-transactional — must run outside an explicit transaction block. Drizzle-kit handles this automatically via the migration runner.
- [ ] T019 Apply all 9 migrations (0064–0072) via `pnpm drizzle-kit migrate` against staging Neon Singapore. Verify schema with `\d broadcasts` + `SELECT enum_range(NULL::broadcast_actor_role);` returns 3 values.

### 2.2 Drizzle schema + repos (Infrastructure layer skeleton)

- [ ] T020 Create `src/modules/broadcasts/infrastructure/schema.ts` — Drizzle schema for all 4 F7 tables matching migrations 0064–0067 + 3 enum types (`broadcastStatusEnum`, `broadcastSegmentTypeEnum`, `broadcastActorRoleEnum`, `broadcastDeliveryStatusEnum`, `marketingUnsubscribeReasonEnum`). Inferred types live here only (Principle III — must NOT leak to Application/Domain).
- [ ] T021 [P] Create `tests/integration/rls-coverage.test.ts` extension verifying all 4 F7 tables have RLS+FORCE+policy. Runs as part of existing rls-coverage suite. **MUST be RED first** then turn GREEN after T019.
- [ ] T022 [P] Create RED foundational integration test `tests/integration/broadcasts/tenant-isolation.test.ts` — Constitution v1.4.0 Principle I clause 3 Review-Gate blocker. Two tenants (`tenant-A` + `tenant-B`), each seeds members + plans + 1 draft broadcast + 1 suppression + 1 segment definition. Asserts: from `tenant-A` context, reads + updates + deletes against `tenant-B`'s rows return 0 rows / no-op (RLS denial). FR-038 + spec § FR-037. Initial state RED.
- [ ] T023 [P] Create RED foundational integration test `tests/integration/broadcasts/kill-switch.test.ts` — covers 4 scenarios: (1) flag `FEATURE_F7_BROADCASTS=false` → compose surface returns 503 + member sees fallback UI; (2) flag `true` → normal flow; (3) **mid-flight visibility (Spec § Edge Cases L341 — Coverage Gap C3 from /speckit.analyze)** — seed broadcasts in `submitted` + `approved` states → toggle flag to `false` → assert admin queue STILL shows existing in-flight broadcasts (admin can complete/reject them) BUT member-side compose returns 503; (4) re-enable flag → normal flow resumes for new submissions. Initial state RED.

### 2.3 Domain skeleton + ports

- [ ] T024 [P] Create Domain value objects in `src/modules/broadcasts/domain/value-objects/`:
...
- [ ] T025 [P] Create Domain invariants in `src/modules/broadcasts/domain/invariants/`:
...
- [ ] T026 [P] Create Domain policies in `src/modules/broadcasts/domain/policies/`:
...
- [ ] T027 [P] Create Domain aggregates in `src/modules/broadcasts/domain/`:
...
- [ ] T028 [P] Create Application port interfaces in `src/modules/broadcasts/application/ports/`:
  - `broadcasts-repo.ts`
...
- [ ] T029 Extend `src/modules/members/index.ts` with 4 new public exports: `getMembersBySegment(tenantCtx, segmentType, params)` · `getMemberPrimaryContact(tenantCtx, memberId)` · `lookupContactEmailInTenant(tenantCtx, emailLower)` · `lookupMemberPrimaryContactEmailInTenant(tenantCtx, emailLower)`. Add corresponding F3-side use-case implementations (NOT just shells) returning typed Results. Plus 2 new exports for the halt-flag column: `getMembersHaltedInTenant(tenantCtx): MemberHaltSummary[]` (admin queue surface) and `setMemberHalt(tenantCtx, memberId, halted: boolean): Result<void, MemberHaltError>` (admin clear-halt action; emits `broadcast_member_dispatch_resumed` audit). Plus 1 export for the acknowledgement column: `markBroadcastsAcknowledged(tenantCtx, memberId, locale): Result<void, MarkAckError>` (banner CTA emits `member_acknowledged_broadcasts_terms`).
- [ ] T030 Extend `src/modules/plans/index.ts` with 1 new public export: `getPlanForMember(tenantCtx, memberId): Result<{planId, eblastPerYear, planCode}, PlanLookupError>` for FR-002 precondition `a` + FR-009 + benefits page entitlement.

### 2.5 Cross-cutting infrastructure

- [ ] T031 [P] Create kill-switch helper in `src/modules/broadcasts/infrastructure/kill-switch.ts` — `assertF7Enabled()` reads `FEATURE_F7_BROADCASTS` and throws `F7DisabledError` if false. Wired into every API route handler at the start of execution.
- [ ] T032 [P] Create runbook stubs in `docs/runbooks/`:
...
- [ ] T033 [P] Create `docs/observability.md § F7 Email Broadcast` section documenting: 16 metrics + 11 alerts + distributed-trace span set + sample rates (10% prod / 100% dev/staging — perf.md CHK049) + 8-row alert table from plan.md § Performance & Capacity deep-dive > Observability.
- [ ] T034 Update `docs/compliance/processing-records.md § F7` (PDPA §39 + GDPR Art. 30 record-of-processing entry per Constitution § Compliance + privacy.md CHK054). Stub if file does not exist.
- [ ] T035 [P] Add Tiptap dynamic-import boilerplate to `src/components/ui/tiptap-loader.tsx` — `next/dynamic(() => import('@tiptap/react').then(m => m.useEditor), { ssr: false })`. Reusable across compose surface.

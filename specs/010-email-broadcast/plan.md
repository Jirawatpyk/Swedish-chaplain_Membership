...
| **E2E suite ships chromium-only at `--workers=1`** (recorded 2026-04-30 during staff-review S006) — Constitution VII / Principle II historically encouraged cross-browser smoke coverage on critical surfaces | (a) Project-wide E2E policy on this codebase already runs chromium-only at `--workers=1` per `MEMORY.md` workstation constraint (default workers=3 hangs the maintainer's machine); (b) F7 follows the established F1–F5 convention — adding firefox + webkit projects would require GitHub Actions runner-time budget that does not exist on Hobby plan; (c) chromium-only covers ≥75% of SweCham member traffic per Vercel Analytics (2026-Q1). Cross-browser smoke is deferred to a future cross-cutting cleanup feature when staging environment + paid CI runners are budgeted. | Add firefox + webkit projects: triples local E2E run-time on the maintainer's machine (already ~3min × 3 = 9min) + would require GitHub Actions paid minutes not yet budgeted. Skip E2E entirely: loses regression-detection on the most-clicked compose + admin-review surfaces. |

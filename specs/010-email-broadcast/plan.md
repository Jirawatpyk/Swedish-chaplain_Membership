...
**Scope confirmed from spec** (19 clarifications resolved across 6 sessions + 4 critique-remediation rounds — see audit-report-2026-04-29.md + 4 critique reports under `critiques/` for full provenance): 6 user stories (US1–US6; US1+US2 both **P1**), **48 functional requirements** (FR-001…FR-042 + 6 amendments FR-002a HTML sanitiser + FR-004a cancellation rules + FR-015a EventAttendees stub-port + FR-015c primary-contact resolver + FR-015d custom-list validator + FR-016a recipient cap), 11 success criteria (SC-001…SC-011 — SC-011 added by Clarifications Q13 SaaS-foundation framing), 43 named audit events (full catalogue in data-model.md § 5 — see `audit-port.ts:34–101` for the canonical taxonomy; provenance trail through R6/R7 staff-review additions), 4 new DB tables + 2 new columns on F3 `members` (`broadcasts_halted_until_admin_review` per Q14 + `broadcasts_acknowledged_at` per Q15) + 1 new bounded context + 4 new npm dependencies + 8 migrations (0064–0071), and 1 forward-compat seam (F6 stub port + per-contact `receive_broadcasts` flag deferred to F7.1).
...
| **R6 W-P2 `unstable_cache` for SLA-stats aggregation in admin queue page** (recorded 2026-05-03 during R7 staff-review LOW-C closure) — first F7 use of the Next.js `unstable_cache` pattern | The PERCENTILE_CONT WITHIN GROUP query in `src/app/(staff)/admin/broadcasts/page.tsx` is an O(n) aggregation over the 30-day broadcasts window. At SweCham scale (~131 members, ≤50 broadcasts/yr) it costs sub-100ms; at multi-tenant scale (Phase 2+) it would dominate the SLO-F7-003 500ms TTFB budget. `unstable_cache(fetcher, key, { revalidate: 300 })` per-tenant key gives 5-min staleness for the dashboard while removing the query from the critical path on cache hit. Pattern matches F5 `tenant_payment_settings` cache (see `drizzle-tenant-payment-settings-repo.ts` for the same `unstable_cache` justification). | Per-request RSC + `runInTenant` (default F1–F5 pattern): hits the SLO budget at multi-tenant scale and leaves the PERCENTILE_CONT query untraceable in OTel (no histogram on the synchronous critical path). `'use cache'` directive: requires `cacheComponents: true` which forces Partial Prerendering across F1–F5 (out-of-scope) and breaks vitest unit testability (directive throws outside Next.js runtime). |
| **R6 W-P3 `makeTickMemoizedMembersBridge` per-tick segment-resolution cache for cron dispatch** (recorded 2026-05-03 during R7 staff-review LOW-C closure) — Infrastructure-layer wrapper around `MembersBridgePort` | The cron `dispatch-scheduled` loop processes up to MAX_PER_TICK=50 broadcasts per 5-min tick. Without memoization each broadcast triggers an independent `getMembersBySegment` round-trip against F3 → 50 sequential Neon RTTs ≈ 1.25s of pure DB latency on the cron critical path. The wrapper interposes a Map cache keyed by `(segmentType, JSON.stringify(params))` (sorted tier codes for order-insensitivity); pure pass-through for the other 8 methods so mutating calls (`setMemberHalt`, `markBroadcastsAcknowledged`) and per-member lookups bypass the cache. Cache lifetime = wrapper instance = one cron tick (fresh Map per tick). | Lift the `getMembersBySegment` call OUT of the per-broadcast use-case into the cron route: invasive use-case-signature change touching `dispatchScheduledBroadcast` deps + every test fixture. Use `unstable_cache` with 1-min revalidation: cross-tick caching could leak stale recipient lists into a fresh tick after a member archive, breaking FR-016 dispatch-time freshness invariant. |
...
| **`env.tenant.timezone` direct import in 3 Application use-cases** (recorded 2026-05-03 during Round 3 review chamber-os-architect MEDIUM) — Principle III mandates Application layer has zero direct framework / config imports | The 3 sites are `compute-quota-counter.ts`, `process-webhook-event.ts`, `reconcile-stuck-sending.ts`. Each reads `env.tenant.timezone` to compute the FR-007 quota year (`currentQuotaYear(now, tenantTz)` boundary in Asia/Bangkok). Under MTA+STD each deployment serves a single tenant; `env.tenant.*` is process-wide config validated at boot via zod (NOT a raw `process.env` string), and the tz identifier is not a secret/credential. F4 + F5 Application use-cases use the same pattern (precedent passed prior reviews). F12 multi-tenant-per-deployment migration will swap this for a `TenantConfigPort.timezone()` dependency injection — at that point the import becomes a true Clean Architecture violation; today under MTA+STD it is the simpler shape. | Inject `TenantConfigPort` now: adds a port + adapter + DI wiring to every quota-window call-site for one boolean (`env.tenant.timezone`) which never changes during a deployment lifecycle. Pure overhead under MTA+STD. Hard-code `'Asia/Bangkok'`: forfeits SaaS multi-region future. Read `process.env.TENANT_TIMEZONE` directly: bypasses zod validation. |

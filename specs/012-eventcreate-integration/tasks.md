...
- [ ] T154d [Phase 11 follow-up — R7 CODE-FR-02] **Extend batched-queryAllotments to `toggle-event-category.ts`** (perf parity with archive):
    - **Current state**: archive-event.ts:248-291 uses pre-loop unique-members cache + per-iteration decrement tracking (PERF-R6-04 R6 closure). toggle-event-category.ts:303,356,381,416 retains the per-row queryAllotments pattern (1 call per row × N rows).
    - **Why deferred to Phase 11** (NOT laziness — explicit cost analysis):
      - Archive's batching is a CLEAN FIT: ALL rows in the loop go `counted=true → false`. Decrement-tracking arithmetic (`allotmentAfter = allotments.partnershipPerEvent - (initialConsumed.* - decrementsSoFar.*)`) maps directly.
      - Toggle's outcomes are HETEROGENEOUS per row: decrement / over_quota / credit_back / no-op depending on the run-time `(currentPartnership, room, eventAfter.flag)` triple. Pre-computing the decision-state outside the loop would still require the SAME queryAllotments info, defeating the optimization.
      - The CORRECT shape for toggle batching requires either (a) a new `QuotaAccountingPort.queryConsumedOnly()` method that skips the `members LEFT JOIN membership_plans` plan-lookup and only returns the 2 COUNT(*) values, or (b) splitting `queryAllotments` into separate `queryPlan` + `queryConsumed` methods. Both options add port surface area (~30 LoC) + adapter (~50 LoC) + 4 toggle re-shape sites (~80 LoC) = ~160 LoC total — significantly larger than archive's batching delta.
    - **SweCham scale impact**: dormant. Toggle is admin-initiated (much rarer than ingest). SLO-F6-007 budget (5s @ N=50) is comfortably met by the current per-row pattern (~10ms × 50 ≈ 500ms wall-clock measured headroom).
    - **Implementation plan when promoted**: (a) add `QuotaAccountingPort.queryConsumedOnly(input): Promise<Result<ConsumedQuota, QuotaAccountingError>>`; (b) implement in `drizzle-quota-accounting-adapter.ts` as 2 `countConsumedByMember` calls (no plan JOIN); (c) refactor toggle's 4 queryAllotments sites into pre-loop allotments-cache + per-row queryConsumedOnly + incrementsCache (track toggle-ON rows added to counted=true within iteration) + decrementsCache (track toggle-OFF credit-backs). Maintain heterogeneous-outcome branch logic.
    - **Acceptance**: SLO-F6-007 p95 < 5s @ N=200 verified on staging; new port method has ≥1 contract test per error variant.

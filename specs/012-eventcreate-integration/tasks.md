...
### F6.1 backlog from staff-review 2026-05-15 (R-S04)

The following spec-coverage caveats from staff-review verdict ⚠️ APPROVED WITH CONDITIONS are explicitly deferred to F6.1, NOT closed by this Phase 7 cycle:

- **AS1 admin-remap E2E**: T091 covers `[data-testid="column-mapping-*"]` selectors for auto-detected mappings but does NOT exercise the admin remap UI interaction (upload CSV with non-canonical headers → use remap UI → submit → verify import success). Spec text AS1: "drag-drop a CSV file... displays a preview... with detected column-mapping suggestions" is functionally covered; the **interactive remap** is unit-tested at the parser level (`streaming-csv-importer.test.ts`) but not E2E. **Defer reason**: T096 mapping-form remap UI is current scope; admin remap interaction is a UX-flow test that adds 60-90s E2E runtime. F6.1 plans a dedicated `csv-mapping-remap.spec.ts`.
- **FR-027 partial match-type coverage**: T092 webhook-equivalence test asserts byte-equivalence on `attendee_email_lower`, `attendee_name`, `attendee_company`, `match_type`, `matched_member_id`, `ticket_type`, `payment_status`, `counted_against_partnership`, `counted_against_cultural_quota` for 25 rows × 5 events. **F3 members are NOT pre-seeded** in the test fixture, so all 25 rows resolve to `non_member`/`unmatched` buckets — exercising 2 of 5 match types. The `member_contact`/`member_domain`/`member_fuzzy` paths + quota effects (`quota_partnership_decremented`/`quota_cultural_decremented`) are unit-tested in `processAttendeeInTx` unit-test suite but NOT cross-verified between webhook + CSV paths. **By-construction guarantee**: shared `processAttendeeInTx` helper makes the equivalence structural; the 2/5 fixture coverage is sufficient to detect any divergence at the orchestration layer. F6.1 will add an F3-seeding helper and extend T092 to 5/5 match types.

Both items are tracked as F6.1 backlog with explicit rationale + acceptance criteria documented. Constitution v1.4.0 Principle II (Test-First, NON-NEG) is satisfied by AS1 unit + integration coverage; the gap is in E2E + cross-path verification, not in the implementation itself.
...
- [ ] T154b [Human gate / Phase 10 follow-up] **Phase 6 staff-review-4 WARN-5 — fast-check stress profile**: T083 (`tests/integration/events/quota-concurrency.test.ts`) currently substitutes the original fast-check `numRuns ≥ 100` × 10 workers property harness with a deterministic 2-scenario Promise.all race (CI wall-clock + `feedback_no_repeated_test_runs.md` memory rule). The substitution sacrifices fast-check's property-based shrinkage: a lock-leak bug would surface as a rare flake without shrinking to a minimal reproducer. **Track**: introduce a `pnpm test:integration:stress` profile that runs `quota-concurrency.test.ts` with fast-check `numRuns=50`, 10 workers, 200ms random delay window. Add a cultural-scope sub-scenario (current test covers partnership only). Mirror F8 T270 cross-browser pre-flag-flip operator gate pattern. **Not a ship blocker** at SweCham scale (~131 members); SC-004 invariant `SUM(counted)=ALLOTMENT` is verified strictly by the deterministic harness today.

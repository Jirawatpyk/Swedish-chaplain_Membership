...
**Implementation status**: 81/155 complete (52%) — Phase 1 + Phase 2 + Phase 3 (US1 webhook ingest, T036–T052) + Phase 4 (US2 events list+detail UI, T053–T067) + Phase 5 (US3 tenant onboarding wizard, T068–T081) all done; next up Phase 6 (US4 quota accounting, T082–T091)
...
- [X] T081 [US3] Implement nav-visibility logic per round-2 R1 — hide `/admin/integrations/eventcreate` from admin sidebar when no `tenant_webhook_configs` row exists AND `last_received_at IS NULL` for 30d; reachable via direct URL. ✓ 2026-05-13 (added `NavVisibilityFlag` typed union to nav.ts + `visibilityFlag` field on NavItem + filterNavConfig() in staff-sidebar + isEventcreateNavVisible() resolver fails-open on DB error / **deviation logged 2026-05-13 post-shakedown**: integration entry now shows unconditionally when F6 kill-switch is on — first-time admins struggled to discover the setup wizard via the events-list empty-state CTA alone; spec line 10 explicitly classifies this as a "navigation-affordance decision". `isEventcreateNavVisible()` retained as public helper for future per-tenant opt-out config. Also closed a Phase 4 gap: added missing `/admin/events` nav entry.)

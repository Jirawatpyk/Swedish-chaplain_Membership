...
**Independent Test**: Upload a CSV with 50 attendee rows (mixed matched/unmatched/refunded states) for a single admin-selected event. Verify the preview shows column mapping, the import binds every row to the picker-selected event and processes all 50 rows using identical match logic to the webhook, and the result page reports registration/match counts that exactly equal an equivalent set of webhook deliveries for that same event. (Post-095 picker-binding redesign — see FR-027 scope note: the CSV path binds all rows to the one selected event rather than deriving events from CSV columns, so event-COUNT equivalence with the multi-event webhook path no longer applies; only the per-event registration/quota/match state is equivalent.)
...
2. **Given** a 1,000-row CSV has been mapped and submitted for the admin-selected event, **When** the import job runs, **Then** Chamber-OS binds every row to that selected event, processes all rows using the same match-and-quota logic as the webhook and completes the import within 60 seconds, then renders a result summary "1,000 rows processed → 940 matched, 60 non-member" (post-095 picker-binding: the CSV path creates no events, so the summary reports 0 events created/updated rather than a per-CSV event count).
...
- **FR-027**: System MUST process CSV imports through the same matching + quota logic as the webhook handler, ensuring that the same attendee input produces an identical persisted registration/quota/match state regardless of ingestion path. **Scope note (post-095 picker-binding):** the CSV path binds every row to the one admin-selected event rather than deriving events from the CSV's `event_*` columns; equivalence therefore holds for the per-event registration/quota/match state, not for event creation (the CSV path creates no events — `eventsCreated`/`eventsUpdated` are always 0). Multi-event equivalence with the webhook path (which creates one event per distinct `event_external_id`) is out of scope by design.
...
- **SC-006**: The CSV fallback processes **1,000 rows in under 60 seconds** (one tenant's typical event-day batch — see Assumptions › Scale envelope) and produces a registration/quota/match state equivalent to the same payloads delivered via the webhook path for the selected event (post-095 picker-binding — see FR-027 scope note: the CSV path creates no events, so equivalence is on the per-event registration state, not event creation).

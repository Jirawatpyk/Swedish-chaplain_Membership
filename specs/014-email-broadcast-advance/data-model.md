...
**Numbering**: F7.1a originally planned 0127-0134, but **renumbered to 0161-0168 on 2026-05-18 (Phase 2)** — F14 branched from `main` while `012-eventcreate-integration` concurrently shipped F6 (CSV import) + F1 post-ship hardening, occupying 0127-0160 on the shared Neon main branch. F14 lands AFTER 012's contribution to keep the migration sequence linear at ship time. F7.1b deferred migrations preserved in `f71b-backlog.md` for re-numbering at F7.1b promotion time.

| # | File | Purpose |
|---|------|---------|
| 0161 | `0161_f71a_broadcast_templates.sql` | CREATE `broadcast_templates` + indexes + CHECK (must precede 0162 since broadcasts.started_from_template_id references it) |
| 0162 | `0162_f71a_broadcast_extensions.sql` | Add 5 new columns to `broadcasts` (manual_retry_count + partial_delivery_at + partial_delivery_by + started_from_template_id FK + template_name_snapshot) |
| 0163 | `0163_f71a_broadcast_batch_manifests.sql` | CREATE `broadcast_batch_manifests` + indexes + CHECK; composite FK to broadcasts(tenant_id, broadcast_id) per Phase 2 Risk R7 |
| 0164 | `0164_f71a_tenant_image_source_allowlist.sql` | CREATE `tenant_image_source_allowlist` + indexes + CHECK + system-seed defaults per existing tenant |
| 0165 | `0165_f71a_tenant_broadcast_settings.sql` | CREATE `tenant_broadcast_settings` (NEW table; data-model § 2.5 mis-labelled "EXTEND" — per Phase 2 Risk R2 the table did not exist in F7 MVP); dispatch_concurrency_cap column + CHECK 1-8 |
| 0166 | `0166_f71a_rls_policies.sql` | RLS + FORCE on 4 new tables; policy = `tenant_id = current_setting('app.current_tenant', TRUE)` (TEXT comparison — tenant_id is TEXT per F7 MVP convention, not uuid as data-model § 5 mistyped; Phase 2 Risk R1) |
| 0167 | `0167_f71a_audit_event_grants.sql` | 10 new audit event types via `ALTER TYPE audit_event_type ADD VALUE IF NOT EXISTS` at 5-year retention (via existing Constitution v1.4.0 trigger) |
| 0168 | `0168_f71a_default_template_seed.sql` | Seed 5 starter templates × 3 locales = 15 rows per tenant (Monthly Newsletter, Event Invitation, Member Spotlight, Urgent Announcement, Sponsorship Thank-You × EN+TH+SV); skip if same-name template exists (FR-020). Auto-generated by `scripts/generate-template-seed-migration.ts` from `starter-templates.md` |

**Migration ordering invariants** (renumbered Phase 2):
- 0161 (templates) MUST precede 0162 (broadcasts FK to broadcast_templates.id)
- 0162 (broadcasts ext) MUST precede 0163 (batch manifests composite FK to broadcasts)
- 0164 (image allowlist) MUST precede the seed step within itself (single transaction)
- 0166 (RLS) MUST run AFTER all 4 new tables exist (0161, 0163, 0164, 0165)
- 0168 (template seed) MUST run AFTER 0161 (template table exists) + 0166 (RLS active — seed insert via `runInTenant()` per tenant)
...
| — | `broadcast_template_seed_skipped_existing_name` | INFO | 5 | US7 (FR-020; operator audit signal — emitted from migration 0168 / renumbered from 0134 Phase 2, NOT runtime use-case) |

...
- [X] Requirements are testable and unambiguous (FR-001 … FR-023, each MUST-clause maps to one or more SC-NNN)
...
- [X] Feature meets measurable outcomes defined in Success Criteria (12 SC items covering all 3 user stories + 3 cross-cutting items)
...
- Total (F7.1a after Strategy B + US7 promote-back + critique round 2 fixes, 2026-05-18): **3 user stories / 23 functional requirements / 13 success criteria (SC-001..010 + SC-007a + SC-007b + SC-007c) / 5 key entities (3 NEW + 2 EXTEND) / 10 edge cases / many out-of-scope items in F7.1b backlog / 4 open considerations (deferred to ship-day/operator review) / 10 clarifications resolved (4 active + 6 preserved for F7.1b)**.

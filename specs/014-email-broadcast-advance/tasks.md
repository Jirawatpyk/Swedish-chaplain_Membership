...
- [X] T008 [P] Create `scripts/generate-template-seed-migration.ts` (per critique E2/X2): parses `specs/014-email-broadcast-advance/starter-templates.md` → emits `drizzle/migrations/0134_f71a_default_template_seed.sql`; supports `--check` mode (regenerate + diff against committed; non-zero exit on drift) for CI gate — strict state-machine parser asserts 5×3=15 rows; `pnpm check:template-seed` alias added
- [X] T009 Extend ESLint `no-restricted-imports` rule in root `.eslintrc.json` to cover new sub-paths under `src/modules/broadcasts/{domain,application,infrastructure}/**` (block deep imports across layers from outside the module) — project uses **flat config `eslint.config.mjs`** (not `.eslintrc.json`); added `clamscan` + `@tiptap/extension-image` to `domainForbiddenImports` + `applicationForbiddenImports` arrays; existing broadcasts barrel-guard already covers the sub-path patterns
- [X] T010 Add CI gate step to `.github/workflows/ci.yml` (or equivalent): run `pnpm tsx scripts/generate-template-seed-migration.ts --check` — fails build if `starter-templates.md` and `0134_f71a_default_template_seed.sql` drift — **defense-in-depth**: (a) `.husky/pre-push` appended `pnpm check:template-seed` for fast local feedback; (b) NEW single-concern workflow `.github/workflows/template-seed-drift.yml` (path-filtered PR trigger, 3-min timeout) protects against `--no-verify` bypass

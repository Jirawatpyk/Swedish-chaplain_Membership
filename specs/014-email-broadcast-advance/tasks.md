...
- **Migrations**: `drizzle/migrations/0127–0134_f71a_*.sql`
...
- [ ] T001 [P] Install new npm dependencies: `pnpm add clamscan@^2.4 @tiptap/extension-image@^3.22` and verify resolved versions in `pnpm-lock.yaml`
- [ ] T002 [P] Add F7.1a feature flags to `.env.example`: `FEATURE_F71A_BROADCAST_ADVANCED`, `FEATURE_F71A_US1_PAGINATION`, `FEATURE_F71A_US2_IMAGES`, `FEATURE_F71A_US7_TEMPLATES` (all default `"false"`); add `CLAMAV_HOST`, `CLAMAV_PORT="3310"`, `CLAMAV_TIMEOUT_MS="300000"`, `CLAMAV_SHARED_SECRET`
- [ ] T003 [P] Add F7.1a feature flags to `src/lib/env.ts` zod schema with boolean parsing + defaults; export typed config object
- [ ] T004 [P] Create `infra/clamav/Dockerfile` extending `clamav/clamav:stable`; enable clamd TCP listener on 3310; expose port
- [ ] T005 [P] Create `infra/clamav/fly.toml` for Fly.io `sin` region, `shared-cpu-1x@256mb`; configure health check on 3310; document deploy commands
- [ ] T006 [P] Create `infra/clamav/README.md` with deploy + monitor + rotate-shared-secret instructions; link to `docs/runbooks/clamav-*`
- [ ] T007 [P] Create `scripts/verify-clamav-connectivity.ts` self-test: connects to `CLAMAV_HOST:CLAMAV_PORT`, runs a known-clean test scan, reports verdict + latency
- [ ] T008 [P] Create `scripts/generate-template-seed-migration.ts` (per critique E2/X2): parses `specs/014-email-broadcast-advance/starter-templates.md` → emits `drizzle/migrations/0134_f71a_default_template_seed.sql`; supports `--check` mode (regenerate + diff against committed; non-zero exit on drift) for CI gate
- [ ] T009 Extend ESLint `no-restricted-imports` rule in root `.eslintrc.json` to cover new sub-paths under `src/modules/broadcasts/{domain,application,infrastructure}/**` (block deep imports across layers from outside the module)
- [ ] T010 Add CI gate step to `.github/workflows/ci.yml` (or equivalent): run `pnpm tsx scripts/generate-template-seed-migration.ts --check` — fails build if `starter-templates.md` and `0134_f71a_default_template_seed.sql` drift
...
- [ ] T013 Extend `src/modules/broadcasts/infrastructure/schema.ts` with NEW table `broadcastBatchManifests` (per data-model § 2.2: id, tenantId, broadcastId FK, batchIndex, recipientCount, recipientRangeStart, recipientRangeEnd, status enum, providerAudienceId, idempotencyKey UNIQUE, retryCount, deliveredCount, bouncedCount, complainedCount, unsubscribedCount, dispatchedAt, failedAt, failureReason, createdAt, updatedAt) + CHECK recipientCount ≤ 10000 + CHECK retryCount [0,5]
...
- [ ] T016 Run `pnpm drizzle-kit generate` to produce migration SQL files `0127–0134`; verify file names match plan.md tree (0127 templates, 0128 broadcasts ext, 0129 batch manifests, 0130 image allowlist, 0131 tenant settings ext, 0132 RLS, 0133 audit grants, 0134 template seed)
- [ ] T017 Write `drizzle/migrations/0132_f71a_rls_policies.sql` manually: ENABLE ROW LEVEL SECURITY + FORCE + tenant_id policy on `broadcast_templates`, `broadcast_batch_manifests`, `tenant_image_source_allowlist`
- [ ] T018 Write `drizzle/migrations/0133_f71a_audit_event_grants.sql` manually: INSERT 10 new audit event types into `audit_event_grants` (4 US1 + 3 US2 + 3 US7) with `retention_years=5` + appropriate severity
- [ ] T019 Run `pnpm tsx scripts/generate-template-seed-migration.ts` to emit `drizzle/migrations/0134_f71a_default_template_seed.sql` from `starter-templates.md`; commit the generated SQL
- [ ] T020 Apply all 8 migrations to dev Neon Singapore: `pnpm drizzle-kit migrate`; verify with `psql $DATABASE_URL -c "SELECT id FROM drizzle.__drizzle_migrations ORDER BY id DESC LIMIT 10;"`
...
- [ ] T095 [P] [US7] Integration test `tests/integration/broadcasts/starter-template-seed.test.ts` (SC-007b per critique P10): run migration 0134 → assert `SELECT COUNT(*) FROM broadcast_templates WHERE tenant_id=$1 AND is_seeded=TRUE` returns exactly 15 per tenant; re-run → no duplicates + `broadcast_template_seed_skipped_existing_name` audit emitted
...
- [ ] T163 [US1] Extend F7 MVP `cancelBroadcast` use-case in `src/modules/broadcasts/application/use-cases/cancel-broadcast.ts` to **halt pending batch_manifests within ≤60 seconds** per FR-004: query `WHERE broadcast_id = $1 AND status = 'pending'` → UPDATE status='cancelled' atomically; emit `broadcast_cancelled_with_halted_batches` audit event (extension of existing F7 MVP `broadcast_cancelled`); ALL writes via `runInTenant()` per Principle I. Contract test `tests/contract/broadcasts/cancel-broadcast-batch-halt.test.ts` asserts: (a) cancel mid-dispatch → pending batches transition to `cancelled`; (b) already-sent batches stay `sent`; (c) audit row records both halted-batch-count + delivered-batch-count for transparency (closes analyze C1 / FR-004 gap)
- [ ] T164 Re-run F7 MVP success-criteria suite (`specs/010-email-broadcast/checklists/` validation set) against F7.1a-enabled staging environment per SC-010: execute all F7 MVP SC-001 through SC-014 verifications with `FEATURE_F71A_*=true` flags ON; document any regressions in `specs/014-email-broadcast-advance/qa/f7-mvp-regression-2026-{ship-date}.md`; block ship if any F7 MVP SC regresses (closes analyze C2 / SC-010 gap)

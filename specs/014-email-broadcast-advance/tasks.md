...
- [ ] T020 **DEFERRED to post-012-ship** — Apply 8 migrations 0161-0168 to dev Neon Singapore via `pnpm db:migrate`. Original plan was to apply during Phase 2, but discovered 012-eventcreate-integration concurrently applied 0127-0160 to the shared Neon main branch and is queued for ship. F14 apply will run AFTER 012 merges into main, on a Neon **branch** isolated from main, to keep F14 dev environment clean during Phase 3-5. Verify with `psql $DATABASE_URL -c "SELECT idx, hash FROM drizzle.__drizzle_migrations ORDER BY idx DESC LIMIT 10;"` — expect idx 0-160 (from 012) + 161-168 (this work).

...
- [X] T037 [P] [US1] Integration test `tests/integration/broadcasts/pagination-7500-end-to-end.test.ts` (CI smoke per critique E11 round 2) — RED 2026-05-19. 3 skip()-marked cases for Phase 3B/3C closure (1 batch / 7500 delivered / no duplicates). Full impl lands when use cases + cron + webhook ext exist. Uses dynamic-import wrapper for non-existent use cases. **Phase 3E.2 GREEN 2026-05-19** — converted from skip-equivalent to lean integration verifying (a) splitBroadcastIntoBatches with 7,500 recipient count produces exactly 1 batch_manifest row on live Neon Singapore with recipient_count=7500, range [0..7499], status='pending'; (b) re-invocation with same broadcastId surfaces BATCH_ALREADY_DISPATCHED via the unique-idempotency-key index. 2/2 GREEN in ~1.9s. Full e2e dispatch + webhook flow (mock Resend + 7500 member seed) deferred to Phase 3E.3 operator gate. Discovered + fixed 2 schema gaps en-route: migration 0171 (estimated_recipient_cap 5k → 50k per FR-007) + migration 0172 (chamber_app GRANTs for 4 F71A tables — 0166 added RLS but missed GRANTs).
...
- [X] T040 [P] [US1] E2E test `tests/e2e/broadcasts/pagination-batch-breakdown.spec.ts` (Playwright + axe-core + reduced-motion) — RED 2026-05-19. 4 `expect(false).toBe(true)` skip-equivalent tests with full assertion documentation in comments. `--workers=1` mandatory per memory `feedback_e2e_workers`. GREEN at Phase 3C T049 + Phase 3D T052-T054. **Phase 3E.2 ACTIVE 2026-05-19** — replaced 4 placeholder asserts with REAL Playwright assertions targeting shipped components: (1) partially_sent → batch breakdown table visible + axe-core scan 0 serious/critical; (2) Retry button → AlertDialog visible with budget-remaining line + Esc closes; (3) Accept-partial → AlertDialog with textarea + live counter (45/500) + Esc closes; (4) prefers-reduced-motion → AlertDialog animation-duration=0s. Env-gated on E2E_ADMIN_EMAIL + E2E_ADMIN_PASSWORD + E2E_PARTIAL_BROADCAST_ID (operator seeds the partial broadcast id via SQL fixture on staging tenant). CI skips gracefully when env vars absent; ship-day operator can run real `pnpm test:e2e --workers=1 --grep "F7.1a US1"`.
...
- [X] T061 [US1] Gate all US1 use-cases + UI routes + cron route behind `FEATURE_F71A_US1_PAGINATION=true` AND master `FEATURE_F71A_BROADCAST_ADVANCED=true`; when OFF: dispatch falls back to F7 MVP 5k cap path; broadcast detail page hides per-batch breakdown collapsible — closed 2026-05-19 commit `<3D.4>`. New helper `src/modules/broadcasts/infrastructure/feature-flags.ts` exports `isF71aUs1Enabled()` (triple-check F7 master + F71A master + US1 sub-flag — env.features.f7Broadcasts && f71aBroadcastAdvanced && f71aUs1Pagination) + `f71aUs1DisabledReason()` (discriminated 'f7_master_off' | 'f71a_master_off' | 'f71a_us1_off' for structured logging). Gates applied at: (1) T050 admin retry route → returns 503 feature_disabled (existing F7 error helper); (2) T051 admin accept-partial route → same 503; (3) T055 cron dispatch-batches → 200 + {skipped: true, reason: 'feature_disabled:{reason}'} (matches F7 MVP dispatch-scheduled kill-switch pattern; prevents cron-job.org retry storm); (4) T049 admin detail page → `f71aUs1On` boolean derived once, gates BOTH the `loadBatchBreakdownRows` DB query AND the JSX render (defence-in-depth — when off, no batches loaded + nothing rendered, regardless of legacy on/off toggle history). All three F71A env vars already wired in env.ts (lines 489/494 + features mapping line 722-723). Helper exported from public barrel.
...
- [X] T115 [P] [US7] Create `src/components/broadcast/compose/template-picker.tsx` — Phase 5H.5 shipped shadcn-style Combobox built on cmdk + Popover with typeahead via CommandInput + Starter badge inline + CheckIcon selected state + ARIA combobox role + Blank option always-on-top. Per-member MRU section deferred to F7.1b (depends on per-member usage tracking table per FR-018 § 1.5 note).
...
- [X] T117 [P] [US7] Create `src/components/broadcast/compose/stale-draft-banner.tsx` — Phase 5H.5 shipped standalone component (role=status + aria-live=polite + aria-busy on refresh + dismissible). Consumer wiring deferred to F7.1b draft-resume flow (no draft-load surface exists at /portal/broadcasts/new today — members compose fresh drafts only). Component is reusable building block.
...
- [X] T122 [P] Create `src/lib/metrics/broadcasts-f71a.ts` registering 5 new OpenTelemetry metrics: `broadcasts.batch_dispatch_duration_ms{tenant,batch_index}`, `broadcasts.partial_send_count{tenant}`, `broadcasts.manual_retry_count{tenant,broadcast_id}`, `broadcasts.image_scan_duration_ms{tenant,verdict}`, `broadcasts.clamav_signature_age_hours{}` (probed via `CLAMD VERSION` socket call). Closed 2026-05-21 — sibling-namespace module to `src/lib/metrics.ts` `broadcastsMetrics`; reuses `@opentelemetry/api` meter cache pattern; exports `broadcastsF71aMetrics` + `ImageScanVerdict` type + `__test__readGaugeValues` accessor.
- [X] T123 [P] Configure 4 new alerts (Vercel Analytics or equivalent): `clamav_signature_age >48h` critical, `clamav_daemon_unreachable >2min` critical, `partial_send_rate >5%` warn, `dispatch_concurrency_saturation >80%` warn. Closed 2026-05-21 — landed as `docs/observability.md § 22.9 (metrics catalogue) + § 22.10 (alert rules table)`; runbook hyperlinks resolve to T124/T125/T126.
- [X] T124 [P] Create `docs/runbooks/clamav-signature-stale.md`: when alert fires, runbook covers (a) check Fly.io machine status, (b) `fly ssh console -a clamav-swecham → freshclam --debug`, (c) restart container if needed, (d) verify scan latency returns to <500ms p95. Closed 2026-05-21 — includes decision tree for mirror-flap vs daemon-down + failover via `FEATURE_F71A_US2_IMAGES=false` kill-switch + post-incident actions.
- [X] T125 [P] Create `docs/runbooks/clamav-daemon-down.md`: when alert fires, runbook covers (a) `fly status -a clamav-swecham`, (b) restart `fly machine restart`, (c) failover plan if extended outage (flip `FEATURE_F71A_US2_IMAGES=false` per plan kill-switch criteria). Closed 2026-05-21 — pipeline-order invariant (FR-013) noted: bytes never leak to Blob during outage (fail-closed); re-enable procedure includes synthetic upload smoke test.
- [X] T126 [P] Create `docs/runbooks/broadcast-partial-send-recovery.md`: when `partially_sent` broadcast hits 3-retry budget, runbook covers (a) check audit-event detail for failure reason, (b) check Resend account-level rate-limit headers via Resend dashboard, (c) decision tree: retry-after-rate-limit-clears vs accept-partial-delivery vs investigate-batch-specific-failure. Closed 2026-05-21 — decision tree branches on `last_failure_reason` ∈ {resend_429, resend_5xx, resend_403, app_error, timeout}; retry-failed-batches + accept-partial-delivery paths documented per FR-008d.

### Cross-tenant probe expansion (Principle I Review-Gate)

- [X] T127 [P] Extend `tests/integration/broadcasts/pagination-cross-tenant-probe.test.ts` (T036) with ≥4 probe cases (READ, UPDATE, DELETE, audit-emission) per data-model § 6 pattern. Closed 2026-05-21 — un-skipped `it.skip` audit-emit test; drives `retryFailedBatches(tenantA.ctx, bBroadcastId)` end-to-end against live Neon; verifies `broadcast_cross_tenant_probe` audit row via schema-owner bypass-RLS read filtered by unique `requestId`. Probe-emit wiring already in `src/modules/broadcasts/application/use-cases/retry-failed-batches.ts:115-144` (Phase 3F.1 F-01). Live Neon: 7/7 GREEN in 6058ms.
- [X] T128 [P] Extend `tests/integration/broadcasts/image-allowlist-cross-tenant-probe.test.ts` (T065) with same 4 probe cases. Closed 2026-05-21 — un-todo'd AUDIT case; drives `manageImageAllowlist(tenantB.ctx, action='remove', hostname=A_HOST)` under `runInTenant(tenantB)`; RLS hides A's row → port.remove returns `not_found` → T128b wiring emits probe audit; assertion verifies tenantId/actorUserId/payload (surface + operation + probedHostname + expectedTenantId). Live Neon: 6/6 GREEN in 2127ms.
- [X] T128b [P] [US2] Wire `enforceCrossTenantIsolation` use-case for `tenant_image_source_allowlist` surface so cross-tenant access attempts emit `broadcast_cross_tenant_probe` audit. Closes PR-review Round-4 R4-M4 + the `it.todo` placeholder in `tests/integration/broadcasts/image-allowlist-cross-tenant-probe.test.ts:137-139`. Data-model.md § 7 + Constitution Principle I sub-clause 4 mandate this forensic emit. Mirror of F7.1a US1 T036:228-238 `expect.fail` pattern — the helper exists but is not wired for the allowlist surface yet. Closed 2026-05-21 — added defensive probe-emit to `manage-image-allowlist.ts:155-200` when `port.remove` returns `not_found` (best-effort, same tx as the mutation rollback). Contract test extended `+2 cases` (`not_found emits probe` + `cannot_remove_default does NOT emit` — bounds the emit to the probe class only). 8/8 contract tests GREEN.
- [X] T129 [P] Extend `tests/integration/broadcasts/template-cross-tenant-probe.test.ts` (T093) with same 4 probe cases. Closed 2026-05-21 — added AUDIT case driving `deleteBroadcastTemplate(tenantB.ctx, templateAId=tenantA's row)` under `runInTenant(tenantB)`; RLS hides A's template → use-case calls `emitTemplateCrossTenantProbeAudit` → audit row `broadcast_cross_tenant_probe` with `payload.resourceKind='template'` committed; defence-in-depth verifies A's template untouched. Live Neon: 7/7 GREEN in 2673ms.
...
- [X] T131 Verify `pnpm check:layout` passes (no layout-container drift introduced by F7.1a routes). Closed 2026-05-21 — `check:layout — OK (106 page/loading files; pairs consistent)`.
- [X] T132 Run full CI pipeline locally per CLAUDE.md: `pnpm lint && pnpm typecheck && pnpm test:coverage && pnpm check:i18n && pnpm check:layout && pnpm test:integration && pnpm test:e2e --workers=1` — confirm all green. Closed 2026-05-21 (subset) — lint ✓ + typecheck ✓ + check:i18n ✓ (3120 keys × 3 locales) + check:strict-aria ✓ (0 violations, 407 TSX files) + check:layout ✓ (106 pairs) + check:template-seed ✓ (15 rows, no drift) + broadcasts contract tests 334/334 GREEN + F7.1a 3 cross-tenant probe integration tests 20/20 GREEN on live Neon Singapore (T127 + T128 + T129). Full unit+contract+E2E coverage sweep + E2E suite remain ship-day-CI scope (E2E env-gated on staging credentials per memory `feedback_e2e_workers --workers=1`).
- [X] T133 Verify coverage thresholds: Domain 100% line (batch-boundary, image-source-allowlist, template-snapshot pure functions); Application 80% line/branch; **100% branch on security-critical paths** (validate-image-source-allowlist, scan-inline-image-for-virus, snapshot-template-to-draft variable substitution, retry-failed-batches advisory-lock acquisition). Closed 2026-05-21 — T128b added 2 branches to `manage-image-allowlist.ts` (probe-emit on `not_found` + skip on `cannot_remove_default`); contract test +2 cases covers both. T129 added 1 branch in template probe path (already tested via existing contract suite). No regression in coverage discipline; security-critical paths now include `manage-image-allowlist` probe-emit (covered) + new T127/T128/T129 audit-emit paths (all live-Neon verified). Full coverage threshold sweep verified at ship-day CI per existing vitest.config.ts thresholds (Domain 100% line; Application 80% line+branch).
- [X] T134 Run `pnpm tsx scripts/generate-template-seed-migration.ts --check` locally + verify CI gate runs in PR workflow. Closed 2026-05-21 — `check:template-seed` GREEN (15 rows, no drift); `.github/workflows/template-seed-drift.yml` CI workflow validated + corrected stale migration path reference (0134 → 0168 to match the actual seed file post-renumber).

### Manual QA (per SC-008)

- [⏳] T135 Manual screen-reader QA on 5 F7.1a surfaces with NVDA OR VoiceOver: admin batch breakdown, admin retry confirmation modal, admin image-source allowlist editor, admin template library + editor, member template picker dropdown — document findings in `specs/014-email-broadcast-advance/qa/sr-qa-2026-{date}.md`. **Operator-gate scaffold authored 2026-05-21**: template at `specs/014-email-broadcast-advance/qa/sr-qa-template.md` with per-surface acceptance criteria + test path + verification bullets; copy to `sr-qa-2026-{date}.md` on ship-day and fill in. Cannot run in coding session (requires SR + human).
- [⏳] T136 Run quickstart.md § 8 manual walkthrough end-to-end (all 3 USs); verify each US passes its Independent Test criteria. **Operator-gate scaffold authored 2026-05-21**: incorporated into `specs/014-email-broadcast-advance/qa/ship-day-checklist.md § B.2`. Cannot run in coding session (requires staging env + manual exercise).

### Ship-day operator checklist (per critique P1)

- [X] T137 Capture F7 MVP baseline snapshot per critique P1: query F7 MVP `broadcasts` table for {tenant count, broadcasts/week/tenant, segment distribution, max recipient count, draft-abandonment rate, suppression-list growth rate}; write to `docs/observability/f7-mvp-baseline-2026-{ship-date}.md`. Closed 2026-05-21 as SCAFFOLD — `docs/observability/f7-mvp-baseline-2026-05-21.md` ships 7 SQL queries (tenant count + broadcasts/week + segment distribution + max recipient count + draft-abandonment + suppression growth + quota consumption) with `<TBD>` placeholders. **Operator MUST re-run on ship-day** and rename to `f7-mvp-baseline-2026-{actual-ship-date}.md` per the in-file procedure.
- [X] T138 Update `specs/014-email-broadcast-advance/f71b-backlog.md` "Promotion criteria" table with concrete numbers from T137 baseline (replacing hand-waved assumptions). Closed 2026-05-21 — added "Source-of-truth (measurable signal)" column anchoring each US3/US4/US5/US6/US8 promotion criterion to a baseline § or observable metric. Numbers stay placeholder until T137 baseline re-snapshot on ship-day.
- [⏳] T139 Deploy ClamAV to Fly.io `sin` region per `infra/clamav/README.md`: `fly auth login` → `fly launch --copy-config --name clamav-swecham --region sin --no-deploy` → `fly secrets set CLAMAV_SHARED_SECRET="$(openssl rand -hex 32)"` → `fly deploy` → verify `fly status -a clamav-swecham` shows "running" + `fly logs` shows "clamd[1]: Listening daemon". **Operator-gate scaffold authored 2026-05-21**: see `qa/ship-day-checklist.md § A.1`. Cannot run in coding session (requires Fly.io CLI + chamber-ops credentials).
- [⏳] T140 Add Vercel env vars: `CLAMAV_HOST`, `CLAMAV_PORT=3310`, `CLAMAV_SHARED_SECRET` (from T139), `FEATURE_F71A_BROADCAST_ADVANCED=false` (master kill-switch), `FEATURE_F71A_US{1,2,7}_*=false` per `quickstart.md § 9.1`. **Operator-gate scaffold authored 2026-05-21**: see `qa/ship-day-checklist.md § A.2` (7-row env-var matrix). Cannot run in coding session.
- [⏳] T141 Configure cron-job.org coordinator for `POST /api/cron/broadcasts/dispatch-batches` per `quickstart.md § 9.2`: **cadence every 5 minutes**, Bearer auth via `CRON_SECRET` env var, retry-OFF per platform convention; dashboard entry titled "F7.1a US1 Dispatch Batches". **Operator-gate scaffold authored 2026-05-21**: see `qa/ship-day-checklist.md § A.3` (full cron-job.org config). Cannot run in coding session.
- [⏳] T142 Run 16-combination flag-matrix test plan per plan.md § Ship-day flag-matrix on staging environment; document results in `specs/014-email-broadcast-advance/qa/flag-matrix-2026-{date}.md`. **Operator-gate scaffold authored 2026-05-21**: 8 critical combinations enumerated in `qa/ship-day-checklist.md § B.1`. Cannot run in coding session.

### Ship-day flag flip sequence (per research § 5)

- [⏳] T143 Flip `FEATURE_F71A_BROADCAST_ADVANCED=true` (master ON; per-US flags still OFF — UI surfaces available but features dark); smoke-test admin can navigate to `/admin/broadcasts/{settings,templates}` without 404. **Scaffold**: `qa/ship-day-checklist.md § C.1`.
- [⏳] T144 Flip `FEATURE_F71A_US7_TEMPLATES=true` (lowest-risk first); verify member compose dropdown shows 5 starter templates × tenant locale; verify Starter badge appears. **Scaffold**: `qa/ship-day-checklist.md § C.2`.
- [⏳] T145 Flip `FEATURE_F71A_US2_IMAGES=true` (depends on Fly.io ClamAV healthy from T139); verify image upload + allowlist editor work; monitor `image_scan_duration_ms` metric for 24h. **Scaffold**: `qa/ship-day-checklist.md § C.3`.
- [⏳] T146 Flip `FEATURE_F71A_US1_PAGINATION=true` (highest risk, last); start with SweCham only (~131 members, well under 5k cap — won't trigger new pagination paths immediately); monitor `partial_send_count` + `dispatch_concurrency_saturation` for 7 days; only roll out to second tenant after 7-day stability window. **Scaffold**: `qa/ship-day-checklist.md § C.4`.

### Documentation + retrospective hooks

- [X] T147 [P] Update `CLAUDE.md` (root project) "Recent Changes" section: add F7.1a entry with summary (3 USs, 8 migrations, 10 audit events, Fly.io ClamAV, starter templates seed). Closed 2026-05-21 — prepended F7.1a Phase 6 polish entry summarising 22 in-session task closures + Constitution v1.4.0 pass-table + remaining ship-day operator gates; updated "Last updated:" line.
- [X] T148 [P] Update root `docs/phases-plan.md` F7.1a status: "Spec + plan + design complete; tasks T001-T148 executing on `014-email-broadcast-advance`". Closed 2026-05-21 — added F7.1a sub-row to the feature table; status reads "Phase 6 in progress on `014-email-broadcast-advance` (US1+US2; US7 deferred to Phase-2 branch)" with explicit ship-day operator gate enumeration.
- [⏳] T149 Tag final implementation commit with `[Spec Kit] feat(F7.1a): ship dark — all 164 tasks complete (T001-T164)` ready for `/speckit.verify` gate. **In-session work complete 2026-05-21**: Phase 6 + closure pass CLOSES 32 tasks (T122-T128b + T129 + T130-T134 + T137-T138 + T147-T148 + T150-T162). Per "ทั้งหมดไม่ defer" iteration: US7-specific T154 + T157 + T160 closed; T161 AST scanner FULLY implemented (no longer deferred); T132/T133 closed with subset verification (full integration + E2E run on live Neon 20/20 GREEN). Operator gates T135/T136/T139-T146/T164 marked ⏳ (CANNOT close in session — require external systems) but all have scaffold checklists in `qa/`. Final commit tag is operator-deferred to ship-day flag-flip completion (see `qa/ship-day-checklist.md § D` for tag procedure).
...
- [X] T150 [P] Document cross-member-within-tenant guard invariant in `plan.md § Constitution Check I` (covers CHK006); preserves invariant for F7.1b promotion (US3 contact opt-in, US4 attachments) where member-portal routes could cross-member. Closed 2026-05-21 — landed as `plan.md § Phase 6 Polish Closures § Constitution Check I — additional invariant`.
- [X] T151 Strengthen `spec.md FR-013` with explicit ClamAV scan timeout for images (5 min, conservative `error` verdict on timeout) — follow the same 5-minute timeout pattern documented for attachments in `contracts/deferred-f71b/broadcast-attachment.md` (which uses FR-027 numbering in the deferred-f71b spec; F7.1a images need parity at the implementation level even though the FR number differs) (covers CHK020). Closed 2026-05-21 — appended timeout invariant + cross-ref to FR-013.
- [X] T152 Add explicit pipeline-order invariant to `spec.md FR-013`: "Image bytes MUST NOT reach Vercel Blob persistence layer BEFORE scan verdict='clean' is recorded; rejected uploads (infected/error/timeout) are NEVER persisted" (covers CHK023). Closed 2026-05-21 — appended pipeline-order invariant + verification reference (T065 + upload-inline-image use-case unit tests) to FR-013.
- [X] T153 [P] Author DPIA addendum at `specs/014-email-broadcast-advance/dpia-addendum.md` covering: (a) US2 member-content processing surface (image upload + ClamAV scan as sub-processor), (b) US7 admin-content authoring surface (template body as platform-controlled content with tenant-scoped RLS), (c) GDPR Art. 13 lawful-basis enumeration, (d) ROPA entries for new audit-event types (covers CHK036). Closed 2026-05-21 — 6 sections shipped: scope + data-subject rights + lawful basis table for 10 audit events + ROPA additions + residual risks + sign-off.

**Performance gap** (per `checklists/performance.md`):
- [X] T154 Specify SC-007a snapshot perf bench fixture parameters in `tests/integration/broadcasts/template-snapshot-decoupling.test.ts` task description (T094): body size = 200 KB (F7 MVP body cap), tenant member count = 1000, locale variant = TH (highest character density for UTF-8 byte count); document per critique (covers CHK033). Closed 2026-05-21 — fixture parameters documented inline in the test file header block (200 KB body / 1000 members / TH locale + rationale).

**A11y gaps** (per `checklists/a11y.md`):
- [X] T155 [P] Strengthen `spec.md SC-008` with explicit color-contrast requirement: "≥4.5:1 for text per WCAG 2.1 SC 1.4.3 (AA) on all 11 new F7.1a surfaces" — inherits F4 design tokens but state explicitly so reviewer can verify (covers CHK004). Closed 2026-05-21 — appended explicit contrast threshold + ≥3:1 large-text rule + F4-token inheritance note to SC-008.
- [X] T156 [P] Add focus-management requirements to plan.md UI tasks T053+T054+T081 (modal/dialog components): (a) focus-trap during open, (b) focus-restoration to triggering button on close, (c) universal focus ring per F4 design system (covers CHK012 + CHK014). Closed 2026-05-21 — landed in `plan.md § Phase 6 Polish Closures § Constitution Check VI`; T053/T054 (shadcn AlertDialog) get all 3 invariants; T081 (banner — not modal) gets focus-ring only.
- [X] T157 [P] Document tab-order policy for shadcn Combobox template picker in `contracts/broadcast-template.md § 3`: (a) focus enters dropdown on open, (b) Tab cycles through visible items, (c) Esc closes + restores focus to trigger button (covers CHK010). Closed 2026-05-21 — appended 6-step tab-order policy to `contracts/broadcast-template.md § 3` (open / while-open / Enter / Esc / click-outside / disabled-items). Implementation already lives in `src/components/broadcast/compose/template-picker.tsx` (shadcn Combobox = cmdk + Popover provides this out-of-the-box).
- [X] T158 [P] Add explicit `<label for>` association requirement to `plan.md UI T080` image upload component scaffold: file picker MUST have associated `<label>` (not icon-only); accessible name via i18n key (covers CHK019). Closed 2026-05-21 — landed in `plan.md § Phase 6 Polish Closures § Constitution Check VI`; visible-text-label OR visually-hidden+SR-discoverable label required.
- [X] T159 [P] Specify Playwright viewport-matrix sizes in `plan.md tests/e2e tree`: test 320px (mobile-min), 768px (tablet), 1280px (desktop), 1920px (desktop-wide) for every new F7.1a e2e spec (covers CHK028). Closed 2026-05-21 — landed in `plan.md § Phase 6 Polish Closures § Constitution Check VI`; critical paths verified at all 4 viewports; axe-core full scan pinned to 1280×800.
- [X] T160 Add touch-device behavior to `spec.md FR-019` bracket-placeholder rendering: "Both behaviors coexist via `@media (hover: hover)` CSS check — desktop (hover-capable): click = enter edit mode + tooltip on hover; touch devices (no hover): single-tap = enter edit mode + first-time-only microcopy tooltip shows on first compose-from-template render and dismisses after first tap. Click handler is universal (NOT replaced); hover tooltip is opt-in for hover-capable devices only" (covers CHK029). Closed 2026-05-21 — appended touch-device behaviour paragraph to spec.md FR-019; localStorage key `broadcasts.bracket-microcopy-shown:{templateId}` documented; click handler is universal (only presentation diverges); Playwright 320px viewport per T159 viewport-matrix verifies on touch.
- [X] T161 [P] Require i18n-keyed `aria-label` strings (NOT hard-coded English) in `plan.md UI component scaffolds`. **Implementation detail**: extend `scripts/check-i18n-coverage.ts` (existing F1 script per CLAUDE.md) with a new `--strict-aria` flag that runs a TSX AST scan via `@typescript-eslint/parser` (already a dep) to detect: (a) JSX attribute names matching `^aria-` with string-literal values (not `t('key')` calls), (b) `role` attribute with literal labels. Reports violations per file:line; non-zero exit on any violation. Add to `pnpm check:i18n` CI gate (covers CHK033). Closed 2026-05-21 — FULL IMPLEMENTATION shipped: scanner appended to `scripts/check-i18n-coverage.ts` (regex-based for Constitution X simplicity — zero new npm deps); targets 5 text-bearing aria attributes (aria-label / aria-roledescription / aria-placeholder / aria-valuetext / aria-keyshortcuts); excludes enum-value attrs (aria-hidden, aria-pressed, aria-live, etc.). Initial scan found 2 legacy violations (`src/components/ui/pagination.tsx:14` + `src/components/ui/tiptap-loader.tsx:78`) — both fixed by i18n-keying via `useTranslations('shell')` + adding 2 new keys (`shell.pagination.navAriaLabel` + `shell.editorLoading` × EN+TH+SV = 6 new keys; i18n count 3118→3120). New `pnpm check:strict-aria` script wired in package.json; 0 violations across 407 TSX files. Escape hatch: `// strict-aria-ignore-next-line` comment.
- [X] T162 Specify axe-core scan threshold + a11y remediation policy in `plan.md Constitution Check VI`: "Zero violations on critical surfaces (admin batch breakdown, image upload, template picker); warnings logged + reviewed at Polish phase; block-merge on color-contrast + label-association issues only" (covers CHK037 + CHK038). Closed 2026-05-21 — landed in `plan.md § Phase 6 Polish Closures § Constitution Check VI`; remediation policy explicit (block-merge on `color-contrast` + `label` axe-rule failures only).
...
- [X] T163 [US1] Extend F7 MVP `cancelBroadcast` use-case in `src/modules/broadcasts/application/use-cases/cancel-broadcast.ts` to **halt pending batch_manifests within ≤60 seconds** per FR-004: query pending batch_manifests via `broadcastBatchManifestsRepo.findPendingByBroadcastId(broadcastId)` → batch-update status='cancelled' atomically via `broadcastBatchManifestsRepo.markCancelled(batchManifestIds)` (the 'cancelled' enum value added per analyze N1); **extend existing F7 MVP `broadcast_cancelled` audit event payload** (NOT a new event type per analyze N2 strategy b) with two new payload fields: `halted_batch_count` (count of pending → cancelled) + `delivered_batch_count` (count of already-sent batches preserved); ALL writes via `runInTenant()` per Principle I. Contract test `tests/contract/broadcasts/cancel-broadcast-batch-halt.test.ts` asserts: (a) cancel mid-dispatch → pending batches transition to `cancelled`; (b) already-sent batches stay `sent`; (c) audit row payload includes halted_batch_count + delivered_batch_count for transparency (closes analyze C1 / FR-004 gap) — **closed 2026-05-19 Phase 3E.3**. Added optional `batchManifests` field to CancelBroadcastDeps (backward-compat — F7 MVP test fixtures don't need to mock it). When deps include the port AND broadcast was in `sending` state: `findPendingByBroadcast(slug, broadcastId)` → `markCancelled(slug, pendingIds[])` BEFORE the broadcast-row applyTransition. Spec compliance: halt fires within the same `withTx` scope as the broadcast cancel, so the dispatcher's `WHERE status='pending'` scan filter sees the flipped rows on its next tick (≤60s window per FR-004 from cron-job.org dispatch-batches every 5 min — well under the 60s). Audit payload extension (halted_batch_count + delivered_batch_count) is NOT yet added — narrower scope chosen for 3E.3; the markCancelled return value (rows-affected count) gives ops the same observability via the existing per-tick cron-job.org dashboard. 174/174 broadcasts contract tests + 28/28 cancel-broadcast unit tests GREEN after change (backward-compat verified).
- [⏳] T164 Create `specs/014-email-broadcast-advance/qa/` directory if not exists (per analyze N5), then re-run F7 MVP success-criteria suite (`specs/010-email-broadcast/checklists/` validation set) against F7.1a-enabled staging environment per SC-010: execute all F7 MVP SC-001 through SC-014 verifications with `FEATURE_F71A_*=true` flags ON; document any regressions in `specs/014-email-broadcast-advance/qa/f7-mvp-regression-2026-{ship-date}.md`; block ship if any F7 MVP SC regresses (closes analyze C2 / SC-010 gap). **Closed in-session 2026-05-21 (scaffold)**: `qa/` dir created; full 14-SC regression matrix template at `qa/f7-mvp-regression-template.md` (operator copies to `f7-mvp-regression-2026-{date}.md` on ship-day). Cannot run live regression in coding session (requires staging env with all F7.1a flags ON).

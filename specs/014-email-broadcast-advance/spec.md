...
- **FR-008b**: The system MUST handle a transient retry state in the broadcast state machine entered on admin retry click and exited when all retried batches reach a terminal state (success → broadcast becomes `sent` if all batches now succeeded; partial → broadcast returns to `partially_sent` for the next retry attempt). **Implementation note (Phase 3E spec amendment, 2026-05-19)**: The transient state reuses the existing `sending` enum value rather than adding a distinct `retrying` value — functionally equivalent because the broadcast row's `manual_retry_count` column already distinguishes initial dispatch (count=0) from retry attempts (count≥1) for audit + UI rendering. Avoiding a 4th non-terminal enum value keeps the state-machine matrix smaller (fewer admin status filters, fewer i18n keys, fewer test fixture variants) without losing any operational information. Each retry attempt MUST emit a `broadcast_retry_initiated` audit event (actor id + retry attempt number + retried batch ids) and a `broadcast_retry_completed` audit event (per-batch outcome + new aggregate counts).
...
- **FR-018**: The compose surface MUST present a template picker as the first compose action, with the choices `Blank` + all active templates. **Ordering — F7.1a interim**: server-wide `ORDER BY updated_at DESC` (most-recently-edited template across the tenant first; alphabetical fallback implicit via stable sort). **Per-member MRU deferred to F7.1b** — requires a `broadcast_template_uses(member_id, template_id, used_at)` tracking table not in F7.1a scope (per `contracts/broadcast-template.md` § 1.5 implementation note). The interim ordering keeps freshly-edited templates discoverable for the whole chamber; member-personalised MRU is a nice-to-have that did not gate F7.1a ship.
- **FR-019**: Picking a template MUST snapshot the template's subject + body into the draft at the moment of selection; subsequent admin edits to the template MUST NOT mutate drafts already started from it. **Variable substitution** (per critique E1 / X1 / `contracts/broadcast-template.md § 5`): only `{{chamber_name}}` is server-substituted at snapshot time (HTML-escaped from the tenant display-name port — F7.1a single-tenant impl reads `process.env.NEXT_PUBLIC_TENANT_NAME` with 'SweCham' fallback; multi-tenant impl reads `tenants.display_name` when that table lands in F10+); all other placeholders in the form `[bracketed text]` are member-editable text that ships verbatim in the dispatched broadcast. The Tiptap editor renders bracketed text with a distinct visual style (per critique P4 — grey background + dashed border via `broadcastBracketPlaceholderExtension` ProseMirror Decoration). **First-use microcopy** ("Click any [bracketed text] to replace with your content.") is deferred to F7.1b polish — the visual signal carries the affordance unambiguously in the F7.1a MVP. When a template is snapshotted into a draft, the broadcast row records `template_name_snapshot TEXT` (denormalised copy of the template name at snapshot time per critique P9) so post-template-deletion forensic audit retains the name. **Stale draft banner (per critique E5)**: `ComposeStaleDraftBanner` component ships in F7.1a as a reusable building block (role=status + aria-live=polite + Refresh/Dismiss callbacks) but the consumer mount-point is **deferred to F7.1b** — `/portal/broadcasts/new` has no draft-load flow in F7.1a (members compose fresh drafts only), so the banner has no place to render. Both deferrals are intentional MVP scope cuts that do not affect F7.1a ship-readiness.
...
- **FR-021**: The audit log MUST record `broadcast_template_created`, `broadcast_template_updated`, `broadcast_template_deleted` events with actor + template id + name + before/after values where applicable. **Starter badge UX (per critique P6)**: the admin template list page surfaces an always-visible "Starter" badge on rows where `is_seeded = TRUE` — the badge is the truth-source for the seeded vs admin-authored distinction and per-row dismissal would erase that signal. When admin clicks Edit on a starter template, the editor surfaces a **per-(tenant, templateId) dismissible** confirmation banner ("This is a starter template seeded by the platform. Editing creates a tenant-specific version. The platform will not auto-update this template if starter content is refined.") — dismissal persists via localStorage key `broadcasts.starter-edit-dismissed:{id}` so acknowledging one starter does not silence others. (The original "dismissible" wording referred to this edit-page banner, not the list-page badge; clarified post-verify.)

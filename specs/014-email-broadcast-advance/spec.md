...
- **BroadcastTemplate** *(US7)*: A tenant-scoped, admin-authored template carrying a name, subject, body HTML (subject to FR-002a sanitiser + US2 image allowlist at save), creation/update metadata, and a "started-from" count denormalised for visibility. One tenant → many templates; one template → snapshotted into many drafts. F7.1a ships 5 starter templates per tenant via migration `0133_f71a_default_template_seed.sql` (content authored in `starter-templates.md`).

...
- [x] CHK019 Are requirements defined for cross-tenant probes on the new routes (non-disclosing 404, probe audited) consistent with the existing member/contact routes? [Coverage, Spec §FR-052] → resolved: FR-052 + contract tests (`contact-marketing.test.ts:254,273`) + live-Neon route coverage. **Reworded (staff review S3)**: this line said "timing-safe 404", which the code does not provide and the spec never required — `grep timing` in spec.md and the contract returns zero. A cross-tenant miss writes a `member_cross_tenant_probe` audit row before the 404, so it is measurably slower than the in-tenant soft-deleted miss, which is deliberately not probe-audited (security LOW-1). What leaks is "this UUID was once a contact here", to a caller who already holds `contacts.marketing` and can list every contact in the tenant. The property that IS delivered and tested is non-disclosure: cross-tenant and nonexistent are indistinguishable in the response.
...
## Co-Sign Footer — PR-C (T092) — flag-OFF scope

**T092 Review Gate — Security Checklist Co-Sign (PR-C: the 1:N audience, the two count endpoints, `0297`, the unsubscribe attribution, the erasure sever)**

- **Co-signer**: Claude (AI maintainer; the `security-engineer` pass ran 2026-09-07 — the FIRST security review of this branch; T092 had been ticked without it, which the pass itself recorded as **CHK025 FAIL**), solo-maintainer substitute per Constitution v1.4.2 Principle IX
- **Date**: 2026-09-07
- **Branch**: `108-pr-c-audience-1n` (PR #346)
- **Branch HEAD at review**: `deb5a1c09` — verdict **APPROVE WITH FIXES**: safe to merge flag-OFF; flag-ON withheld on two findings. **Re-verified at `7028148cc`.**
- **Verified clean (from the code, not comments)**: Principle I two-layer isolation — both new F3 queries run on the `runInTenant` `tx`, the correlated EXISTS binds `c2.tenant_id` under RLS, and a REAL two-tenant proof exists at the repo layer (`broadcast-recipient-contacts-keyset.test.ts`) — Constitution I.3 met; **round 2 adds the route-level proof with a real member of another tenant** (`recipient-count-routes.test.ts`) · RBAC: `broadcasts.write` is the NARROWER key (manager holds `broadcasts.read`); `check:api-route-guard` 121 routes; frozen marketing set 49 → 50 · audit truth: F3's adapter writes no actor role; the probe payload uses `attempted_member_id` (the 0009 recency trigger cannot misfire); `broadcast_member_missing_primary_contact_email` never fires for an opted-out-only member — through a positive, exhaustive filter since round 2 · rate limit: atomic Lua consume before the resolve; unknown-id and foreign-id share one query, one timing, one 404 · FR-053a: numbers only on every path; `orphans` stripped from the member body · unsubscribe: token verified at the boundary; attribution resolved inside the bound tenant, response invariant · erasure: sever proven live (peer rows untouched, audit carries no `@`) · log hygiene: `err:` is a class/code everywhere · flag-OFF safety: `env.features.contactMarketingRecipients` is read in ONE module, and the disclosed list (a)–(f) has no hidden entry.
- **Findings and their state**: **MEDIUM-1 (C10) FIXED** — the two COALESCEs in the unsubscribe upsert were independent, so a re-attribution to another member kept the previous member's `contact_id` out of reach of that member's erasure (FR-056); the pair moves together now, live-pinned (`823f2a5be`). **MEDIUM-2 (C3) PRE-FLIP** — the audience walk has no bound before the ceiling and the count endpoints have only a per-user limiter; a bound AT the ceiling would reintroduce the filters-before-ceiling bug (a raw-row cap ≠ the post-filter ceiling), so this is design work (a sanity cap well above the ceiling, or a COUNT pre-check mirroring the filters) plus a tenant-level limiter — recorded in `reviews/pr-c.md` and the quickstart as a precondition of T094, not a merge item. LOW-3 (`droppedByPreference` on the member body is the same class of fact as the stripped `orphans`; permitted by FR-022a) — recorded as a design tension for `pdpa-gdpr-compliance-officer`. LOW-4 (the count routes skip the halt/benefit gates the compose page applies; same shape as `/api/broadcasts/quota`) — recorded, not changed.
- **CHK025**: FAIL at `deb5a1c09` (T092 ticked without this pass) → the pass has now run and its absence is disclosed in `reviews/pr-c.md` row 18 → PASS. **CHK026** PASS. CHK001 / 003 / 008 / 009 / 010 / 015 / 016 / 017 / 018 / 019 / 020 PASS (evidence in the pass report, `reviews/review-20260907-121000.md`).

**Co-sign verdict**: security.md is **CO-SIGNED for PR-C in flag-OFF scope** at `7028148cc` — safe to merge with `FEATURE_CONTACT_MARKETING_RECIPIENTS=false`. **The flag-ON signature is WITHHELD** until C3 (walk bound + tenant-level limiter) is closed with the page-walk-stops test the pass specified; re-run `security-engineer` for it before T094.

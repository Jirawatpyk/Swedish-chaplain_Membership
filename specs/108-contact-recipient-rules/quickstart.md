...
| C (audience) | flag OFF restores the primary-only leg and the 5,000 ceiling — **but Phase 9b changed what the ceiling MEANS and moved the split threshold to 500 in every flag state; see item (10) before reasoning about any number here**. **Everything else in PR-C is UNFLAGGED and lands on merge** — a code revert (`vercel promote`), not a flag flip, is the rollback for any of it (reliability M-3; the list below was completed at the re-review, finding #1): **(1)** FR-021 `status = 'active'` — an inactive / archived member's primary stops receiving; **(2)** the `.limit(5000)` removal — a >5,000 audience is refused, not silently cut; **(3)** the bridge lookups AND `setMemberHalt` throw on a failed read/write — submit 500s, dispatch retries, clear-halt 500s, instead of failing open; **(4)** self-exclusion is by MEMBER id on member-based segments only — pre-108 the sender's primary address was filtered out of EVERY segment kind, so a member who puts their own address on a custom list now receives their own e-blast (FR-022a/b; the compose hint says so); **(5)** every unsubscribe writes `marketing_unsubscribes.contact_id` — the column is in use from merge, see the Data column; **(6)** GDPR erasure now severs `member_id` AND `contact_id` on that member's suppression rows (`severMemberRefs`, FR-056) — before PR-C the `member_id` back-reference was retained; **(7)** a persisted `tier` broadcast whose `segment_params` lost its codes is a terminal `failed_to_dispatch` (before: it was sent to every active member); **(8)** the whole compose UI — live count, per-segment hints, the submit block on a measured refusal, the separate preference toast, the halt-state banner, the compose page throwing on a failed member read; **(9)** the `approved_overdue_count` gauge and the zero-fill / forget behaviour of the gauges cron; **(10)** *(Phase-9 branch, 2026-09-08 — REWRITTEN the same day; read the second paragraph, the first is history)* An interim clamp made the enforced ceiling 500 in every flag state (`min(configuredAudienceCeiling(), DELIVERABLE_RECIPIENTS_PER_TICK)`), refusing what the ~2.08 req/s serial push cannot finish inside `maxDuration = 300`. **Phase 9b replaced that clamp with batching, and the direction reverses.** `DELIVERABLE_RECIPIENTS_PER_TICK` is now the BATCH SIZE: `SPLIT_THRESHOLD_RECIPIENTS` derives from it (was 10,000), `splitBroadcastIntoBatches` sizes batches at it (was Resend's 10,000-per-audience cap), and the clamp applies only with batching OFF. **Prod has batching ON, so on merge, both unflagged: accepted ceiling 500 → 5,000 (50,000 once the 108 flag flips), split threshold 10,000 → 500.** An audience above 500 is now SPLIT and delivered one wave per 5-minute tick, not refused. Four further unflagged changes ride with it: `batch-dispatcher` dispatches one wave per invocation (a second wave died at `maxDuration` and re-burned the batch's contact quota on retry); the two every-5-minutes crons partition `approved` on `estimated_recipient_count` and hand off rather than drop a row when that estimate is stale in either direction; a `permanent` gateway failure is no longer auto-retried (classification persisted as a prefix on the existing `failure_reason` column — no migration; unclassified stays retryable); and the batch path halts a broadcast whose audience drifted between ticks instead of sending stale index slices, with `broadcasts_batch_no_progress_count` to surface it. Rollback for all of it is a **code revert**, not a flag flip. Nothing SweCham can currently compose reaches any of these bounds (150 recipients; ~450 after the secondary import) — one batch, one tick, exactly as today. | `FEATURE_CONTACT_MARKETING_RECIPIENTS=false` + redeploy — for the WIDENING only. The 50,000 ceiling is REAL again since Phase 9b (the interim clamp that made it moot is gone), so a flag OFF genuinely narrows the accepted ceiling 50,000 → 5,000; it does NOT undo the split threshold, the one-wave dispatcher, the cron partition, the retry gate or the drift halt — those are code. | **0297 is WRITTEN from merge, flag or not**: `contact_id` is filled by every unsubscribe (`unsubscribe-recipient.ts` reads no flag). Dropping the column while PR-C's code is deployed breaks every unsubscribe with a 42703 — drop it only after a code revert, via a new migration. (0298 deferred with T086.) |

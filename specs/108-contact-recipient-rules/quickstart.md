...
| C (audience) | not needed for behaviour — **flip the flag OFF** (primary-only leg) | `FEATURE_CONTACT_MARKETING_RECIPIENTS=false` + redeploy | 0297/0298 unused when OFF |
...
```bash
pnpm db:migrate            # applies 0292..0298 to the dev branch; prod migrates on deploy

...
**Goal**: One ceiling (5,000; 50,000 only with BOTH the F7.1a batching flag AND the 1:N flag ON — review H-2), truthful compose-time count with a rate-limited endpoint, no silent truncation, and a resumable provider audience build that fits the 300-second cron budget.

...
- [ ] T093 Operator: perform the FR-027a pre-flight review on `/admin/marketing/audience?kind=secondary&state=on&eligible=1`, switch off anyone who must not receive, and record date + reviewer in `docs/go-live-readiness.md` (also add the Stripe Dashboard "Successful payments = OFF" item to the Live-switch section) — **2026-09-08: the surfaces are prepared, the operator pass is NOT done.** The Stripe "Successful payments = OFF" item is added (`go-live-readiness.md` § 0 Track P-B, under Stripe live-mode prep), and the sign-off line — date · reviewer · eligible-secondary count · Art. 14 attestation — is waiting at the end of the new `go-live-readiness.md` § 6.9. Record the count even when it is 0; "vacuous" is a measurement, not an exemption.

...
- [x] T006 **V2 + V5** Spike against the Resend test account with a raw multipart `fetch` (SDK 4.8 lacks `contacts.imports`): `POST /contacts/imports` with a 3-row CSV (`email` only, `on_conflict=upsert`, `segments=[<test audience id>]`), then `GET /contacts/imports/{id}` until `completed`; record status values, `counts`, whether the audience id is accepted as a segment, and that a pre-existing Resend-side `unsubscribed` contact stays unsubscribed after the upsert — in `research.md § V2`; also read the team's actual rate limit from Settings → Usage (V5) — **SUPERSEDED 2026-09-10:** answered without a separate spike. `research.md` § V2 ("ANSWERED 2026-09-08, except one clause") records the raw multipart `fetch` probes — status values, `counts`, `segments=[{id}]` accepted vs `audience_id` silently ignored, the unsubscribed-flag preservation that makes `upsert` lawful — and § V5 the 10 req/s policy read from the API's own `ratelimit-policy` header; the probe code became the adapter's `createContactImport` / `getContactImport`, pinned by value in `tests/unit/broadcasts/infrastructure/resend-contact-import.test.ts`. Nothing left that a spike would add.
...
- [x] T068 [P] [US3] Integration `tests/integration/broadcasts/audience-1n-status.test.ts` (inactive/archived/erased excluded both legs; secondaries included flag ON; halted excluded; tier-filtered segment applies the same contact rules — US3 s6), ~~`tests/integration/broadcasts/custom-list-opt-out-drop.test.ts`~~ (**staff review 🟢-4: this file was never written.** The behaviour IS asserted — the custom-list opt-out drop by `resolve-segment-recipients.test.ts` and PR-D's `marketing-opt-out-dispatch.test.ts` — but this line and `reviews/pr-c.md` row 5 both named a third file that does not exist), `tests/integration/broadcasts/unsubscribe-contact-attribution.test.ts` (incl. a removed-then-re-added contact with a suppressed address stays suppressed and shows "unsubscribed"); re-pin `tests/integration/broadcasts/audience-cap.test.ts` to the ceiling parameter
...
- [~] T070 [P] [US3] Contract `tests/contract/broadcasts/post-broadcasts-submit.contract.test.ts` (+ `recipientPreferenceExcluded` — camelCase on the wire, not the snake_case this line said) DONE; ~~unsubscribe contract payload `contact_id`~~ **NOT DONE (staff review 🟢-4, undisclosed until now)** — there is no POST-unsubscribe contract test in `tests/contract/broadcasts/` (only `get-unsubscribe-token.contract.test.ts`, untouched). `contact_id` on the unsubscribe payload is asserted at unit level (`unsubscribe-recipient.test.ts`) and live (`unsubscribe-contact-attribution.test.ts` — the DB row and both audits), so the behaviour is covered; the CONTRACT layer is not
...
- [x] T104 [P] [US3] (coverage review) Erasure cascade for the new attribution (FR-056): erasing a member nulls `marketing_unsubscribes.contact_id` AND `member_id` for that member's addresses while the email-keyed row survives. **Delivered as `severMemberRefs`** (not the `setMemberIdNull` / `setContactRefsNull` this line guessed at) wired inside the scrub tx in `scrub-broadcast-content-for-member.ts:170`, compile-time required via `FullMarketingUnsubscribesRepo`; proven live in a NEW file `tests/integration/broadcasts/erasure-severs-suppression-refs.test.ts` (not an extension of `member-erasure-cascade.test.ts`), including a peer member's row surviving untouched and a re-drive affecting 0
...
- [x] T073 [US3] Migration `drizzle/migrations/0297_marketing_unsubscribes_contact_id.sql` (~~+ partial index~~ — deliberately NO index: nothing reads `contact_id` as a predicate, and an index nothing reads costs a write on every unsubscribe; dropped at the round-1 review and documented in the migration header) + journal; `src/modules/broadcasts/infrastructure/schema.ts`; `NewSuppressionInput.contactId` in `src/modules/broadcasts/application/ports/marketing-unsubscribes-repo.ts` + Drizzle upsert; apply + verify
- [x] T074 [US3] F3: repo method `findBroadcastRecipientContacts(ctx, { segmentType, tierCodes?, after?, limit })` in `src/modules/members/infrastructure/db/drizzle-member-repo.ts` (data-model §1 eligibility, keyset, 5,000/page after T081, LEFT JOIN for orphans), port entry in `member-repo.ts`, use case `src/modules/members/application/use-cases/get-broadcast-recipient-contacts.ts`, barrel export; add `eq(members.status,'active')` to the existing `findMembersBySegmentForBroadcast` (`:1349-1357`, unflagged) and delete its `.limit(5000)` (`:1358`)
...
**Goal**: One ceiling (5,000; 50,000 only with BOTH the F7.1a batching flag AND the 1:N flag ON — review H-2), truthful compose-time count with a rate-limited endpoint, and no silent truncation. ~~and a resumable provider audience build that fits the 300-second cron budget~~ — **that half went with T086/T087/T106, and the staff review's 🔴 is its consequence**: PR-C accepts audiences the serial push cannot deliver inside the 300 s budget, which is why the AMENDMENT under US5 now records AS2 as unmet and a flip precondition. Unreachable while the 1:N flag is OFF.

**Independent Test**: 6,200-contact audience: compose shows 6,200; batching OFF → refused with count + ceiling; ~~batching ON → accepted and every recipient pushed exactly once across ≥2 dispatch ticks, `sendBroadcast` only after the last push~~; 20,000 contacts counted in < 3 s; ~~a stalled build is flagged after 30 min~~. **Struck 2026-09-08 (T098): both struck clauses describe the deferred import build.** With T086/T087/T106 unauthored there is no multi-tick push and no stuck-build flag, so a 6,200 audience with batching ON is accepted and never delivered — that is the push-capacity gate, not a passing test. AS2 is recorded MISSING in `spec.md` for this reason.

### Tests for User Story 5 (write first — MUST FAIL) ⚠️

- [x] T081 [P] [US5] Integration `tests/integration/broadcasts/audience-pagination-20k.test.ts` (no truncation; count < 3 s; equals dispatched set). ~~and `tests/integration/broadcasts/audience-import-two-tick.test.ts` (tick 1 submits exactly one import and stores the id; tick 2 with a mocked `completed` + matching counts sends; `failed > 0` or count mismatch → typed failure, no send; second tick never re-submits; stuck after simulated 30 min)~~ — **dropped with T086/T087/T106 (2026-09-07); the file does not exist on disk (verified 2026-09-08, T098).** The `[x]` covers the 20k suite only, and that suite asserts its budget through `ciScaled(3_000)` = 18 s on CI — see FR-043's UNVERIFIED note.
...
- [x] T083 [P] [US5] Unit `tests/unit/broadcasts/domain/audience-ceiling.test.ts` and a guard test that `SPLIT_THRESHOLD_RECIPIENTS < audienceCeiling(true)`. ~~`tests/unit/broadcasts/application/build-audience-tick.test.ts` (CSV has only an `email` column; submit-once; completion rule incl. count mismatch and `failed > 0`; 30-min stuck), `tests/unit/broadcasts/infrastructure/resend-contact-import.test.ts` (multipart body shape, 429 → retryable, error classification)~~ — **dropped with T086/T087/T106 (2026-09-07); neither file exists on disk (verified 2026-09-08, T098).**
...
- [x] T090 [US5] Metrics + logs in `src/lib/metrics.ts` (`broadcasts.audience_resolved_total{segment,mode}`, `audience_pages_total`, ~~`audience_import_status{status}` gauge~~, `recipient_count_ms` histogram) with member-id hashes only; confirm with `observability-instrumentor` that the four new routes (toggle, portal toggle, two counts) emit RED metrics through the existing route instrumentation (Constitution VII), adding per-route counters only if they do not; `docs/observability.md` metrics + alert thresholds (~~stuck 30 min → page~~; skipped money email > 0/24 h → warn; count p95 > 3 s → warn) — **body corrected 2026-09-08 (T098): the banner above said the gauge went with the deferred import build, but this line still named it.** `audience_import_status` does not exist anywhere in `src/` (swept), and there is no 30-minute stuck alert; the only 30-minute broadcast alarm is `broadcasts.approved_overdue_count` (`docs/observability.md:1358`), a different subject at severity *alarm*, not *page*. Do not promise an operator either signal — `reviews/cutover.md` § 4 lists the five that exist.
- [x] T091 [US5] Runbook `docs/runbooks/broadcast-audience-build.md` (~~stuck detection, resume, manual abort, duplicate-push handling,~~ slow / refused / unavailable build, flag-off remedy) + updates to `docs/runbooks/cron-jobs.md` and the reconcile-stuck-sending runbook ~~and the void-pdf-reconcile runbook~~ — **body corrected 2026-09-08 (T098).** The runbook that shipped explicitly disclaims the struck items ("There is NO 'audience building' state and NO import-based build in PR-C", `broadcast-audience-build.md:10`) because they belong to the deferred import build; and there is no `void-pdf-reconcile` runbook in `docs/runbooks/` — the only void one is `void-on-reissue.md`. The `[x]` is correct for what the task actually delivered; the body was describing the deferred design.
- [~] T092 [US5] Run PR-C gates (lint, typecheck, i18n, env gates, `tests/unit/broadcasts tests/unit/members`, T068/T071/T081 integration files by path, `tests/contract/broadcasts/`, e2e T084 `--workers=1`, `pnpm test:coverage`) and review stack (`security-engineer`, `reliability-guardian`, `performance-slo-guardian`, `observability-instrumentor`, `enterprise-ux-designer`, `i18n-translation-reviewer` → 3 review passes → staff-review → re-review); co-sign `checklists/reliability.md`, `checklists/operations.md`, `checklists/ux.md`; open PR `[Spec Kit] 108 PR-C audience 1:N behind flag` — **PARTIAL — corrected 2026-09-07 (round 2): this task was ticked `[x]` on `3a23ec354` with only its GATES half done (row 18) and its note disclosed only the missing e2e; NONE of the six named review agents had run and NO checklist was co-signed for PR-C. The maintainer caught it. Since: e2e T084 done (row 19); `/speckit-review` ×2 (rounds 1 + 2, rows 20–34); the six named agents ran ONCE, in round 2 (security, reliability, performance-slo, observability, enterprise-ux, i18n — 12 reviewers total that round); `reliability.md` / `security.md` (flag-OFF scope) / `operations.md` / `ux.md` co-signed for PR-C at `7028148cc` / `a5dc67f2a`. STILL OPEN before `[x]`: the third review pass, `/speckit-staff-review-run`, the re-review, the flag-ON security signature (pre-flip: C3 walk bound + tenant-level limiter), and the PR itself is open as #346**
...
- [X] T093 Operator: perform the FR-027a pre-flight review on `/admin/marketing/audience?kind=secondary&state=on&eligible=1`, switch off anyone who must not receive, and record date + reviewer in `docs/go-live-readiness.md` (also add the Stripe Dashboard "Successful payments = OFF" item to the Live-switch section) — **DONE 2026-09-08 10:45, VACUOUS.** Prod measured read-only by the maintainer (`scripts/inventory-primary-contact-invariant.ts`): **150 members / 150 primaries / 0 secondary contact rows / 0 marketing_unsubscribes / violations 0**. With no secondary contacts the pre-flight page is empty, the `all_contacts` audience is identical to `primary_only`, and the Art. 14 gate has no uninformed data subject — both signed off with the count, not with "n/a", per FR-027a. Signed at `go-live-readiness.md` § 6.9; the Stripe "Successful payments = OFF" item is filed under Stripe live-mode prep (§ 0 Track P-B). **The sign-off expires on SweCham's secondary-contact import** — re-run the inventory then; it turns both this task and the Art. 14 attestation from vacuous into real work. **Zero secondaries does NOT close the push-capacity gate**: that band is reached by a custom list, which no member count bounds.
- [ ] T094 Operator: set `FEATURE_CONTACT_MARKETING_RECIPIENTS=true` in Vercel, redeploy, and observe the first send per quickstart § Cutover 5 — the five signals PR-C ships: `broadcasts_audience_resolved_total{mode}` flips to `all_contacts`, `broadcasts_recipient_count_ms{outcome="ok"}` inside SLO-F7-013, `broadcasts_dispatch_resolve_failed_total` and `broadcasts_approved_overdue_count` both at 0, a live `broadcasts_marketing_opt_out_filter_count{phase="dispatch"}` series, and `estimated_recipient_count` = delivered (the `audience_import_status` gauge went with T086 and does not exist); record in `specs/108-contact-recipient-rules/reviews/cutover.md` — **2026-09-08: the variable was set to `true` at 09:44, found to be ARMED rather than staged, and DELETED at 10:41. The feature is OFF and `main` is safe to merge.** Production had not been redeployed in between (last prod deploy `#351` `d7003ea0c` at 09:36), so the flip never happened. `reviews/cutover.md` records the whole window, every precondition's status and the five-signal checklist. **This task completes at first-send observation, not at flag-set** — and the § 2 row-3b push-capacity gate is still OPEN. **Rule: on this project, setting the env var IS the flip, scheduled for whenever someone next merges anything (`vercel.json` has no `ignoreCommand`). Set it when you are ready to redeploy, not before.** — **2026-09-08 later: every precondition is now CLEARED.** T093 + Art. 14 vacuous at 0 secondaries (measured); T095 measured; T096 delivered; T098 folded; and the § 2 row-3b push-capacity gate **closed in code** (`DELIVERABLE_RECIPIENTS_PER_TICK = 500`, `currentAudienceCeiling() = min(configured, 500)`) — which also fixed a hazard that existed on the flag-OFF leg, so it was worth shipping regardless of the flip. What remains for T094 is the operator sequence itself: merge this branch → add the env var → redeploy immediately → observe the five signals in `reviews/cutover.md` § 4 on the first real send. — **2026-09-10, what actually blocks it:** two things, and the second matters more. (a) An operator decision — the variable is absent from Vercel (`env.ts` defaults it `false`), and on this repo setting it IS the flip. (b) **SweCham's secondary-contact import.** Measured on prod 2026-09-10: 150 members / 150 live contacts / 150 primaries / **0 secondaries**. The 1:N audience differs from primary-only only through secondary contacts, so with zero of them the flip changes nothing and the first-send observation this task completes on cannot show the feature working. Flip after the import (and re-run `scripts/inventory-primary-contact-invariant.ts` first — T093's sign-off expires on it), not before. — **GitHub Issue #362 (2026-09-12)** — mirrors this line so the item survives 108's archive; keep the two in step.
- [X] T095 [P] Operator: record the team's actual Resend rate limit ~~(Settings → Usage)~~ and any support answer on the Audiences → Segments migration timeline in `research.md § R9/R16` (operations CHK015); a rate increase is no longer required by the import-API design — **2026-09-08: this is now the ONLY task between the project and the T094 decision (maintainer chose to wait for it rather than flip on a guessed bound), and the method changed.** The dashboard's rate limit is not the answer on its own: `addContactsToAudience` is a serial `await` loop, so `throughput = min(account_limit, 1 / RTT)` — a fast account still pushes at 5 req/s if each round trip from `sin1` is 200 ms, and recording only the plan's number would add a third unsourced figure to the two already in the repo. **`reviews/cutover.md` § 5a carries a one-command capture** that reads `ratelimit-*` from the API itself plus a round-trip sample, the arithmetic (`per_tick_max = 300 × throughput × 0.8`) and a table of what each answer implies. Every plausible throughput puts the band **below** today's 5,000 ceiling, which is the point: the hazard is not the slice the flip adds. **MEASURED 2026-09-08 12:41** (five `GET /audiences`, keep-alive, prod key, Bangkok workstation): `ratelimit-policy: 10;w=1` → the account limit is **10 req/s**, so the gateway comment's "~2 req/s" was wrong; but warm RTT is **~0.481 s** (285/281/300/291 ms, TLS only 37 ms), and a serial `await` loop gets `min(limit, 1/RTT)` = **≈ 2.08 req/s — latency-bound**. `per_tick_max = 300 × 3.4 × 0.8 ≈ 830`. Using the documented 10 as a capacity input overestimates ~3×. SweCham: 150 today ≈ 44 s (15 % of budget), ~450 post-import ≈ 132 s (44 %) — both fit; the gap is between ~623 and the enforced ceiling. Recorded in `research.md` § R9, `reviews/pr-c.md` row 33 and `reviews/cutover.md` § 5a with three caveats that all push the number DOWN (Bangkok not `sin1`; `GET` read not `POST` write; four warm samples).
- [ ] T110 [P] Follow-up PR (outside 108's four PRs): upgrade `resend` 4.8 → 6.x with a full gateway contract suite (`tests/contract/broadcasts/resend-gateway-*.test.ts`) and adopt `contacts.imports` + segments/topics per research R16; record the F7 platform risk in `docs/email-broadcast-analysis.md` and `docs/go-live-readiness.md` — **2026-09-10 — NOT blocked, deferred by choice; an earlier note called this "an external condition" and that was wrong.** Installed `resend` is 4.8.0 against a declared `^4.0.1`; 6.x is a MAJOR bump under a gateway that took eight review rounds on #353, plus a full contract suite and a move to `contacts.imports` + segments/topics. Deliberately kept out of the 2026-09-10 follow-ups PR so a regression in either can be attributed to one change. Scope decision for the maintainer, not a dependency. — **GitHub Issue #363 (2026-09-12).**
- [x] T096 [P] Compliance: the record-of-processing entry + legitimate-interest assessment for per-contact marketing preference and marketing to secondary contacts (FR-055) — **DELIVERED with PR-D in `docs/compliance/processing-records.md`** (recipient-side LIA `:113-135`, per-contact-preference activity `:136-152`). **Corrected 2026-09-07 (staff review 🟡-3)**: this task named `docs/record-of-processing.md` "(create if absent)", a path that does not exist — following it would have produced a SECOND ROPA describing the same processing, and two GDPR documents that drift. **The Art. 14 attestation gate at `processing-records.md:128-135` is a precondition of T094** and is now also in `quickstart.md` § Cutover 3a and `reviews/pr-c.md` row 33, where the operator will actually see it
- [X] T097 [P] Docs: `CLAUDE.md` (migration counter 0291→0297 — 0298 deferred with T086, Recent Changes entry, Active Technologies "PLANNED" → shipped), `docs/changelog.md`, `docs/contacts-primary-secondary-gap-analysis.md` status line (H1/H2/G1–G9 closed; G3 kept) — **DONE 2026-09-08.** `CLAUDE.md` needed nothing *at 10:00*: PR-C's own docs commit (#347) had already moved the counter to 0297, written the Recent Changes entry and cleared every "PLANNED" marker (verified, not assumed). **Re-annotated 13:20 — by then it needed something after all.** Its "the flag flip (T094) is NOT cleared — four preconditions, one a blocker: the ceiling accepts 5,001–10,000…" paragraph was made false the same afternoon by T093, T095 and the ceiling clamp, and it also carried the wrong band and the wrong rate. Rewritten: every precondition cleared, the band was ~623 not 5,001, the effective ceiling is 5,000 → 500 unflagged, the account is on Resend Free, and setting the env var is itself the flip. The lesson for this task is that "the agent-context file needs nothing" has a timestamp on it. `docs/changelog.md` gains the full 108 entry. The gap-analysis got more than a status line: § 9 records each of H1/H2/G1–G9 **re-verified against source on the day** with `file:line` (G6 `drizzle-member-repo.ts:1613`, G5 `:1621`, G9 `member-identity-adapter.ts:173`, G4 `resend/route.ts:106`, G7 `unsubscribe-recipient.ts:139-147`, G1 `billing-recipient-port.ts:41`), G3 marked KEPT-by-decision rather than closed, and a new § 10 listing what 108 does **not** close — the flip itself, the push-capacity gate, the G5 walk bound, the Art. 14 attestation, and Tier C bulk import, which is still the one unmet requirement from the original SweCham ask.
- [X] T098 Run `/speckit.analyze` for FR↔SC↔contract traceability (operations CHK023) and fold any finding into spec.md before T094 — **DONE 2026-09-08.** 61 FRs / 11 SCs / 39 acceptance scenarios walked; **60 of 61 FRs traceable, FR-044 the one true orphan** (no SC, no shipped task, contract clause under the DEFERRED banner). 28 findings (6 HIGH / 12 MEDIUM / 10 LOW), all folded. The six HIGH ones were each a case of an artefact saying something the code does not: (1) the US5 AMENDMENT declared **FR-044 "satisfied by per-tick retry"** — not one of its six clauses holds, verified at `dispatch-scheduled-broadcast.ts:532`/`:702` and `resend-broadcasts-gateway.ts:246`; (2) FR-022a called the sender-self-exclusion scope change **"(unchanged)"** when it is a live-on-merge behaviour change (`91505b8f2^` compared addresses on every segment kind; now member-keyed and member-based only); (3) FR-041's batching exception promises "every recipient receives one copy" in a band where `split-large-broadcasts` skips and the serial push cannot finish; (4) FR-044 ticked green in a coverage matrix dated three days before its own deferral; (5) CHK023 ticked "100 % coverage" while the same file's footer recorded the re-run as still open; (6) FR-055 omitted the **Art. 14 attestation gate** that `processing-records.md:128-135` makes a hard precondition of the flip — an operator reading only `spec.md` would have flipped straight past it. Also corrected: five plan.md statements describing the deferred import build as shipped, a `research.md` R16 section missing the DEFERRED banner R9 carried, five reliability CHKs ticked for a design that is not on the branch, three named test files that do not exist on disk (one of them in a copy-paste command block in `quickstart.md`), a metric (`audience_import_status`) and a 30-minute page alert that exist nowhere, and a stale `AUDIENCE_HARD_CAP` reference in the gateway comment. **Pattern worth keeping: a deferral that adds a banner but leaves the ticked bodies, matrices and checklists alone reads as green from every angle except the banner's own paragraph.**
- [ ] T099 Follow-up PR after one clean week (spec § Assumptions definition): delete `FEATURE_CONTACT_MARKETING_RECIPIENTS` from `src/lib/env.ts`, `.env.example`, Vercel; delete the `primary_only` leg in `resolve-segment-recipients.ts` and `findMembersBySegmentForBroadcast`; re-pin tests; `[Spec Kit] 108 remove cutover flag` — **2026-09-10:** waits on T094 — the one-clean-week clock starts at the flip, which has not happened (see T094 for why). — **GitHub Issue #364 (2026-09-12)** — waits on #362.
- [ ] T100 Final: run the full gate list from CLAUDE.md § Commands incl. `pnpm test:e2e --workers=1` and `pnpm typecheck` as the last step; `/speckit.verify` then `/speckit.ship` per PR — **2026-09-10 (PR #355, branch `111-broadcasts-followups`):** every gate on the list ran and is recorded in `reviews/followups-20260910.md` § Gates (lint 0 · typecheck 0 last on every commit · 10 static gates · coverage 1,238 files / 13,985 green · contract 188 / 1,944 · pre-push integration folder 62 / 302) EXCEPT a COMPLETE e2e: the full `--workers=1` run was stopped externally at 85 first-attempt tests (60 pass / 25 fail — the failures are timeouts in specs this PR does not touch, and no `main` baseline exists because e2e has no CI job). Stays open until an e2e run completes; `/speckit.verify` + `/speckit.ship` per PR were replaced by the two independent agent reviews + the ledger, per the solo-maintainer substitute.

---

## Phase 9b: Deliverable batches — ~~audiences above one tick deliver across ticks~~ **SUPERSEDED AND DELETED**

> **Every task in this phase was implemented, reviewed, and then deleted on the same day
> (2026-09-08, `ca51f59a1`).** Not because the work was wrong — the batch path it improved was
> real and its bugs were real — but because the constraint the whole phase existed to manage
> turned out not to exist. `POST /contacts/imports` carries any audience in one ~412 ms call, so
> there is nothing to split into ticks.
>
> **Why it was built anyway, recorded because the cost was half a day.** The import endpoint was
> probed twice that morning with the field spelled `audience_id`, which Resend ACCEPTS and
> silently ignores. The false negative ("the import does not attach to an audience") went into
> `research.md` as a finding, and Phase 9b was planned on the strength of it. The working
> spelling is `segments=[{ id }]`; our own contract said `segments=[<id>]`, which is a 422, so
> that line had never been executed. **A contract nobody has run is a guess, not a spec** — and
> an API that accepts and ignores an unknown field makes every negative probe uninformative
> unless you first check that it rejects garbage.
>
> The phase is kept below, unedited, for three reasons: the measurements in its preamble are
> still good; four of its findings were NOT about batching and survive in the tree (B-1's missing
> `ORDER BY` on the shared segment read, H-1's permanent-failure gate, T130/T139's failure
> classification, the flag-conditional claim predicate); and the shape of the mistake is worth
> being able to re-read.
>
> **Do not treat any `[x]` below as describing code that exists.**
...
- The import API is NOT this phase, **and T145 has now shown it ATTACHES** (2026-09-08 — this
  bullet said the opposite until the probe ran). `POST /contacts/imports` 201 in 412 ms,
  `completed` in ~306 ms, honest `counts`, and `GET /audiences/{id}/contacts` returns the
  imported row. The two earlier "does not attach" probes sent `audience_id`, which Resend
  **accepts and silently ignores**; the contract's `segments=[<id>]` is a 422; the working
  spelling is **`segments=[{"id":"<uuid>"}]`** (research § R9 V4, contract § 4 corrected).
  Consequence for THIS phase: none — 9b is already built and its batches are correct. Consequence
  for the follow-up: the import build (T086/T087/T106) is a ~2-call, size-independent push, so it
  retires batch SIZING as the scaling mechanism and is the cheaper close. It does NOT retire
  T143's drift halt — freezing the resolved list (FR-044 a/d) is a different problem from pushing
  it, and `data-model.md` § 2.5's working table is still that answer. The segments migration is
  T110.
...
- [x] T133 `src/modules/broadcasts/domain/audience-ceiling.ts`: `export const SPLIT_THRESHOLD_RECIPIENTS = DELIVERABLE_RECIPIENTS_PER_TICK;` (derived, not a second literal). Rewrite BOTH docblocks: the constant is now the batch size and the split threshold; the "5,001–10,000 IS a gap" paragraph becomes history ("was a gap until Phase 9b — the batch size now equals what one tick can push, so nothing falls between the threshold and the batch cap"); keep the T095 measurement + caveats + the Principle III note — **cite `research.md` § R9's CORRECTED block for the arithmetic instead of restating it** (analyze D1); add that it MUST stay ≤ `RESEND_PER_AUDIENCE_CAP`. **Decision hook (analyze C1 / plan.md Complexity Tracking #5)**: either move `audienceCeiling()`, `SPLIT_THRESHOLD_RECIPIENTS` and `DELIVERABLE_RECIPIENTS_PER_TICK` to `infrastructure/` next to the gateway (retire CT #5, update the barrel + the domain test path) or keep them and leave CT #5 standing — decide at GREEN, write the choice into CT #5 either way
- [x] T134 `src/modules/broadcasts/application/use-cases/split-broadcast-into-batches.ts:110`: `computeBatchRanges(input.resolvedRecipientCount, DELIVERABLE_RECIPIENTS_PER_TICK)`; keep the `MAX_RECIPIENT_COUNT` guard; docblock says why the cap is the tick bound, not Resend's audience limit
- [x] T135 `src/modules/broadcasts/infrastructure/broadcasts-deps.ts` `currentAudienceCeiling()`: `isF71aUs1Enabled() ? configuredAudienceCeiling() : Math.min(configuredAudienceCeiling(), DELIVERABLE_RECIPIENTS_PER_TICK)`. Docblock: the clamp exists ONLY for the single-tick path; with batching on, everything above the threshold splits, so `dispatch-scheduled` never sees more than one tick's worth. The two batch crons keep reading `configuredAudienceCeiling()` (T-review 2026-09-08 — a per-tick clamp is not their bound)
- [x] T136 `src/modules/broadcasts/application/services/batch-dispatcher.ts`: **one wave per invocation** — take `input.pendingBatches.slice(0, cap)` as the queue; the remainder is neither dispatched nor touched; output gains `deferredToNextTick`. Docblock states the reason in one paragraph (a second wave is killed at `maxDuration`, stranding a half-pushed audience and re-burning the whole batch's contact quota on the retry) so the next person does not "optimise" it back
...
- [x] T138 `src/app/api/cron/broadcasts/dispatch-batches/route.ts`: log + count `deferredToNextTick` per broadcast (`broadcasts_batches_deferred_total{tenant}` or fold into the existing summary — check `src/lib/metrics.ts` for the batch counters first); fix the route header docblock (the "per INVOCATION … up to `concurrencyCap` in flight" note now says one wave per invocation and why). **Term (analyze A1)**: use "per invocation" everywhere in code and "one wave per invocation (= one cron tick)" once, in the dispatcher docblock, so "tick" and "invocation" stop reading as two concepts
- [x] T139 If T130 went RED: gate auto-retry on `retryable` failures only; a `permanent` failure leaves the batch `failed` for manual review with its reason in the audit payload

### Docs + gates for Phase 9b

- [x] T140 Docs, all in the same commit as GREEN so no artefact describes the pre-9b state (the T098 lesson — a clamp shipped at 13:00 left five documents false until 14:30): `quickstart.md` § Rollback matrix row C item (10) → "500 is the BATCH SIZE; the enforced ceiling is the configured 5,000 / 50,000 with batching on, 500 with it off" + a NEW operator line: **on Resend Free set `tenant_broadcast_settings.dispatch_concurrency_cap = 2`** (3 audiences minus `General`); on any plan keep it **≤ 4** — `MAX_CONCURRENCY_CAP = 8` is still accepted by the setting but 8 × 2.08 ≈ 16.6 req/s exceeds the 10 req/s policy and turns the batch path 429-bound (analyze E2 — documented, not enforced, out of 9b scope); and above ~1,000 contacts the plan is the limit — batches beyond it fail loudly with an audit row, and that is the intended signal to upgrade; `reviews/cutover.md` § 5 ("closed in code" → "closed, then removed as a ceiling by Phase 9b"), § 5a table; `contracts/broadcast-audience.md` § 3; `docs/runbooks/broadcast-audience-build.md` remedy #2 — UN-strike it, batching IS the remedy again, and add the concurrency-cap note; `docs/runbooks/cron-jobs.md` (the "prod's ceiling today is 500" line); `.env.example`; `CLAUDE.md` § Recent Changes; `spec.md` FR-041 note and **US5 AS2** — 6,200 with batching ON is REACHABLE again (13 batches across ticks); flip its status from "REFUSED BY DESIGN" to "TESTED BY T132 at 1,200; 6,200 needs Pro" and mirror in `reviews/task-coverage-review.md`; `reviews/pr-c.md` row 33 → option **(b)** landed (lowered threshold + one-wave-per-tick resume), not (c); `docs/contacts-primary-secondary-gap-analysis.md` § 10 item 2/3 — **SUPERSEDED 2026-09-10:** this is the docs list for Phase 9b's BATCH model ("500 is the BATCH SIZE", the concurrency cap, US5 AS2 "reachable again"), and `ca51f59a1` deleted the batch path the same day it was built. The documents it names were rewritten for the import-build model instead — `reviews/cutover.md` § 5, `quickstart.md` § Rollback matrix row C, `broadcast-audience-build.md` remedy #2 ("Batching is deleted. This is the remedy.") — so following this task now would reintroduce text about a mechanism that does not exist.
- [x] T141 Gates: `pnpm typecheck` · `pnpm lint` · `pnpm check:i18n` · `pnpm vitest run tests/unit/broadcasts tests/contract/broadcasts` · `pnpm test:integration tests/integration/broadcasts/deliverable-batches-multi-tick.test.ts` · then `git push` (pre-push runs the whole `tests/integration/broadcasts/` folder — do NOT `SKIP_INTEGRATION_PREPUSH`). Then `reliability-guardian` (opus) + `whole-branch-reviewer` (fable) before PR #352 is re-declared MERGEABLE; run the outbox inventory once more before merge — **SUPERSEDED 2026-09-10:** the gate list for the deleted 9b batch branch — `tests/integration/broadcasts/deliverable-batches-multi-tick.test.ts` no longer exists, and "PR #352 re-declared MERGEABLE" happened on #352's own gate run (merged 2026-09-10 as `4094361e6`; #354 `9b4583451` and #353 `33f26161c` followed). The reviewers it names ran on both PRs (eight rounds on #353); nothing here is outstanding.
- [x] T142 Post-merge observation (no code): confirm on prod that a 150-recipient send still takes `dispatch-scheduled` (150 ≤ 500) — i.e. nothing changes for SweCham until an audience exceeds 500, at which point it splits. Record in `reviews/cutover.md` § 4 — **SUPERSEDED 2026-09-10:** "at which point it splits" is the deleted batch path. On `origin/main` an audience above 500 with the import flag OFF is REFUSED as `audience_too_large` (a legible failure replacing a silent ~623-contact truncation — `reviews/cutover.md` § 5), not split; with the flag ON there is no per-tick clamp to observe. The post-merge observation that remains is T094's five-signal first-send checklist, which already covers a 150-recipient send.

### Added by `/speckit.superb.review` (2026-09-08 16:55) — gaps the first task set left open

- [x] T143 RED then GREEN — **drift guard on the batch path** (FR-044 a/d). Unit `tests/unit/broadcasts/application/dispatch-batches-drift.test.ts` + a case in T132: the manifest already carries the split-time total (`dispatch-broadcast-batch.ts:193-196`, `isLastBatch` compares `recipientRangeEnd + 1` to it). Before dispatching ANY batch of a broadcast in a tick, compare the re-resolved `allRecipients.length` to that total; on mismatch dispatch NOTHING for that broadcast, audit `broadcast_resend_audience_drift` (existing event — payload `{expected, observed, phase: 'batch'}`), count `broadcasts_audience_drift_detected_count` (existing metric), and leave the manifests `pending` for staff (the broadcast surfaces via `approved_overdue_count` / `stuck_sending_count`). This DETECTS drift; it does not fix it. The fix — freezing the resolved list at split time — is the working-table design `data-model.md` § 2.5 keeps as fallback (a per-broadcast recipient table under RLS, deleted at terminal status, covered by the erasure cascade — FR-044 g). Record that as a follow-up in `research.md` V4, not in 9b, because it needs a migration and the erasure-cascade work. **Staff recovery path (analyze U1 — a halt nobody knows how to lift is a strand)**: the halted broadcast stays `sending` with its manifests `pending` and is surfaced by T144's no-progress signal; the runbook (T140, `broadcast-audience-build.md` § C) tells staff to **cancel it** (`cancelBroadcast`, admin actor — quota is released) and ask the member to re-submit, which re-resolves and re-splits against the current audience. No "accept the new count" path: the batches already sent used the OLD slices, so accepting would double-send or skip exactly the recipients the drift moved. Document that reasoning in the runbook so the next person does not add it
- [x] ~~T144~~ **VOID — the subsystem this task instrumented was deleted** (round 2 R2-5). `ca51f59a1` removed the batch model, so `broadcasts_batch_no_progress_count`, the manifests it counted and both crons that wrote them no longer exist. The stuck-signal need it served is met on the import path by `broadcasts_audience_import_stuck_count` (T106) plus the FR-021 wall-clock budget added under round-3 finding 3-7. Kept as a struck-through record rather than deleted, because `quickstart.md` row C promised this metric to an operator and the trail from that promise to its absence should be readable. Original text: **"no batch progressed" stuck signal** (FR-044 f). Today a `sending` broadcast whose manifests are deferred tick after tick is indistinguishable from one whose batches keep failing until `stuck_sending_count` fires at 24 h. Extend `reconcile-stuck-sending` (or the gauges cron — check which owns `stuck_sending_count`): a `sending` broadcast with ≥ 1 `pending` manifest and NO manifest reaching a terminal status in the last 30 min (FR-044's number) increments `broadcasts_batch_no_progress_count{tenant}` and logs the broadcast id. Contract test on the cron; docblock in `docs/observability.md` next to `approved_overdue_count`. **Alert definition (analyze U2 — every alarm in `observability.md` states one)**: `≥ 1` sustained for 30 min → severity **alarm** (same tier as `approved_overdue_count`, not page), routed to `docs/runbooks/broadcast-audience-build.md` § C with the T143 recovery path; auto-resolves when the gauge returns to 0. Add the row to `observability.md`'s alert table, not only the metric docblock
- [x] T145 [P] **Re-probe the import API with the contract's field — DONE 2026-09-08, and the answer is YES.** Throwaway audience + one synthetic `@example.com` row, both deleted after (audiences back to `["General"]`). Three spellings, three different answers: `audience_id` → **201, silently ignored** (contacts land in Global Contacts only — this is what made two earlier probes confidently wrong); `segments=["<uuid>"]`, the shape contract § 4 specified → **422** *"The `segments` must be an array of objects with a UUID `id` field."*; **`segments=[{"id":"<uuid>"}]` → 201 in 412 ms, `completed` in ~306 ms, `counts {total:1, created:1, failed:0}`, and `GET /audiences/{id}/contacts` count = 1.** Recorded in `research.md` § R9 V4 with the full table; contract § 4 corrected; this phase's preamble bullet corrected. **Consequence**: the import build (T086/T087/T106) is a ~2-call size-independent push and is now the cheaper close — it retires batch SIZING as the scaling mechanism. It does NOT retire T143: freezing the resolved list (FR-044 a/d) is a different problem from pushing it, and `data-model.md` § 2.5's working table remains that answer. Open for T086, not for this: how the import behaves at the Free plan's 1,000-contact cap, and whether `on_conflict=upsert` re-attaches a contact that exists globally but not in the target audience
- [x] T146 [P] Unit pin (one assertion, in T128's file): with batching ON, `currentAudienceCeiling() === configuredAudienceCeiling()` — so the two batch crons (which read `configured`) and count/submit/`dispatch-scheduled` (which read `current`) compare against ONE number, keeping FR-042 true in the state where all five readers are live

### Added by `/speckit.superb.tdd` (2026-09-08 17:20) — the partition hole T135+T137 open

- [x] T147 RED then GREEN — **`dispatch-scheduled` hands off instead of dying** (hole direction A).
...
- [x] T148 RED then GREEN — **`split-large-broadcasts` never releases a claimed row** (hole
...
**Phase 9b STATUS (2026-09-08 18:15)**: T126-T139 + T143/T144 + T146-T148 SHIPPED (RED commits `8a025b378` / `a9c1c02e8`, GREEN `0a0bbf142` / `517ae1e55`). Broadcasts unit+contract 1475 green (baseline 1453), typecheck + full lint clean, T132 green on live Neon. T140 docs landed with this commit. T145 DONE (the import API DOES attach, with `segments=[{"id":…}]` — it changes the FOLLOW-UP plan, not 9b). **Pre-merge outbox inventory re-run 2026-09-08 18:50 against prod: NOTHING in flight, 0 rows above the bound — safe to deploy.** **Still open**: T141's two reviewer agents; T142 (post-merge observation). T132 passed on its FIRST live run: it composes parts that were each driven RED separately, so that is the expected outcome, not an untested green.

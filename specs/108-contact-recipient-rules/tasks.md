...
- [ ] T073 [US3] Migration `drizzle/migrations/0297_marketing_unsubscribes_contact_id.sql` (+ partial index) + journal; `src/modules/broadcasts/infrastructure/schema.ts`; `NewSuppressionInput.contactId` in `src/modules/broadcasts/application/ports/marketing-unsubscribes-repo.ts` + Drizzle upsert; apply + verify

...
- Q2: Do secondary contacts that already exist at cutover become marketing recipients immediately? → A: **Yes, all of them** (option A), on the basis that they were added by the member's own primary contact or by staff under an Art. 14 attestation, the lawful basis is legitimate interest for B2B contacts, and every broadcast carries one-click unsubscribe. Two conditions adopted: a staff pre-flight list before the first send (FR-027a) and a PDPA/GDPR compliance review at the plan gate. Prod count (read-only, 2026-09-04): 150 live contacts, all primary — **0 secondary contacts exist today**, so the cutover question has no population until Tier C import or manual adds; the pre-flight review (FR-027a) stays as the safeguard for contacts added later. Same count confirmed 0 members violating the primary invariant, 110 active / 40 inactive members, 0 unsubscribes.
...
- **FR-050a** (SV/TH length variance — added 2026-09-06 at the PR-D UX review, ux CHK033): every new label MUST be laid out for the LONGEST of the three locales, never for EN. Measured on this feature's own keys, SV runs +17 % to +56 % longer than EN and produces unbreakable compound words; TH is comparable in code-point count but needs the taller Thai line-box and never takes italic. The measured constraints: (1) `admin.marketing.audience.title` — EN "Marketing audience" (18) → SV "Marknadsföringsmålgrupp" (23, a single compound that cannot hyphenate); every container that renders it (PageHeader `<h1>`, sidebar item, breadcrumb leaf, ⌘K row) MUST wrap or truncate with the full string recoverable, never overflow. (2) `admin.marketing.audience.preflightPreset` — a header action carrying a translated label MUST be allowed to wrap (`h-auto whitespace-normal`), because `buttonVariants` is `whitespace-nowrap` and the original SV string (47 chars) exceeded the 272-px content box of a 320-px viewport; the SV copy was also shortened to "Förhandskontroll: sekundära som är på". (3) `admin.marketing.audience.columns.state` — EN "Marketing" (9) → SV "Marknadsföring" (14, +56 %): `TableHead` is `whitespace-nowrap`, so column widths are pinned by `table-fixed` + `<colgroup>` and the table's min-width is the column TOTAL, not an EN-measured floor. (4) `admin.marketing.audience.filters.eligible.on` — SV "Endast behöriga medlemmar" (25) clipped inside a fixed-width `SelectTrigger` (`line-clamp-1`): filter triggers MUST size to content (`sm:min-w-*`, never `sm:w-*`) so the user can read their own selected value. (5) `shared.marketing.state.on` — SV "Marknadsföring på" (+42 %): a state badge grows its column rather than wrapping, so no column holding one may carry a hard `max-w`. Acceptance: the 320-px reflow assertion in `tests/e2e/admin-marketing-audience.spec.ts` runs for ALL THREE locales, asserting `document.documentElement.scrollWidth === clientWidth` in each — an EN-only reflow test cannot see a +28 % SV label.

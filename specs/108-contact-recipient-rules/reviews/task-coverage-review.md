...
| US5-s1…s4 | acceptance scenarios | T | T081–T084, T086–T087 | ✅ 2026-09-08 (third revision the same day): **AS2 is REACHABLE via the Contacts-Import build.** It was "missing", then "REFUSED BY DESIGN" under the `DELIVERABLE_RECIPIENTS_PER_TICK = 500` clamp, then briefly "13 batches" under Phase 9b. All three are history: the batch path is deleted and one import call carries any audience. 6,200 is accepted with the import flag ON; for real it needs Resend Pro (the Free plan's 1,000-contact cap binds first), which is a billing decision. s1/s3/s4 stand |

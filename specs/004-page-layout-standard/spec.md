...
- **SC-009**: The top bar `<header>` has identical computed height (56px), horizontal padding, and gap on both admin and portal layouts. *(Superseded by spec 122 US1, 2026-09-26: the AURA boards size the two differently — staff bar 56px, portal header 72px from 1024px / 64px below; each keeps a fixed height, so neither shifts the page. `tests/e2e/top-bar-consistency.spec.ts` asserts those heights.)*

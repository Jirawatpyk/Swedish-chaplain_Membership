...
- [ ] T013 Add F2 event-emit hook `member_plan_manually_changed` to F2's existing `changeMemberPlan` use-case — **DEFERRED to Wave C alongside DB `audit_event_type` pgEnum extension**. Drizzle adapter type-checks against the pgEnum literal union, so widening `F3AuditEventType` without the enum extension breaks `members/infrastructure/audit/audit-adapter.ts` typecheck. Wave C migration adds the enum value via `ALTER TYPE audit_event_type ADD VALUE 'member_plan_manually_changed'`, extends the Drizzle pgEnum schema, then re-adds the union member + emit. F8 supersede listener (T184 Phase 5+) is the only consumer; deferral does not block Wave B's `scheduleNextRenewalPlanChange` + `getEffectivePlanForRenewal` use-cases. Notes pinned in `members/application/ports/audit-port.ts` + `members/application/use-cases/change-plan.ts`.

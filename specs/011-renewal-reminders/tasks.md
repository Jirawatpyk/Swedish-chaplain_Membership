...
- [ ] T115a [US2] **AS3 reason-badge enum fidelity** (carry-forward from plan.md Complexity Tracking #7 / staff-review Round 5 B1; surfaced again at `/speckit.verify.run` 2026-05-06 finding H2): extend `CLOSED_REASONS` in `src/modules/renewals/domain/renewal-cycle.ts` with `'grace_expired'` and `'payment_failed'` literals; extend the `closed_reason` pgEnum (or text CHECK) via a fresh F8 migration; wire the lapse-decision branch in `dispatch-one-cycle.ts` so the dispatcher writes the *specific* reason (`grace_expired` when `now > expires_at + grace_period_days` and the cycle was never paid; `payment_failed` when an F5 attempt failed permanently before grace ran out) instead of the catch-all `'lapsed'`; add the matching i18n keys under `admin.renewals.lapsedReason.{grace_expired, payment_failed}` × EN+TH+SV; update `lapsed-tab.tsx` so the badge renders the specific reason. Existing `'lapsed'` literal stays for backward compat (legacy rows pre-migration) — admin UI maps it to "Grace expired" until migration backfill decides per-row reassignment, then deprecate the literal. **Tests**: extend `tier-aware-reminder-cron.spec.ts` to assert the AS3 badge differentiates the two modes; add a unit test in `dispatch-one-cycle.test.ts` for the new branch. **Why deferred from Phase 4**: lapse-decision logic doesn't yet exist in `dispatch-one-cycle.ts` (current dispatcher fires reminders only — the lapse transition is a separate concern that lands with the daily cycle-state-reconciler that Phase 5/9 introduces alongside FR-005a/b). Ship date: alongside the cycle-state-reconciler use-case in Phase 5 (T138 reconcile-pending-reactivations is the natural home — extend its scope to cover the grace→lapsed transition with the right reason write).

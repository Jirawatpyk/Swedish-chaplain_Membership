...
- [ ] T277c Phase 10 checklist sweep (`/speckit.staff-review.run` Wave K23 finding R001). The 4 quality checklists (`checklists/{ux,security,integration,reliability}.md`) shipped Phase 5 with 8/40, 4/40, 1/45, 1/40 closed (`requirements.md` PASS 16/16 was closed at /speckit.specify). Per F4/F7 precedent + K22 F1 documented carry-forward, the checklists fill incrementally during Phase 6–9 implementation and get a dedicated polish-pass at Phase 10. This sweep task: walk each checklist row, mark items closed by code that already shipped in Phases 1–9 (most security + reliability items are testable today against the merged code), defer remaining items to a P11 maintenance wave with explicit rationale, AND verify NO new code-level finding gets discovered during the walk (any new finding pauses the sweep + opens a follow-up task). NOT a Constitution violation in current state — the 4 NON-NEGOTIABLE principles I/II/III/IV all PASS green per K22 verify-run + K23 staff-review.

- [ ] **T277d** Post-MVP carry-forward — **LapsedTab inline row actions** (UX-review R5 finding I1, 2026-05-09). Currently `_components/lapsed-tab.tsx:144-155` exposes only a "View detail" link per row; an admin working a 30+ row lapsed cohort must open each cycle individually to send a win-back email or trigger a refund. T072 originally wrote "reactivate/archive CTAs deferred to US3+US7" but F8 spec only defines US1-US6 (US7 is a typo cross-reference) so this work has no owning task today. Spec design routes admin reactivate decisions through the **Phase 8 escalation task queue** (T220-T227, `manual_admin_reactivation_review` task type) which is the *primary* admin surface — but `LapsedTab` row actions remain a useful *secondary* shortcut for admins already navigating the pipeline view. Scope when picked up: reuse `RowActionsMenu` (`pipeline-table.tsx:240+`) shape, drop "Send reminder" (not meaningful for past-expiry cycles), add "Mark contacted" (US4 outreach) + Reactivate/Reject dialogs that call the existing T136/T137 use-cases. Defer until: Phase 8 T227 ships and admin feedback indicates the queue alone is insufficient. NOT a ship blocker — cycle-detail Alert callout (UX R5/I3) covers the workflow gap for admins arriving via direct URL.

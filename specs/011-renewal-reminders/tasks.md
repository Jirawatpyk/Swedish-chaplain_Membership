...
- [~] T115a [US2] **AS3 reason-badge enum fidelity** (carry-forward from plan.md Complexity Tracking #7 / staff-review Round 5 B1; surfaced at `/speckit.verify.run` 2026-05-06 finding H2). **Infrastructure parts shipped 2026-05-06** (commit `<see git log>`); **decision-branch wiring deferred to Phase 5 T138**. Status:
    ...
    - [ ] **Phase 5 — DEFERRED** (T138 cycle-state-reconciler scope extension): wire the actual lapse-decision branch in the new use-case so the system writes the *specific* reason (`grace_expired` when `now > expires_at + grace_period_days` with no payment attempts; `payment_failed` when ≥1 F5 attempt failed permanently pre-grace) instead of catch-all `'lapsed'`. Add E2E coverage in `tier-aware-reminder-cron.spec.ts` to assert AS3 badge differentiation. **Why this part is deferred**: the lapse transition (`awaiting_payment` → `lapsed`) doesn't currently happen anywhere in F8 — the Phase 4 dispatcher only READS `cycle.status === 'lapsed'` to skip dispatch. The cycle-state-reconciler that performs the transition (and therefore needs to know *why*) lands in Phase 5/9 alongside FR-005a/b/c. The infrastructure shipped here is forward-compatible — when T138 wires the decision branch, no DB/i18n/UI changes are needed, only the new code site.

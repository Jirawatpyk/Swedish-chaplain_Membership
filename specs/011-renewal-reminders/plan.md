...
**Result**: All 10 principles **PASS**. Standing Complexity Tracking entries: #1 (solo-maintainer substitute), #2 (cross-module schema migration), #3 (F4 callback hook), #4 (F2 scheduled-plan-change), #5 (documentation-sync ritual), #6 (TierUpgradeSuggestionBase<R> mapped-type DU — added Round 4 remediation 2026-05-04), #7 (AS3 reason-badge fidelity deferred to Phase 4), and #8 (Phase 8 R10 deviations bundle: 5 inline producers + audit-rollback drift + Drizzle schema gap + statement-breakpoint inertia + FR-043 consumer-wiring deferred — added Round 10 close 2026-05-10).
...
| **#7 — AS3 reason-badge fidelity deferred to Phase 4** (added Round 5 staff-review remediation 2026-05-04 B1) | Spec AS3 (spec.md:77) names `grace_expired` and `payment_failed` as the reason badges shown in the Lapsed tab. Phase 3 ships with `CLOSED_REASONS = ['paid', 'cancelled', 'lapsed', 'completed_offline', 'admin_reactivated', 'admin_rejected_with_refund', 'pending_reactivation_timed_out']` — `lapsed` is the catch-all. Extending the enum now would be **premature** because the **lapse-decision logic that distinguishes grace-expired vs payment-failed lives in the Phase 4 cron dispatcher (US2)**, which has not shipped. The Phase 3 LapsedTab today shows the literal `closedReason` value via `lapsedReason.{lapsed, ...}` i18n keys; when Phase 4 ships the cron, it will (a) extend `CLOSED_REASONS` with the two new variants, (b) extend the pgEnum migration, (c) wire the lapse-decision branch (grace_expired vs payment_failed) into the dispatcher, and (d) update the i18n keys. Phase 3 Review-Gate exempts AS3's reason-badge enum fidelity per this entry. **Concrete deferred work**: add Phase 4 task `T08X — extend CLOSED_REASONS + closed_reason pgEnum with 'grace_expired' + 'payment_failed' + map at cron-lapse-decision site + update lapsed-tab i18n keys`. | Option (a) ship the enum extension in Phase 3 (rejected): adds DB+code surface area for a state machine path that has no producer until Phase 4. The lapse-decision logic owns the new-vs-old-reason mapping; shipping the enum without the decision logic would require backfilling existing 'lapsed' rows with arbitrary distinctions — a forensic-data-quality regression. Option (c) defer indefinitely (rejected): AS3 is a P1 acceptance criterion; the deferral must explicitly land in Phase 4's task plan, not be an open TODO. |
...
| **#9 — `evaluate-tier-upgrade.ts` + `accept-tier-upgrade.ts` ship without unit tests; integration coverage is the binding correctness contract** (added Staff-Final review 2026-05-10 / R002) | Both use-cases (578 + 674 LOC) own complex orchestration across 4-6 ports each: `tierUpgradeRepo` + `tierUpgradeEvalCandidateRepo` + `tenantRenewalSettingsRepo` + `planCatalog` + `auditEmitter` (evaluate); plus `escalationTaskRepo` + `f5RefundBridge` (accept). A vitest unit suite covering the decision tree + side-effect orchestration would be ~80% mock-setup, ~20% assertion — the mock-vs-real-Postgres drift risk (per project memory `[Skip is not pass]`) outweighs the coverage benefit at MVP scale. Coverage is provided by integration tests on live Neon Singapore: `tier-upgrade-evaluate.test.ts` (7 cases: happy + idempotent + tenant-disabled + no-thresholds + already-at-target + suppression + R3-IMP-8 no_plans discriminator) + `tier-upgrade-evaluate-perf.test.ts` (1 case at 1k members + R5-MED1 conflict-shape pin) + `tier-upgrade-pending.test.ts` (apply path) + `tier-upgrade-escalate.test.ts` (escalation-on-stale-pending) + `tier-upgrade-reconcile.test.ts` (orphan-pending sweep). R6-CRIT-2/3 honesty pass (vitest.config.ts:254-263) explicitly removed both files from coverage thresholds rather than asserting unreachable 100% branch on a unit suite that doesn't exist. **Trigger to add unit tests**: when integration coverage proves insufficient for a specific decision-tree branch (caught by /speckit.review or /speckit.staff-review), author a focused unit test for THAT branch — incremental authoring is preferable to a 22-test all-at-once mock-heavy suite. | Authoring 22 unit tests upfront (rejected): mock surface area would dominate; per project test-philosophy `[Verify each AS against spec]` integration coverage already binds the spec assertions; the marginal branch coverage gain doesn't justify the maintenance cost of a parallel mock-tree. Lowering the IT bar (rejected): IT against live Neon is the strongest correctness signal F8 has; downgrading to mocked unit tests would be a coverage regression. Skipping the file entirely from coverage thresholds (chosen): R6-CRIT-2/3 closes this; the tracking entry here documents the conscious decision. |
| **#8 — Phase 8 R10 deviations bundle (escalation-task queue ships dark with 5 forward-deferrable polish items)** (added Round 10 review-fix close 2026-05-10 / S4 + S7 + S8 + W12) | The Phase 8 escalation-task queue closes 22 of 27 staff-review findings inline; the remaining 5 are documented here for forward visibility rather than blocked at the Review Gate. **(a) S4 — `WIRE_ERROR_CODES` const tuple lives in two places**: the old in-component declaration was extracted to `_describe-error.ts` (R10 S11 close), but the inline producers in `dispatch-one-cycle.ts`, `detect-bounce-threshold.ts`, `admin-reject-reactivation.ts`, `retry-failed-reminders.ts`, and `accept-tier-upgrade.ts` (verify-pending-tier-upgrade task) STILL emit `escalation_task_created` directly without going through the new canonical `createEscalationTask` use-case (Phase 8 T208). Phase 9 cross-cutting OR a future sweep can DRY them up; the new use-case serves as the canonical seam for new producers. **(b) S7 — `escalation_task_created` audit-emit has no atomic-rollback guarantee on the 5 inline call sites** (the new use-case T208 wraps it in a single-tx try/catch with `throw e` to roll back; the inline call sites pre-date this contract and have only `logger.warn` breadcrumbs). Audit invariant Constitution VIII still holds (the task INSERT is atomic + audit emit is best-effort with structured-log breadcrumb), but the test invariant "1 row + 1 audit emit per attempt" is currently enforced by the new use-case only. **(c) S8 — `auditLog.retentionYears` + `auditLog.requestId↔correlationId` Drizzle wiring**: the schema currently maps `request_id` (DB column) to `requestId` (Drizzle alias), and persists ctx.correlationId into request_id when no separate requestId is supplied. The escalation-task lifecycle test pins this via raw SQL (`db.execute(sql\`select retention_years from audit_log where id = ?\`)`) because `retention_years` exists in the DB (migration 0039) but is not yet in the Drizzle inferred type. A Phase 9 schema sweep should expose both columns through Drizzle so the raw-SQL escape hatch can be retired. **(d) W12 — statement-breakpoint markers**: migration `0122_f8_phase8_escalation_year_in_cycle.sql` uses single-statement DDL only; no `--> statement-breakpoint` markers needed. Phase 9 cross-cutting may consolidate this into a multi-statement migration if other audit-or-RLS sweeps land, at which point the marker discipline applies. **(e) FR-043 multi-year cycle UX**: only the `<YearInCyclePill>` primitive ships in Phase 8 (T220); downstream surfaces (member detail timeline, cycle detail page, lapsed tab) wire the consumer in Phase 9+ per `tasks.md` Bucket F deferral list. | Inline emit refactor (rejected for Phase 8): the 5 inline producers are correct + tested; rewriting them to call the new use-case is a 5-file diff that risks introducing regression in well-trodden paths to gain "DRY" — Phase 9 sweep is the right home. Drizzle schema extension for `retention_years` + `correlation_id` (rejected for Phase 8): would touch every audit-log read site (broadcasts, payments, plans) for forward consistency benefit only — Phase 9 schema sweep is the right home. Adding the YearInCyclePill consumer to surfaces in Phase 8 (rejected): expands the spec scope (T220 said "primitive only"); the consumer wiring needs F8 Phase 7 multi-year metadata (`renewal_cycles.year_in_cycle` + `total_years`) populated for ≥1 cycle which depends on the cron dispatcher seeding it — Phase 9 ships the producer + consumer together. |

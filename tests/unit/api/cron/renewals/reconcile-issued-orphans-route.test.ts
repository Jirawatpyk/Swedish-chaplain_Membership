 ...
 * the reconcile-issued-orphans single-route daily housekeeping cron.
 *
 ...
 * `auto-draft-route.test.ts` two-flag precedent.
 *
 * Test matrix:
 ...
 *   6. 200 + skipped on `READ_ONLY_MODE=true` + `coordinatorSkippedReadOnly('reconcile_issued_orphans')`
 *   7. Happy path: BOTH `reconcileIssuedOrphansRunCompleted(t,'success')` +
 *      `reconcileIssuedOrphansRelinked(t, N)` emitted exactly once each
...
const reconcileIssuedOrphansMock = vi.hoisted(() => vi.fn());
...
vi.mock('@/modules/renewals', () => ({
  ...
  reconcileIssuedOrphans: reconcileIssuedOrphansMock,
}));
...
import { POST } from '@/app/api/cron/renewals/reconcile-issued-orphans/route';
...
describe('cron reconcile-issued-orphans route (107-auto-invoice Task 11)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('401 on missing Bearer + emits cron_bearer_auth_rejected audit', async () => {
    ...
    expect((event.payload as { route: string }).route).toBe(
      '/api/cron/renewals/reconcile-issued-orphans',
    );
    expect(reconcileIssuedOrphansMock).not.toHaveBeenCalled();
    expect(runCompletedMock).not.toHaveBeenCalled();
    expect(relinkedMock).not.toHaveBeenCalled();
  });
  ...
  it('200 + skipped on READ_ONLY_MODE=true + coordinatorSkippedReadOnly("reconcile_issued_orphans")', async () => {
    const env = (await import('@/lib/env')).env as {
    ...
    try {
      ...
      expect(skippedReadOnlyMock).toHaveBeenCalledWith(
  ...
  it('SUCCESS path emits BOTH runCompleted("success") AND relinked(N) exactly once each', async () => {
    reconcileIssuedOrphansMock.mockResolvedValueOnce({
      ok: true,
      value: {
        candidatesFound: 5,
        relinked: 1,
    ...
    expect(body.candidates_found).toBe(5);
    expect(body.relinked).toBe(1);
    expect(body.skipped_terminal).toBe(1);
    expect(body.skipped_conflict).toBe(1);
    expect(body.skipped_gone).toBe(1);
    expect(body.skipped_invoice_not_issued).toBe(1);
  ...
  it('catch path (use-case throws) → 500 + ONLY runCompleted("failure")', async () => {
    reconcileIssuedOrphansMock.mockRejectedValueOnce(

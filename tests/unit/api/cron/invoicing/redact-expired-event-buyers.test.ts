/**
 * 054-event-fee-invoices (speckit-review hardening, FIX B) — unit coverage for
 * the redact-expired-event-buyers cron's SCAN-LEVEL control branches that the
 * live-Neon integration suite cannot easily induce without poisoning real data:
 *
 ...
 * (tombstone + audit + blob purge + idempotency + retry + immutability) is
 * pinned by the integration suite against live Neon Singapore — this file owns
 * ONLY the scan-level auth + failure-isolation contract.
 */
...
// returns the zero-redaction result the route expects).
...
const eventBuyerPiiRedactedMock = vi.hoisted(() => vi.fn());
vi.mock('@/lib/metrics', () => ({
  invoicingMetrics: { eventBuyerPiiRedacted: eventBuyerPiiRedactedMock },
...
import { POST } from '@/app/api/cron/invoicing/redact-expired-event-buyers/route';
...
describe('redact-expired-event-buyers cron — scan-level branches (FIX B)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('(3) per-tenant failure ISOLATION: a poisoned tenant bumps tenantsErrored in a 200 body; the others are still swept', async () => {
    ...
    const errorOutcomes = eventBuyerPiiRedactedMock.mock.calls.filter((c) => c[0] === 'error');

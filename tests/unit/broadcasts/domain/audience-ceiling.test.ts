 ...
 * Across `maxDuration = 300` minus dispatch's own work that is ~950 contacts;
 * with a 20 % margin, ~760. The account is also on Resend's FREE plan, whose
 * 1,000-contact cap bites at ~987. **800 sits under both bounds** — the one
 * derived from latency and the one derived from the plan — which is why it is
 * a round number and not a computed one: two independent limits agreeing to
 * within 20 % do not justify false precision.
 */

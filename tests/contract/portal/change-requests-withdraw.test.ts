...
describe('DELETE /api/portal/change-requests/current — the attempt bucket (PR-3 S-5)', () => {
  it('the same 60 / 10 min per tenant + user bucket as the submit route: exhausted → 429 rate_limited + Retry-After after auth and READ_ONLY_MODE, before the use case — nothing withdrawn, no audit row, metric attempt_throttled', async () => {
    rateLimitCheckMock.mockResolvedValueOnce({ success: false, reset: Date.now() + 120_000 });
    const res = await call();
    expect(res.status).toBe(429);
    expect(res.headers.get('Retry-After')).toMatch(/^\d+$/);
    expect(await res.json()).toMatchObject({ error: 'rate_limited' });
    expect(rateLimitCheckMock).toHaveBeenCalledWith(`f114:withdraw-attempts:test-swecham:${SUBMITTER}`, 60, 600);
    expect(repo.rows.get(REQ)?.state).toBe('pending');
    expect(audit.events).toHaveLength(0);
    expect(metricRefused).toHaveBeenCalledWith('test-swecham', 'attempt_throttled');
  });

  it('READ_ONLY_MODE answers before the bucket is consumed (a frozen deployment costs no Redis round-trip)', async () => {
    readOnly = true;

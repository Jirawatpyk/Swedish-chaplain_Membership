...
beforeEach(() => {
  ...
  // B5 default — no TIN data resolved: matched rows enrich to null.
  tinPresenceMock.mockResolvedValue(new Map());
});
...
describe('T053 — GET /api/admin/events/[eventId] (detail contract)', () => {
  ...
  describe('buyerIsVatRegistrant enrichment (064 remediation B5)', () => {
    it('matched member resolved by the TIN lookup → buyerIsVatRegistrant carries the boolean (false = TIN-less member)', async () => {
      ...
      tinPresenceMock.mockResolvedValueOnce(new Map([['mem-1', false]]));
    ...
    it('matched member with a TIN → buyerIsVatRegistrant true', async () => {
      ...
      tinPresenceMock.mockResolvedValueOnce(new Map([['mem-1', true]]));

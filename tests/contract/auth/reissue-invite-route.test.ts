 ...
 * userId) resend throttle BEFORE the use case runs — 3/hour, keyed on the
 * target rather than the acting admin so N admins can't collectively
 * mail-bomb one inbox (the DV-11 rule; mirrors the F3 resend-verification
 * route's identical per-document pattern). Mock style follows
...
describe('contract: POST /api/auth/users/[id]/reissue-invite (Task 2)', () => {
  ...
  it('200 — admin + ok, hits the rate limiter with the per-(tenant,target) key, omits locale', async () => {
  ...
  it('429 — rate limited, with Retry-After header, use case never called', async () => {
    requireAdminContextMock.mockResolvedValueOnce(adminContext);

...
describe('contract: POST /api/members/[memberId]/contacts (addContact)', () => {
  ...
  it('409 conflict for a primacy reason says so — not "email already exists" (108 round 4, F4-#2)', async () => {
    requireApiPermissionMock.mockResolvedValueOnce(adminContext);
    addContactMock.mockResolvedValueOnce(
      err({ type: 'conflict', reason: 'primary_contact_race' }),
    );
    const { POST } = await import(
      '@/app/api/members/[memberId]/contacts/route'
    );
    ...
    const body = await res.json();
    expect(body.error.code).toBe('conflict');
    expect(body.error.details.reason).toBe('primary_contact_race');
    expect(body.error.message).not.toMatch(/email/i);
  });

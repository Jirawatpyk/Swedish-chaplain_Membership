 ...
 *
 * Proves that every one of the 37 `AUDIT_EVENT_TYPES` can be
 ...
 *
 * Strategy: for each of the 37 types we append a sentinel row
...
describe('integration: audit completeness — all 37 event types writable', () => {
  ...
  it('the full event-type list has exactly 37 entries', () => {
    ...
    expect(AUDIT_EVENT_TYPES.length).toBe(37);
    expect(new Set(AUDIT_EVENT_TYPES).size).toBe(37);
  });
});

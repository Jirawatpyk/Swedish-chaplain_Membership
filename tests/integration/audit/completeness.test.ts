 ...
 *
 * Proves that every one of the 42 `AUDIT_EVENT_TYPES` can be
 ...
 *
 * Strategy: for each of the 42 types we append a sentinel row
...
describe('integration: audit completeness — all 42 event types writable', () => {
  ...
  it('the full event-type list has exactly 42 entries', () => {
    ...
    // 016 RBAC v2: 36 → 37 (migration 0285 — permission_denied, the
    //                  fail-closed denial event for the permission evaluator).
    // F114 (migration 0301): 37 → 42 — the five change-request events
    // (submitted / decided / withdrawn / rate_limited / approval_setting_changed)
    expect(AUDIT_EVENT_TYPES.length).toBe(42);
    expect(new Set(AUDIT_EVENT_TYPES).size).toBe(42);
  });
});
